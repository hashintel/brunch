...
- The package `files` list gains a `!dist/probes` negation, dropping 68 entries and ~370 KB unpacked from the tarball. The probe scripts still build to `dist/probes` locally, so their build-path consumers are unaffected.
