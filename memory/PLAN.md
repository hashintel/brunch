...
- 2026-07-09 `tool-schema-convergence` (FE-1163) — sweep exhausted and ledger deleted after closing all required rows over the re-based 51-tool provider-facing Brunch schema surface. The single adapter seam (`src/.pi/extensions/shared/tool-schema.ts`) now covers Zod and TypeBox schema sources; row 11's registry oracle covers elicitor, executor, and dev toolsets for provider legality; `src/.pi/extensions/TOPOLOGY.md` and SPEC D118-L/I60-L carry the durable rule.
...
- **Status:** done 2026-07-09; sweep exhausted and temporary ledger deleted.
...
- **Aggregate DoD:** met: no required ledger row remains `spec`/`partial`; both legacy adapters (`exchanges/pi-schema.ts`, `shared/pi-tool-schema.ts`) are deleted; every in-boundary `parameters:` site routes through the single shared adapter; the registry-wide legality oracle (elicitor + executor + dev toolsets) is green.
- **Inventory authority:** exhausted temporary ledger `memory/cards/tool-schema-convergence--ledger.md` was deleted at closure. The durable result is this frontier completion note plus SPEC D118-L/I60-L and `src/.pi/extensions/TOPOLOGY.md`.
