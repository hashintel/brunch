...
- 2026-07-14 `petri-execution-parity` (FE-1195) — completed the old-`main` live-parity closure and review hardening: timestamped firing wire, structural pass/fail branches with complete failed-slice terminal evidence, stale serial-state retirement at parallel admission, journal-idempotent restart-stable terminal authority, causality-gated Petrinaut replay/export, strict staging/RPC definition contracts, and a view-only projection with mechanically pruned isolated places, preserved connected IDs/arcs, contextual labels, locale-independent ordering, and collision-free compact execution bands. Durable terminal evidence wins over later abandonment metadata; raw executor topology/markings and SSE firing order remain unchanged. Full per-slice attempt identity remains the current projection; standardized subnet grouping/folding should be revisited above roughly 12 slices without claiming color-fold parity. Manual Rust fixture comparison remains pre-PR outer evidence.
...
- `petri-execution-parity` ([FE-1195](https://linear.app/hash/issue/FE-1195/petri-execution-parity-isolated-slice-fan-in-and-epic-integration)) — ✓ built + review-hardened 2026-07-14; timestamped firing compatibility, structural verification verdict/failure identity, collision-free locale-independent viewer layout, strict staging definition parity, and the tightened `execute.run` replay contract are closed without changing executor topology, marking, or event order. Manual Rust fixture/Petrinaut comparison remains pre-PR outer evidence. Scope: [`memory/cards/petri-execution-parity--petrinaut-live-parity.md`](cards/petri-execution-parity--petrinaut-live-parity.md).
...
- **Status:** ✓ built 2026-07-14; awaiting PR tie-off and the manual Rust fixture/Petrinaut comparison.
...
- **Completion:** timestamped wire, verdict topology, terminal evidence, and the compact deterministic viewer projection are implemented and locally gated in [`memory/cards/petri-execution-parity--petrinaut-live-parity.md`](cards/petri-execution-parity--petrinaut-live-parity.md). The viewer mechanically omits only zero-degree unmarked places, preserves connected IDs/arcs, and carries contextual names plus finite coordinates through replay/SSE/RPC without changing raw topology, runtime marking, or firing order.
