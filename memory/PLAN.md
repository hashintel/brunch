...
- **Kind:** structural
- **Status:** active — **slices 1–3 + 4A done** (slice 1: executability contract + repair + `Toolchain` descriptor; slice 2: toolchain-agnostic cook prompts + runner — `ToolchainTestRunner`, `sliceTestTask`/`epicVerifyTask`, `test-writer.md` de-hardcoded, resolved from `plan.profile`; slice 3: build-architect prompt — `projectPlanningContext` lifts spec relation edges into `req-*` slice space, each slice's acceptance criteria + the three reference fixtures (comment-stripped, `plan-exemplars.ts`) are inlined into the planning prompt, output schema unchanged; slice 4A: file-ownership contract — `Slice.writes?: string[]` + `file-write-conflict` design-class warning in `checkPlan` (single-writer-per-file, never auto-repaired) + D160-K amendment (I132-K); all 2026-06-09). **Slice 4 was split** (oracle 2026-06-09): 4A is the deterministic contract half (done); **4B (emitter/LLM authors `writes` + requirement decomposition + join-slice synthesis) is deferred** to a later sub-slice + the slice-5 eval harness, because the current planner schema only enriches existing `req-*` slices and cannot honestly synthesize joins. **Next: slice 4B or the eval harness.** SPEC records A100-K (partially-validated), D160-K (amended), D167-K (+ base/emitted profile-split refinement), I129-K, I130-K, I131-K, I132-K, Future Direction §Cook plan generation.
