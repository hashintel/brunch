...
- `walkthrough-remediation-1` ([FE-1180](https://linear.app/hash/issue/FE-1180/walkthrough-remediation-sweep-1-run-ac-findings-closure)) — coverage-shaped remediation sweep over the run-A/run-C findings ledger (`TESTING_FINDINGS.md` A1–A10, C1–C5) plus the 2026-07-09 `ln-induct` promoted lenses. Runs **before** walkthrough runs B/D — those runs' pre-written concerns (seed legibility, orientation-before-domain-questions, populated-cwd/brownfield entry) are better witnessed against the remediated surface and double as closure evidence. Definition below.
...
- `walkthrough-evidence-batch` ([FE-1167](https://linear.app/hash/issue/FE-1167/walkthrough-evidence-batch-outer-loop-checks-for-merged-orientation)) — the one batch owning all outer-loop residue from the merged lanes (FE-1134 generative-menu evidence, FE-1137 thin/rich Execute beats + deferred orientation-choice questions, FE-1124 Card 3 + seed worklist, FE-1107 KA-conversation residue, and — folded 2026-07-08 from the retired handoff — the FE-1164 walkthrough items). Unblocked by the FE-1164 and FE-1169 (#305) merges; prefer running after `walkthrough-remediation-1` lands so the beats witness the remediated surfaces (and note remediation row 5's conduct changes shift what "expected conduct" means for the generative beats). Closes arc `deterministic-orientation`. Definition below.
...
- **Inventory authority:** ledger at `memory/cards/walkthrough-remediation-1--ledger.md` once `ln-scope` authors it; PLAN owns the frontier id and sequencing. Findings stay in `TESTING_FINDINGS.md` (evidence, not a second plan).
...
frontiers:
  Active:
    alpha-walkthrough-lane (not a frontier)
      status: runs A + C done + collated 2026-07-09; runs B/D deferred -[hard]-> after walkthrough-remediation-1
      feeds: walkthrough-remediation-1 (findings inventory), spec-posture (run D outer oracle),
             walkthrough-evidence-batch (FE-1167 overlap rows when naturally witnessed)
  Next:
    walkthrough-remediation-1 (FE-1180)
      status: admitted 2026-07-09 (post-walkthrough collation); branch ln/fe-1180-walkthrough-remediation
      shape: coverage-shaped remediation sweep; inventory = TESTING_FINDINGS.md runs A/C + induction lenses
      ledger: memory/cards/walkthrough-remediation-1--ledger.md (authored at ln-scope)
      rows 6-7 -[evidence]-> ln-diagnose (JSON leak, seed insertion)
      -[hard]-> walkthrough runs B/D (runs execute after remediation; their beats are closure evidence)
      promoted rows: spec-posture (stays open in ledger until landed)
