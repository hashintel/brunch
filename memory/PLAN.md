...
- **Post-hardening validation:** FE-1348 freezes the current alpha boundary into a closed usage/verification-path inventory. A51-L's colleague walkthrough is complete; two product rows remain open: Execute evidence and cross-surface settlement. CS1 and SW2 are fixed in code; the consumed final witness reached one canonical settled proposal without SW2 duplication, but React did not make settlement or its Approve/Request changes/Reject continuation semantics legible/actionable, so the user stopped before approval. The cross-surface row remains partial under `shared-session-host-cutover` repair plus fresh-authorization re-entry. The sweep records incidental simplification findings without widening into fixes. Existing provider campaigns, KA evidence, comparison witnesses, legacy-data migration, cross-platform rendering, and actual publication retain their current owners and gates.
...
- **Members:** `shared-session-host-tracer` ✓ → FE-1348 `post-hardening-alpha-validation` A51-L and standalone-web rows ✓ (the frontier's other rows are product-wide) → `shared-session-host-cutover` (CS1/SW2/SW3 are fixed in code, and FE-1348's latest one-shot witness passed SW3's pre-approval semantic gate; the still-open settlement row now waits on fresh authorization in a handoff context that demonstrably keeps its sole host/browser live for the user's decision).
...
- `post-hardening-alpha-validation` ([FE-1348](https://linear.app/hash/issue/FE-1348/validate-current-brunch-usage-and-testing-paths)) — **active, evidence-gated coverage frontier:** validate the frozen current alpha entry/usage/verification inventory through real source, product, dev, comparison-gate, and installed-package paths; disposition incidental audit findings without implementing them in the sweep. A51-L's colleague walkthrough is complete; two product rows remain open: Execute evidence and cross-surface settlement. CS1/SW2/SW3 are fixed in code, and the latest one-shot witness passed the repaired pre-approval React/JSONL gate with one settled requirement, zero edges, and one canonical three-choice set. The delegated host then exited before the user could approve, so no settlement/reload evidence exists. Cross-surface re-entry requires fresh authorization and an execution/handoff context that proves its sole host/browser remain live for the user's decision. Definition and active sweep ledger below.
...
- `shared-session-host-cutover` — tracer, A51-L, and standalone-web evidence are complete; FE-1348's CS1/SW2/SW3 repairs are fixed, and the latest one-shot witness outer-proved SW3's pre-approval React semantics without reaching approval because its delegated host did not survive the handoff. Cutover closes the remaining enumerated shared-contract surface without forcing physical host unification; FE-1348's fresh settlement witness is independently gated on reliable handoff lifetime plus authorization. Definition below.
...
- **Why now / unlocks:** FE-1311–FE-1321 pruned, reconciled, isolated, and hardened the live stack, but the broad alpha walkthrough script predates those changes. The frozen current-boundary pass supplied the A51-L judgment; its remaining evidence supplies the product-level confidence needed before shared-session-host cutover or another architecture-sensitive frontier.
...
- **Classification:** tracer, A51-L, and standalone-web evidence complete; SW2 and CS1 are fixed in code. FE-1348's consumed final witness proved one canonical settled review set without SW2 duplication but stopped before approval because React did not render settlement or Approve/Request changes/Reject continuation semantics legibly; the cross-surface row remains partial pending this frontier's semantic-projection repair and fresh authorization.
...
active:
  post-hardening-alpha-validation (FE-1348)
    ...
    open: execute interaction | cross-surface settlement (CS1 + SW2 fixed; final witness stopped before approval on React settled-review/continuation semantic legibility)
    ...
    relevant_rows: cross-surface settlement -[hard]-> shared-session-host-cutover
