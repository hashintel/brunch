...
- **Current execution pointer:** Active chain `memory/cards/renderer-golden-coverage--house-style-chain.md` (authored 2026-06-16, replacing the superseded `--render-stage-chain.md`). Card 1 (substrate) and Card 2 (`<workspace>`, commit `d37c8a2a`) are **done**; Card 3 (`<specification>`) design pass is **approved 2026-06-16** (Overview · Sessions md-table · Gaps TOON sorted band-then-priority; graph block deferred) and the card is **next** to build. The graph-render migration (which absorbs the deferred `<specification>` graph block) and the `<session>` render are scoped in a later pass — graph representation already design-it-twiced (overview => G-D, neighborhood => G-C).
