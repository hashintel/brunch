...
- `epic-verify-recovery` — **FE-884**, branch `ka/fe-884-epic-verify-recovery` (stacked on FE-883's `ka/fe-883-worktree-gc`). Make a failed epic verification recoverable instead of terminal: a remediation loop on the verify-epic fail-sibling (detect-and-reject + dual re-verify + diff-transfer round-trip), epic infra/timeout classification (B), then partial promotion (C). Slices A + B landed; C remains. Per-branch queue in `memory/CARDS.md` (FE-884 on this branch). **Also landed here (FE-883 seam fix, surfaced by spec-49 dogfood):** the I124-K fold/seed phantom-deletion bug — `seedSliceSandboxFromDeps({preserveExisting})` skipped a dep's edit to a base-checkout file (the dependent inherited the run-base file via `ensureSliceWorktree`), so the fold dropped it (`@xyflow/react`/`d3-force`/`index.css`) and the dual re-verify never converged. Fixed by overlaying dep edits onto base-checkout files while preserving slice-authored paths (content vs HEAD).
...
- **Linear:** FE-894
- **Kind:** hardening
- **Status:** in progress (2026-06-18) on `ka/fe-894-cook-latency-hardening` (stacked on FE-884). **P0 landed** — `runPi` returns `{output, usage, timingMs}`; per-action token usage (incl. cacheRead/cacheWrite) + cold-start/prompt-turn timing now ride in the `tests-written`/`code-written`/`epic-verified` report payloads. **P1 is next.** Backed by analysis in `docs/design/cook-latency.md` + an ln-spike that retired the prompt-caching uncertainty (see Findings).
...
- **Build order (slices — keep in CARDS/session, do not fragment):**
  - **P0 (prerequisite) — LANDED 2026-06-18** — `runPi` reads usage via `session.getSessionStats().tokens` after `prompt()` resolves (clean: fresh-session-per-action means session stats == action stats — sidesteps the text-only `subscribe` stream) and times cold-start vs prompt turn; returns `{output, usage, timingMs}` and the writer/verify actions attach `usage`+`timingMs` to their report payloads. `sessionStatsToUsage` is pure + defensive. Makes prompt-cache hit/miss and per-action latency observable in `reports.jsonl` — the evidence base for prioritizing P1–P5.
  - **P1** — split during scoping (2026-06-18):
    - **P1a `cacheRetention` short→long (1h) — BLOCKED.** The high-level pi `createAgentSession` API exposes no `cacheRetention` knob (`CreateAgentSessionOptions`/`PromptOptions` have no such field; it's a low-level pi-ai *stream* option defaulting short, with no public pass-through). Needs an upstream pi change or the `agent-extension-host` coordination — not a local edit. Parked until then.
    - **P1c `stagger-prime` parallel slices — buildable, brunch-owned (the parallel-slice win).** When N same-action slices fire in one batch with a cold-but-identical prefix they all miss+write; release the first, then start siblings once it has primed the prefix (turns N writes → 1 write + N−1 reads). Pure orchestrator firing timing (`net-compiler`/`petri-net` `runParallel`), **independent of P1a** (siblings fire seconds apart, well inside the 5-min TTL) and **does not touch the pi session config / agent-extension-host contract**. Open risk: a fixed time-stagger may not reliably prime — the robust release signal is the first sibling's `cacheWrite` from P0 telemetry, not a sleep.
    - **P1b prefix-stability guard** — small regression test that `buildSessionOptions` yields a byte-identical system-prompt prefix per action type (spike found it already static; this pins it).
