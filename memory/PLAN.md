...
- **Acceptance:**
  ...
  - **`<specification>`** context render (spec header/readiness + graph overview + ranked gaps + per-spec sessions) is built and golden-locked; **`countTurnEntries` is audited** against the current JSONL entry model before the session turn-count is surfaced.
  - **`<session>`** context render migrates runtime-frame + mentions + world-update + transcript onto the house style; goldens re-locked.
  - **Tool-result renders** (`graph/graph-slice`, `graph/node-neighborhood`, `exchanges/*`) migrate onto the md-pen substrate; goldens re-locked; the legibility rule (neighborhood projects relations to prose, no structural leak) preserved.
...
- **Current execution pointer:** Active chain `memory/cards/renderer-golden-coverage--house-style-chain.md` (authored 2026-06-16, replacing the superseded `--render-stage-chain.md`). Card 1 (substrate adoption: md-pen + TOON + stringify-tree + `<section>`) is done; Cards 2 (`<workspace>`) and 3 (`<specification>`) are **design-gated** — their exact output shape needs a collaborative design pass (approved sketch appended to the card) before build. `<session>` + tool-render migration are scoped in a later pass.
