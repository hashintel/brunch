...
- 2026-06-05 `dev-seed-fixtures` — Done: spec-scoped graph LSN hardening. Replaced workspace-global graph-clock/change-log semantics with `(spec_id, lsn)` storage and selected-spec LSN allocation through `CommandExecutor`; updated graph snapshots, RPC expectations, prompt context wording, migrations, seed-fixture assertions, and canonical docs so sibling-spec mutations no longer make an unchanged spec appear stale. Verified: `src/graph/command-executor.test.ts`, `src/graph/command-executor/commit-graph-batch.test.ts`, `src/graph/snapshot.test.ts`, `src/graph/seed-fixtures.test.ts`, `src/rpc/handlers.test.ts`, `src/agents/contexts/graph.test.ts`, `npm test`, seeded-dev-rpc smoke.
