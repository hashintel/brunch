...
- `epic-verify-recovery` (FE-884, PR #232, merged 2026-06-22; slices A + B) — a failed epic verification is recoverable, not terminal: remediation code agent on the folded `__epic__` tree bounded by `epic-retry-budget`, detect-and-reject + dual re-verify guards, diff-transfer round-trip to the slice branch so the fix promotes (A; production action registered in `createPiActions` per `79376fe0`); infra/timeout fail-path split under a separate budget, `ETIMEDOUT`→infra, 60s→180s ceiling (B). Establishes I138-K, D170-K. **Slice C (partial promotion) descoped → re-homed as `cook-partial-promotion` (Parallel).**
- `cook-artifact-lifecycle` (FE-883, PR #224, merged 2026-06-19) — wired the `run-artifact.ts` git-merge composer into the live promotion/verify path (brownfield 3-way `merge-tree` fold, fail-closed on real conflict; greenfield keeps the file-copy union), epic-verify + probes resolve deps in the merged tree, run worktree/branch GC, plus serve-reliability hardening (action-timeout raise, Opus orchestration model, slice-grid abort fidelity, `serve --land`). Verified tree now equals shipped tree. Establishes I137-K, D169-K; amends I124-K, preserves I135-K.
...
- ~~**merge deep-copies a clobbered node_modules → ENOSPC**~~ — **resolved (2026-06-18)**: an in-slice `npm install` replaces the shared `node_modules` symlink with a real tree, which `walkFiles` then deep-copied per-slice + per-epic until the disk filled. Fixed by skipping `SHAREABLE_TOP_LEVEL_ENTRIES` by name (not just by symlink-ness) in `epic-sandbox-merge.ts`; re-link via `linkSharedTopLevelEntries` unchanged. **Dep-resolution gap closed for the common case (2026-06-18, D169-K/I137-K):** the epic verify tree now links `node_modules` from a completed slice that installed deps (superset of the parent's pre-run install) over the parent, so a single slice's added dep resolves under verify-epic; divergent multi-slice installs resolve last-in-plan-order with the divergence reported (assumes a complete in-slice install, A101-K — needs outer-loop validation). **Still deferred:** full union of divergent installs via lockfile-merge + reinstall into a shared store (the "reconstruct derived from source" half of the git-merge remodel); run-dir GC remains operator-owned (`worktree.ts`).
...
- `cook-latency-hardening` — **FE-894**, branch `ka/fe-894-cook-latency` (off main, post-FE-1049 merge). Latency-first cook perf. **P0 telemetry + lever ① (harness prior-art prime) landed**; the measured run showed ~75% of wall-clock is test-authoring rediscovery, not caching. ②/③ resolved (see definition); ①'s wall-clock payoff is pending a measured cook run.

### Horizon

**Full cook orchestrator — Arc 2 (full orchestrator; autonomy ladder, gated behind the semantic/Petri-Phase-3/4 substrate):**

- `interactive-recovery` — keystone safety rung: on rework-budget exhaustion or irreducible oracle ambiguity, synthesize a question into a `qa`/`strategy` secondary chat; the answer resumes the run. Depends on chat runtime (FE-716, done) + `durable-resume` (Petri marking persistence, split from the Phase-4 sim oracle so this rung is **not** gated on FE-700/Phase-3) + `changeset-ledger` (FE-701). Do first — makes the orchestrator safe to run unattended before re-plan/intent-verification are perfect; near-term-reachable on Arc-1 substrate.
...
- `adaptive-replan` — architect amends the plan from execution feedback; recompile the affected sub-net + resume. Depends on Petri Phase 3 (`petri-graph-compilation`, the hard gate) + `durable-resume` + FE-738's deferred stale-graph criterion. The execution-side instantiation of the architect-generator-loop pattern (distinct from the product-layer `architect-generator-loop` graph-proposal frontier); highest cost, last rung.
...
- `durable-resume` — persist a run's Petri marking and re-enter the interpreter at it. The resume primitive **split out of `petri-simulation-oracle`** so `interactive-recovery` (and later `adaptive-replan`'s resume step) need not inherit the Phase-3/FE-700 gate. Smaller than the analysis oracle — marking persistence + a resume entrypoint, no reachability/deadlock analysis. Near-term-reachable on the Arc-1 substrate.
- `petri-simulation-oracle` — reachability analysis, deadlock detection (Phase 4 analysis oracle). Planning oracle for plan-shape defects. Resume split out → `durable-resume`. Depends on `petri-graph-compilation`.
...
- `architect-generator-loop` — autonomous architect proposing workspace intent-graph edits via the changeset/reconciliation pathway (product layer; distinct from Arc-2 `adaptive-replan`, which amends cook *execution plans*). Shared generator-loop pattern, different surface.
...
- **Linear:** FE-894
- **Kind:** hardening
- **Status:** in progress (2026-06-24) on `ka/fe-894-cook-latency` — **replanted onto `main`** after the FE-1049 merge (main now carries the in-process pi harness + unified runner + epic-remediate, so the work lands cleanly off main; the prior FE-884-stacked branch is retired). **P0 telemetry + lever ① landed**; a real multi-slice cook run (~85 min, FE-1049 graph-canvas) showed **75% of wall-clock is *test-authoring*** rediscovery, not verify/caching — so ① (harness prior-art prime) is the headline lever, and the topology-guess ranking (P3-first) is demoted. ①'s wall-clock payoff is **unproven (A102-K)**: a first A/B (2026-06-24, `todo-cli` spec ± notes) showed **no win** (notes-OFF ~8% faster on test-authoring, within noise) — but on a trivial spec with no rediscovery, so it tests ① off-target. Draft PR **#257** open (off main). **Recorded and parked:** the rediscovery-heavy graph A/B (the real test) and the "gate emission to non-obvious seams" follow-up are deferred. See **Measured findings** + **Build order** below.
...
- **Measured findings (real cook run, 2026-06-23 — FE-1049 graph-canvas, ~85 min, via P0 telemetry):** Phase split — **test-authoring 3860s (75%)**, implementation 1255s (21%), 3× epic-verify 690s (~13%); tests:code ≈ **3.07:1**. Serial critical-path floor ≈ 5100s: `types (282, solo gate) → foundation verify (179) → canvas-join (900) → canvas verify (285) → route-join (1260) → route verify (226)`; the **join + route slices own ~3460s of the path**, and their cost is *test-authoring*, not execution. **This validates the spike's caching premise but overturns the topology-based ranking**: the levers that move the needle attack test-authoring rediscovery on the critical path (①, ②), not the verify/cache path (P3 is ~13%, P1c is cold-start only). Combined ①+②+④ on the CP ≈ **15–20 min off an 85-min run** with no parallelism change.
- **Why now / unlocks:** FE-743 proved parallel beats serial but never tuned the harness; the analysis surfaced latency levers, but only measurement could rank them. P0 + the measured run did that. Independent of the Arc-1/Arc-2 feature ladder — pure harness hardening on a settled seam.
- **Build order (re-ranked by the measured run, 2026-06-23 — keep slice cards in CARDS/session, do not fragment):**
  - **P0 (prerequisite) — LANDED 2026-06-18** — `runPi` reads usage via `session.getSessionStats().tokens` after `prompt()` resolves and times cold-start vs prompt turn; returns `{output, usage, timingMs}`; writer/verify actions attach `usage`+`timingMs` to report payloads. `sessionStatsToUsage` is pure + defensive. This is the instrument the measured run read.
  - **① harness prior-art prime — IN PROGRESS (biggest measured win, brunch-owned).** Inject project build/framework prior-art into slice/epic agent tasks so critical-path join/route agents stop rediscovering it per slice. Est. **~700–900s** off the CP. **Decision (2026-06-23):** source is **plan/architect-supplied** (`Plan.harnessNotes`), not toolchain (toolchain is runner-level; prior-art is target-codebase architecture). Split: **1a injection plumbing — LANDED** (`Plan.harnessNotes`; `sliceTestTask`/`epicVerifyTask`/`epicRemediateTask`/write-code task append it via `withHarnessNotes`, byte-identical when absent). **1b architect emission — LANDED** (`architectDraftSchema.harnessNotes` optional; `buildArchitectPrompt` rule 8 asks for cross-cutting build/framework prior-art, not test-runner setup; `materializeArchitectedPlan` carries `draft.harnessNotes` → `plan.harnessNotes`). **Wired end-to-end & unit-tested.** Remaining: **a real LLM cook run to confirm the ~700–900s win = frontier acceptance #3 (outer loop, not yet run).**
  - **② cheap foundation oracle — BLOCKED ON DATA (2026-06-23, scoped).** The premise (the foundation `types` slice's 227s is an *eval*/runtime-TS-compile, swappable for a type-probe) **conflicts with the study's own phase table**, which attributes the 227s to test-*authoring* on `types`. No `reports.jsonl` with P0 `timingMs` survives on disk, so it can't be disambiguated without a fresh measured run. **If 227s is authoring → ② is not a separate lever; it's ① applied to the foundation slice (already covered).** **If 227s is eval → ② is a real cheap-oracle change.** Cannot scope a buildable slice until a P0-instrumented run shows the split. Converges with ①-validation: the same measured run resolves both.
  - **③ = P1c `stagger-prime` parallel slices — DROPPED 2026-06-23 (scoped, found net-negative).** Two problems: (1) **no release signal** — the intended "fire siblings once the first's `cacheWrite` lands" is unimplementable: P0 reads usage only *after* `prompt()` resolves, and `session.subscribe` is text/lifecycle-only, so there's no mid-stream cache signal; the only reliable fallback (release after the first sibling fully completes) serializes a whole ~10–20m join-slice action. (2) **the win is tiny** — ③ targets cache-*write* overhead on the shared prefix (system prompt + tool defs, a few KB; ~25% premium + ~1–2s prefill saved per sibling), but the measured cost is **agent work / test-authoring rediscovery**, not prefix prefill. So it saves *seconds* per batch for a structural `runParallel` change. Lever ① attacks the real cost directly. Park unless a cheap mid-stream usage hook appears.
  - **④ overlap verify / scheduling — NEW.** 3 epic verifies run serial (690s) + ~11×20s semantic/scheduling gaps; overlap the non-dependent semantic step off the CP. Est. **~150–220s** gap reclaim.
  - **P3 (DEMOTED — measured ~13%)** — trim re-verification + the opus read-only `evaluate-done` gate (`pi-actions.ts:511`); incremental build/test scope. Smaller than the topology guess implied; do after ①/②/④.
  - **P2** — right-size `agentPoolSize` (default = `plan.slices.length`, `net-compiler.ts:49`) vs the Anthropic rate ceiling (tuning).
  - **P1a `cacheRetention`→1h — BLOCKED** (pi `createAgentSession` exposes no `cacheRetention` knob; needs upstream pi / `agent-extension-host` coordination). **P1b prefix-stability guard**, **P4 session/prefix reuse**, **P5 model routing** — small follow-ups.
- **Acceptance:** (1) ✅ a cook run emits per-action usage + timing telemetry in `reports.jsonl` (P0); (2) ✅ findings confirmed by the measured run (test-authoring dominates; caching premise holds); (3) the headline lever **①** (and ≥1 of ②/③/④) lands with a measured wall-clock improvement on a representative multi-slice run, no behavior change (engine contract suite green on both engines).
...
- **Depends on:** **satisfied on main** — the in-process pi session harness (`createAgentSession`/`getSessionStats`, FE-841), the unified test runner (FE-872), and epic-remediate (`epicRemediateTask`, FE-884) all merged to `main` via the FE-1049 line, so the work lands off `main` with no parental stacking. **Soft** — coordination with `agent-extension-host` for P1a only (the pi session-config contract). P0/① are local to `pi-actions.ts`/`plan-architect.ts`/`plan-materialize.ts`.
- **SPEC:** reconciled 2026-06-23 (ln-sync) → **D171** (cook latency = test-authoring rediscovery; lever is harness prior-art, not caching; pi session config owned by agent-extension-host) + **A102-K** (①'s wall-clock payoff unproven, pending the deferred measured run). Lever-outcome detail (②/③) stays here in PLAN.
...
- **Acceptance:** the three cards in Status — (1) data-driven registry + 4 TS profiles, (2) live + strict selection, (3) architect profile classification — all landed.
...
- **Traceability:** Requirements 46–50; refines I130-K; greenfield regression oracle (3 reference fixtures + smoke remain the guard; not a mandate to preserve mode-forked internals). **D160-K boundary:** detection is plan-time profile-*id* resolution (an input to authoring), not architect host-introspection — D160-K constrains the architect/authoring stage, not profile resolution, so `brunch-detect` needs no D160-K amendment.
...
- **Acceptance:** (1) architect emits a generic wiring slice for feature epics; (2) cook agent resolves real wiring by reading the worktree; (3) integration oracle gates completion on product reachability, mode-aware where the product meaning differs (brownfield reachable-in-app / greenfield self-compose); (4) the brownfield orphan-module regression (`spatial_graph_layout`) is caught; (5) greenfield regression oracle holds — 3 reference fixtures + greenfield smoke score identically; (6) `writes` advisory in brownfield, authoritative in greenfield where that semantic distinction still matters; (7) the wiring agent is an `execute`-mode plugin on `agent-extension-host`, not a bespoke `pi` call.
...
- **Traceability:** Requirements 46–50; D160-K (amendment pending), D161-K, D167-K, A98; FE-800 integration-blind follow-on; greenfield regression oracle.
...
- **Name:** Cook artifact lifecycle — git-merge composer wiring + worktree/branch GC
- **Linear:** FE-883 — **done** (PR #224, merged 2026-06-19)
- **Kind:** structural (amends I124-K on wiring) / hardening
- **Shipped:** wired `run-artifact.ts` (`commitSliceWorktree` + `foldSliceBranches`) into the live promotion/verify path, replacing the file-copy union — brownfield composes by 3-way `git merge-tree` (slice commits carry dep commits as parents so dep-seeded files merge as edits; **fail-closed** on a real conflict), greenfield keeps the file-copy union (no common ancestor); epic-verify + probes resolve deps in the merged tree; run worktree/branch GC; serve-reliability hardening (action-timeout raise, Opus orchestration model, slice-grid abort fidelity, `serve --land`). Verified tree equals shipped tree.
- **Traceability:** Requirement 49; establishes I137-K, D169-K; amends I124-K (file-copy → fork on `plan.mode`), preserves I135-K; A49. Precursor to Horizon `parallel-merge-conflict-reconciliation`.
- **Design docs:** `docs/design/orchestrator.md`.

### epic-verify-recovery

- **Name:** Epic verification recovery — recoverable epic verify instead of terminal halt
- **Linear:** FE-884 — **done** (PR #232, merged 2026-06-22; slices A + B)
...
- **Linear:** unassigned
- **Kind:** structural
- **Status:** horizon (Arc 2 keystone) — gated on `durable-resume` (not the full Phase-4 oracle; **not** on FE-700/Phase-3).
- **Objective:** When a slice exhausts its rework budget or an oracle rejects on irreducible ambiguity, synthesize a coherent question (what's blocking, options) and land it as a turn in a `qa`/`strategy` secondary chat; the user's answer resumes the run from durable markings. Makes unattended failure graceful (ask, don't orphan or ship-wrong) and fuses the interview and execution substrates into one loop. The asking reuses the existing `elicit`-mode / secondary-chat substrate (FE-716) — not a new Q&A channel; the load-bearing new work is **resume from durable markings** (`durable-resume`, the resume primitive split out of the Phase-4 simulation oracle). **Composition:** this is the ask/resume *channel*; paired with `adaptive-replan` (the graph-mutation *engine*) it becomes **human-triggered replan**, so the question payload is structured to be plan-amendment-ready from the start, not free-text — even though the answer only resumes the unchanged plan until `adaptive-replan` lands.
- **Why now / unlocks:** The graceful-degradation layer that makes the orchestrator safe to run unattended even before re-plan and intent-verification are perfect. Highest value-per-cost Arc 2 rung; do first — and reachable far earlier than the rest of Arc 2 once `durable-resume` is split from the simulation oracle.
...
- **Depends on:** `chat-runtime-secondary-chats` (FE-716, done), `durable-resume` (Petri marking persistence; split from `petri-simulation-oracle` so this rung is **not** gated on FE-700/Phase-3), `changeset-ledger` (FE-701) for durable answers, `agent-extension-host` (question agent = `execute`-mode plugin).
...
- **Linear:** unassigned
- **Kind:** structural
- **Status:** horizon (Arc 2, highest cost) — gated on Petri Phase 3 (mutable plan graph, the hard gate) + `durable-resume`. **Genuinely un-shortcuttable** — the architect must amend against current world state, which needs the plan to be a mutable, revision-tracked graph.
- **Objective:** When execution reveals the plan is wrong (missing dep, absent integration point, wrong scope), re-invoke the architect with execution feedback + world state to amend the plan, recompile the affected sub-net, and resume — instead of retrying the same frozen slice. Requires the plan to be a mutable graph (Phase 3) with durable, resumable markings (`durable-resume`) and stale-graph detection (FE-738 deferred criterion 5). **Composition:** paired with `interactive-recovery` this is **human-triggered replan** — the recovery question carries the amendment proposal for approval before the graph mutates; the same engine also runs autonomously when no human gate is configured.
- **Why now / unlocks:** Removes the last "plan was right" assumption from autonomy — the orchestrator becomes self-correcting. The **execution-side instantiation of the architect-generator-loop pattern** — the product-layer `architect-generator-loop` frontier is the workspace-graph-proposal sibling (same loop, different surface; not a duplicate). Most structurally expensive (touches the core substrate commitment); last rung.
...
- **Depends on:** `petri-graph-compilation` (Phase 3, mutable plan graph — the hard gate), `durable-resume` (resume from markings), FE-738 deferred criterion 5, `intent-graph-semantics` (FE-700), `changeset-ledger` (FE-701), `agent-extension-host` (replan agent = `execute`-mode plugin). `petri-simulation-oracle` optional — simulate an amended sub-net before resuming.
...
- **Relationship to `adaptive-replan` (Arc 2):** same architect-proposes-then-HITL-review loop, **different surface** — this proposes **workspace intent-graph** edits (product layer, over the changeset/reconciliation substrate); `adaptive-replan` amends **cook execution plans** from run feedback (orchestrator layer). Not a duplicate; do not fold. `adaptive-replan` is the execution-side instantiation of the shared generator-loop pattern.
  ...
  └──→ petri-semantic-lanes (Phase 1: two-lane subnet + §7 events — done)
        └──→ petri-parallel-execution (Phase 2: concurrent firing + resource pools — done)
              ...
              └──→ petri-declarative-routing (Phase-3-prep: topology-level Guard predicates; FE-700-independent — done)
                    ...
                    ├──→ durable-resume (resume primitive split out of Phase 4: persist marking + re-enter interpreter; FE-700-independent; unblocks Arc-2 interactive-recovery near-term)
                    └──→ petri-simulation-oracle (Phase 4 analysis: reachability, deadlock; resume split → durable-resume; declarative-routing prerequisite satisfied; Phase 3 still needed for graph-derived gates)
