...
- **Current execution pointer:** Blocked on prep-envelope tie-off. When unblocked: **(1)** create `src/db/` with Drizzle schema for nodes + edges + change_log + graph_clock per D52-L; **(2)** scope the narrow `CommandExecutor` result contract and one-transaction LSN/change-log skeleton in `src/graph/`; **(3)** widen to intent-plane single-node/edge CRUD against the locked schema; **(4)** add `commitGraph` batch mutation with intra-batch and existing-node reference resolution (D53-L, I34-L); **(5)** add graph snapshot readers at cursory and neighborhood detail levels (I35-L); **(6)** add the reconciliation-need substrate and oracle-plane stub tables. Keep M4 thin enough to falsify A3-L / A4-L / A6-L / A8-L / A14-L-readiness before widening CRUD or coherence homes.
