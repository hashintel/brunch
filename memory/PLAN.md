...
- **Current execution pointer:** Structured-exchange schema lock complete and structurally guarded: present/request/capture `toolResult.details` and active structured-exchange tool params are Zod-authored; the only Pi `TSchema` accommodation is `src/.pi/extensions/structured-exchange/pi-schema.ts`; active Pi tool files call canonical projectors instead of hand-building details; shared `tool_meta`/display/header atoms compose present/request/capture variants; session pending recovery no longer uses a TypeBox semantic schema; the proof-era `brunch.structured_exchange.result` path is retired. Remaining FE-809 work is approval-to-`acceptReviewSet` product wiring and the real `project-graph` proposal probe.
