...
- `renderer-golden-coverage` — **reshaped 2026-06-16 by D83-L** (context-render house style: md-pen + TOON + stringify-tree, `<workspace>`/`<specification>`/`<session>` scope clustering) and now **actively worked on its own branch `ln/fe-870-renderer-golden-context-tools`** (FE-870), even though it remains below the demo lane in priority and is **never a ship gate**. The old prepared chain is superseded; re-author via `ln-scope` (substrate → `<workspace>` → `<specification>` → `<session>` → tool-render migration). Open prioritization question: whether to keep it below the demo line or treat the active branch as a parallel track.
...
- **Acceptance:**
  ...
  - **`<specification>`** context render is built and golden-locked as **Overview** (spec id/title + graph size + soft readiness, md bullets) · **Sessions** (per-spec, md table — small bounded roster per the D83-L size-aware rule; columns name/file/turns) · **Gaps** (ranked `read_elicitation_gaps` as TOON, sorted band-then-priority). The **graph overview is deferred** to the graph-render-migration step (the Overview block carries only the graph size, and the per-turn seed's `renderGraphSeed` still delivers full topology to the agent independently — note: `formatGraphOverview` serves `read_graph` only; the seed uses its own `renderGraphSeed`, not a shared render, despite a stale docstring on `formatGraphOverview`). **`countTurnEntries` is audited** against the current JSONL entry model — both its filter and its messages-vs-turns semantics — before the count is surfaced. The `name` column renders `—` for now. Note (2026-06-16 refactor): `latestSessionName(entries)` in `canonical-session-files.ts` **already extracts an existing session name** when one is present — wiring it into the column is a small behavior-change follow-up (moves the golden off `—`), distinct from **session auto-naming** (auto-*generating* names, the named adjacent capability not built here). When either lands, this render is revised to populate `name` and its golden re-locked.
  - **`<session>`** context render migrates runtime-frame + mentions + world-update + transcript onto the house style; goldens re-locked.
  - **Tool-result renders** (`graph/graph-slice`, `graph/node-neighborhood`, `exchanges/*`) migrate onto the md-pen substrate; goldens re-locked; the legibility rule (neighborhood projects relations to prose, no structural leak) preserved. **House-style representation chosen 2026-06-16 (design-it-twice):** the graph **overview** uses **G-D** (TOON node roster + pseudo edge lines — each sub-shape in its strongest notation per the legibility rule); the **node-neighborhood** uses **G-C** (per-node adjacency, `<-`/`->` grouped — poor for the overview but right for neighborhoods); uniform sets (gaps etc.) stay TOON. This step also absorbs the `<specification>` graph block deferred from the Card 3 design pass. **Scoped 2026-06-16** in `memory/cards/renderer-golden-coverage--graph-render-migration.md`, where scoping found the surface is three renderers of differing live-ness: `formatGraphOverview` (live, `read_graph` only), `formatNeighborhood` (live), `formatGraphSlice` + variants (**dead — zero non-test callers**, slated for retirement), and `renderGraphSeed` (live, separate seed render). `renderGraphSeed`'s migration belongs with the `<session>`/seed re-cluster, not here.
...
- **Current execution pointer:** The `memory/cards/renderer-golden-coverage--house-style-chain.md` chain is complete: substrate, `<workspace>`, and `<specification>` are done. Follow-on repair closed the local A1–A7 renderer findings: readiness rendering now lives in `renderers/session/readiness-estimate.ts`, `<specification>` computes soft readiness over the full gap register while rendering only the ask-eligible agenda, selected-spec binding resolution is shared across context tools, the renderers ledger no longer points at deleted cards, and the `countTurnEntries` audit is closed by reusing the canonical session-file inspection. Graph-render migration is now **scoped** in `memory/cards/renderer-golden-coverage--graph-render-migration.md` (Mode: chain) — Card 0 retires dead `formatGraphSlice` + variants (buildable now), Card 1 migrates `formatGraphOverview` to G-D (design-gated), Card 2 adds the deferred `<specification>` graph block, Card 3 migrates `formatNeighborhood` to G-C (design-gated). The `<session>` render (and `renderGraphSeed`'s seed migration) still need a fresh scope pass.
