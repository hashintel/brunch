...
- **Status:** active — **demo block 1** of the lower line (2026-06-11 demo cut). Rank/select + prompt surfacing landed; **capture-reflection writeback is now demo-blocking** (the demo claims self-updating gaps), no longer deferrable residue.
- **Certainty:** proving
...
- **Current execution pointer:** `memory/cards/elicitation-driver--agent-gap-affordances.md` (chain: gaps-mode kill → `read_elicitation_gaps` → capture-reflection writeback). The writeback micro-decision is resolved at scope level: **dedicated `update_elicitation_gaps` tool** over the existing `CommandExecutor` gap methods, not a `mutateGraph` grammar extension — gaps are not graph plane, and the write side mirrors the read side's register separation.
