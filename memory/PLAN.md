...
- **Workflow + persistence seam (`src/server/core.ts`, `src/server/app.ts`, `src/server/db.ts`, `src/server/schema.ts`, `src/server/parts.ts`, `src/shared/api-types.ts`, `src/shared/project-state-turn.ts`)** — read-model truth now projects from workflow state plus active-path turns, and phase-intent chat submits now prepare interviewer turns directly from derived landing without fabricating kickoff / recovery rows. The remaining runtime cleanup is to quarantine the last legacy control-row helper paths (still centered on `ensureProjectFrontier()` and compatibility-only seeded kickoff support) until projected controls are the only user-facing interaction contract.
