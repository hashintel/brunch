...
- **Linear:** unassigned (create on start)
- **Kind:** structural
- **Status:** not-started (drafted 2026-06-15; **`dogfood-spike` done 2026-06-16**) — Arc 1; the concrete mechanism behind `integration-oracle`'s reachability claim. Spike verdict: booting the real app entry + probing over the wire is **feasible** (a `node:http` entry booted on `listen(0)` and answered `http.get` cleanly), but boot carried real per-stack friction — the agent had to invent a Node `.js→.ts` resolve hook to load the TS entry from a test. **Design signal:** this frontier should own the toolchain-derived boot mechanism so each integration test doesn't reinvent it. Dep-carrying boot still unproven (spike app was zero-dep).
