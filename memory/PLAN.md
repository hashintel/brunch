...
- `renderer-golden-coverage` — **reshaped 2026-06-16 by D83-L** (context-render house style: md-pen + TOON + stringify-tree, `<workspace>`/`<specification>`/`<session>` scope clustering); **actively worked on `ln/fe-870-renderer-golden-context-tools`** (FE-870), below the demo lane and **never a ship gate**. **Landed:** substrate, `<workspace>`, `<specification>`, and the full graph-render migration (overview → G-D, neighborhood → G-C, `<specification>` graph block embedded; dead `formatGraphSlice` retired) — scope cards retired. **Remaining (fresh `ln-scope` pass):** the `<session>` render, `renderGraphSeed`'s seed migration, the `exchanges/*` tool-result migration, the `formatRelatedNodesResult` (`related`-mode) structural-leak repair + relocation into `renderers/`, and the `brunch print` house-style-vs-status fork. Open prioritization question: keep below the demo line or treat the active branch as a parallel track.
...
- **Verification:** Probe-backed transcript and capture read-back oracles (watermark invariant, gradient adherence, gap-spawn outcomes); the re-aimed scenario matrix at probe tier; CI guards structural legality only at the `CommandExecutor` boundary. Route through `ln-oracles` before the conduct slices.
...
- **Current execution pointer:** none — full streaming battery built (claims 1–4, 6, 7, command-intake `session.driveTurn`, claim-5 `request_answer` answer broker); all scope cards consumed and retired. See Recently Completed (2026-06-17) for commits, SPEC §Verification Design for per-test evidence, and `src/rpc/README.md` §Streaming transport coverage for the ledger.
...
- **Topology materialization:** observer relay extends `src/rpc/` via `session-event-relay.ts`, `websocket.ts`, and `web-host.ts`; command-intake adds `methods/session-driver.ts`, `methods/session-exchange-answer.ts`, and `createWebSidecarRpcHandlers(...)`; `src/app/brunch-tui.ts` shares the live `AgentSession` ref and a `LiveExchangeBroker` with the sidecar after Pi creates the session/runtime. `session.driveTurn` is discoverable only on sidecars with a turn-driver handle; driverless sidecars reject it as method-not-found, and attached handles that report no current live session map to `-32010`. `session.answerExchange` is discoverable only with an answer-broker handle and maps no matching pending exchange to `-32008`. The streaming chat consumer lives in `src/web/`. **No new canonical store**, no DB-backed chat/turn projection (D19-L); the live event stream and answer broker are ephemeral/process-local, the same category as `brunch.updated`.
...
- **Decisions / docs to revisit when built:**
  - **[LANDED]** **D84-L / D19-L / D5-L / D33-L / D49-L** — observer relay seam (`SessionEventRelay`, `brunch.sessionEvent` as the ephemeral `brunch.updated` category, no second store), the sidecar re-entry methods `session.driveTurn` + `session.answerExchange` (each discoverable only when its handle is attached), and the live-answer-broker re-coupling of `request_answer` to live events. `session.submitExchangeResponse` (Mechanism-2 transcript-append) remains for out-of-turn answers / not-yet-brokered variants, not deleted. Surfaces documented in `src/rpc/README.md`.
...
nodes:
  ...
  runtime-vocab-leaf             [parallel · proving] src/session/schema/kinds.ts source-of-truth leaf for op_mode/strategy/lens/goal (D73-L direction)
