...
- **Linear:** [FE-898](https://linear.app/hash/issue/FE-898/realign-prompt-resource-skills-to-the-agent-skills-standard) (reverses the D85-L deferral; **not** an FE-861 slice)
- **Branch:** `ln/fe-898-prompt-skill-topology` — create with `gt create` on the FE-861 tip **after** the matrix lands and the tree is clean (so it does not carry in-flight FE-861 work); stacks on `ln/fe-861-generalized-capture-2`
...
nodes:
  ...
  prompt-skill-topology          [parallel · proving · FE-898] adopt Agent Skills SKILL.md topology+frontmatter via pi loader over a code-owned path list; reverses the D85-L deferral; card scoped, branch on a clean FE-861 tip
