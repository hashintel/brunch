...
- `petri-execution-parity` ([FE-1195](https://linear.app/hash/issue/FE-1195/petri-execution-parity-isolated-slice-fan-in-and-epic-integration)) — final review closure is complete on PR #325: journal-ahead authority uses one topology-validated multiset classifier across drive, standalone, and observer paths; an independently persisted preparation marker prevents correlated journal/definition loss from recreating the journal. `npm run verify` is green.
...
- **Status:** ✓ review-clean on branch; all scope leaves are closed and `npm run verify` passes (2,106 tests passed, 2 existing skipped).
