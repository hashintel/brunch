...
1. `pi-ui-extension-patterns` — Finish FE-744's remaining Pi-wrapping proof now that raw Pi RPC editor fallback and public Brunch JSON-RPC ten-turn tuple parity are covered: prove web real-time structured-exchange observation, then recover branded/themed chrome.
...
- **Acceptance:** Briefs #1–#7 present in `.fixtures/briefs/`; adversarial briefs present with documented targets; expectations for brief #7 satisfied per fixture-strategy.
