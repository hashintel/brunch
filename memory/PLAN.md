...
- **Kind:** structural
- **Status:** active — **slices 1–4 done** (slice 1: executability contract + repair + `Toolchain` descriptor; slice 2: toolchain-agnostic cook prompts + runner; slice 3: build-architect prompt enrichment — `projectPlanningContext` + criteria + fixture exemplars; slice 4A: file-ownership contract — `Slice.writes` + `file-write-conflict` design-class warning + D160-K amendment (I132-K); slice 4B: **the architect now AUTHORS the slice set** — `architectPlan` (new schema: authored slices with `writes` + `derivedFrom`) → `materializeArchitectedPlan` (deterministic normalize: unknown-ref filtering, dep-clean, cycle-break, epic resolution, criteria-into-prose, coverage sidecar) → `repairPlan` + `checkPlan` (emitted + generalized requirement-provenance coverage), with a deterministic projection fallback on authoring throw / parse-fail / uncovered requirement (I133-K); all 2026-06-09). **Slice 4 was split** (oracle 2026-06-09): 4A = deterministic contract half, 4B = LLM authoring. The slice-3 `planExecutionOrdering` enrichment stage is **superseded on the mainline** (I131-K superseded by I133-K) and dormant pending retirement. **Next: slice 5 (eval harness vs fixtures) + retire `planExecutionOrdering`.** SPEC records A100-K (partially-validated), D160-K (amended), D167-K, I129-K, I130-K, I131-K (superseded), I132-K, I133-K, Future Direction §Cook plan generation.
