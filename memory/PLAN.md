...
- **Linear:** FE-847 — built as a slice group on the FE-847 branch (2026-06-10 single-branch decision); no separate issue.
...
- **Linear:** FE-847 — built as a slice group on the FE-847 branch (2026-06-10 single-branch decision); no separate issue/branch.
...
nodes:
  ...
  kick-and-context-seeding       [next · proving]   separate product frontier/branch: honest kick via triggerExchange + boot/resume context seeding (S4); pre-reconcile-tail policy; boot idempotence (S5 share)
...
edges:
  ...
  dx-tier-2-harness         -[hard]->         kick-and-context-seeding        (S4 origination is proven through the Tier-2 chassis; same FE-847 branch, last slice group)
