...
- **Workflow + persistence seam (`src/server/core.ts`, `src/server/app.ts`, `src/server/db.ts`, `src/server/schema.ts`, `src/shared/api-types.ts`, `src/shared/project-state-turn.ts`)** — this stack still fabricates kickoff / recovery as durable `turn_kind` rows, auto-seeds frontier turns through `ensureProjectFrontier()`, and exposes frontier/control meaning through turn records. Active items 4, 5, and 6 must replace that with explicit stream projection, anchored phase-outcome authority, truthful hydration landings, and projected control semantics.
- **Workspace stream controller + routed interview view (`src/client/routes/project/$id/_view/-interview-controller-core.ts`, `src/client/routes/project/$id/_view/-interview-controller.ts`, `src/client/routes/project/$id/_view/-interview-data.ts`, `src/client/routes/project/$id/_view/-interview-view.tsx`)** — this pipeline still chooses bottom-of-phase UI from `turn_kind`, missing-frontier heuristics, and generic closed-shell fallbacks. Active items 1, 4, 5, and 6 must move it to an explicit merged stream projector that can render durable turn cards, projected kickoff / recovery / handoff controls, phase markers, and activity cards without pretending they are all turns.
- **Card primitives and closed-state affordances (`src/client/components/question-cards.tsx`, `src/client/components/review-set-card.tsx`)** — the review-specific work is underway, but the card family still sits inside a broader turn-centric shell. Active items 1, 5, and 6 must finish the separation between substantive turn cards and structural control / completion artifacts while keeping the accepted-set review seam authoritative.
- **Ontology + sidebar/read-model seam (`src/shared/knowledge.ts`, `src/client/components/EntitySidebar.tsx`, `src/server/db.ts`, `src/server/observer.ts`, `src/shared/api-types.ts`)** — legacy knowledge facades, optional `reviewStatus` shapes, and framing-era grouping assumptions still leak through the shared types and sidebar projections. Active items 2 and 3 must narrow this to the canonical durable ontology and accepted-review projections so the UI, observer, persistence, and fixtures all speak the same kind language.
- **Fixtures, manifests, seeded scenarios, and oracle tests (`src/server/fixtures/manifest.ts`, `src/server/fixtures/scenarios.ts`, `src/server/fixtures/manifests/*.json`, `src/server/fixtures/corpus.ts`, `src/server/fixtures/walkthrough.test.ts`, `src/server/core.test.ts`, `src/server/app.test.ts`, `src/client/routes/project/$id/_view/*test.tsx`)** — these files currently encode kickoff / recovery as durable frontier rows and assert directly on `turn_kind` or phase shells that the new architecture demotes to projection detail. Active items 4, 5, and 6 must rewrite the seeded states and tests around projected control cards, anchored phase outcomes, and resumed landing states instead.
- **Naming, routing, and grounding-language seam (`src/shared/phase-routes.ts`, `src/shared/phase-display.ts`, `src/client/routes/project/$id/_view/framing.tsx`, `src/client/routes/project/$id/index.tsx`, `src/server/interview.ts`, `src/server/tools/index.ts`, `src/shared/grounding-strategy.ts`)** — the codebase still mixes `scope`, `framing`, kickoff-specific brownfield ritual language, and route names that predate the grounding/card-owned model. Active items 2, 5, and especially 7 must simplify these seams so reusable grounding/context-gathering and later naming normalization do not keep inheriting obsolete product language.
