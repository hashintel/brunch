...
- **Status:** active follow-up. The alpha-orchestrator tracer landed on branch/PR #311, including subnet-preserving compilation, raw event journaling, replay/read projections, durable marking snapshots, preserved epic identity in compiled topology + runtime journal facts, graph-derived slice provenance preserved through compiled topology/export and slice transition facts, epic-aware frontier hints/read surfaces with active in-flight slice identity preserved on non-start steps, Petri-honest greedy frontier selection over co-fireable enabled transitions, and a persisted claim-set seam on `petrinaut/marking.json` that records the currently reserved firing set without treating it as lifecycle authority and now surfaces it live through execute/web update paths; durable parallel side-effect authority remains the next substantive Petri question.
- **Certainty:** proving.
