...
- **Status:** not-started (drafted 2026-06-23) — post-Arc-1; spec landed (D171-K, I139-K). Stacks on the FE-883/FE-884 cook-substrate cluster. **Current execution pointer:** slice 1 (substrate seam) scoped, ready for `ln-build`; the `.git`-leak risk is retired (already excluded in all merge/promotion/CoW paths).
