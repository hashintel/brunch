...
- **Members:** `shared-session-host-tracer` (inner contracts, production wiring, and every automated witness built) → FE-1348 `post-hardening-alpha-validation` (A51-L colleague-walkthrough row only; the frontier's other rows are product-wide) → `shared-session-host-cutover` (wait-gated).
...
- **Branch:** `ln/fe-1348-validate-current-brunch-usage-and-testing-paths`; stacked after FE-1321 so the sweep validates the complete current runtime-contract tracer.
...
- **Certainty:** proving.
- **Stabilizes:** the current product-entry, runtime-authority, durable-state, developer-verification, comparison-gate, and installed-package evidence boundary; retires A51-L if its required colleague walkthrough passes.
...
- **Classification:** evidence-gated — the inventory and existing oracles are available, but required outer rows need fresh human/PTY/browser evidence on the current stack.
...
- **Why now / unlocks:** FE-1311–FE-1321 pruned, reconciled, isolated, and hardened the live stack, but the broad alpha walkthrough script predates those changes. A frozen current-boundary pass supplies the product-level confidence and A51-L judgment needed before shared-session-host cutover or another architecture-sensitive frontier.
- **Verification:** each row binds a real entry point to a canonical artifact/query/contract oracle; qualitative UI/conduct claims use owned outer walkthroughs. Incidental simplicity/quality observations are evidence, never completion criteria unless promoted.
- **Traceability:** product requirements 1–5, 7–12, 16–19, 24–32; A5-L, A42-L, A48-L, A51-L; D39-L, D123-L, D132-L, D133-L, D141-L; I24-L, I32-L, I54-L, I59-L, I64-L–I69-L; SPEC §Verification Design.
...
- **Classification:** wait-gated on `shared-session-host-tracer` and FE-1348's A51-L colleague-walkthrough row.
...
active:
  shared-session-host-tracer (FE-1321)
    ...
    next: FE-1348 runs A51-L's colleague walkthrough as a required validation-sweep row — the
          last open evidence on this frontier, carrying TESTING_FINDINGS.md SA1/SA2; every
          automated limb is witnessed
    ...
    -[hard]-> post-hardening-alpha-validation (FE-1348)
  post-hardening-alpha-validation (FE-1348)
    ...
    includes: A51-L colleague walkthrough | current alpha product entrances | current verification and installed-package paths
