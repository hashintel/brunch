...
- **Status:** active. Admitted 2026-07-07 (spike → grill → spec, same day); the model-allowlist tracer, packaging fixes, release verification smoke, and `brunch login` are built, while no-auth gate remains open.
...
- **Current execution pointer:** `memory/cards/alpha-release-readiness--no-auth-gate.md` (proving; owns `src/app/model-policy.ts` and warning/juncture-gate paths). Packaging + allowlist + release-check threads landed 2026-07-07 (`2a60e16a`, `53a69a51`, `bcbb3d35`); `brunch login` landed 2026-07-07 in this slice (`src/app/brunch-login.ts`, `src/app/brunch.ts`).
