...
- **Aggregate DoD:** met: both legacy adapters remain deleted; all 51 in-boundary schemas route through the shared adapter; object-root/top-level-union constraints, nested unions, and exact registry provenance are covered; persisted pre-sweep semantic baselines protect every family whose authoring representation changed.
- **Inventory authority:** the exact 51-member inventory is pinned by `src/.pi/extensions/__tests__/registry.test.ts`; the exhausted temporary ledger was deleted.
...
frontiers:
  Active:
    ...
    tool-schema-convergence (FE-1163)
      status: done 2026-07-10; exact 51-tool oracle + semantic preservation closure green
