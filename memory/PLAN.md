...
- `standalone-web-session-host` (FE-1200) — **presentation-coverage sweep scoped** (retires I65-L breadth). Ledger: [`memory/cards/standalone-web-session-host--presentation-coverage.md`](cards/standalone-web-session-host--presentation-coverage.md). A2–A4 are built; next row is A5.
...
- **Current execution pointer:** presentation-coverage `Mode: sweep` ledger scoped at [`memory/cards/standalone-web-session-host--presentation-coverage.md`](cards/standalone-web-session-host--presentation-coverage.md) (buildable-now; A2–A4 built, 5 required rows remain across ask-terminal + `present_*` sub-seams). Next: A5.
