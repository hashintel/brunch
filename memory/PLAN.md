...
- **Status:** active follow-up. The alpha-orchestrator tracer landed on branch/PR #311, including subnet-preserving compilation, raw event journaling, replay/read projections, durable marking snapshots, preserved epic identity in compiled topology + runtime journal facts, graph-derived slice provenance preserved through compiled topology/export and slice transition facts, epic-aware frontier hints/read surfaces with active in-flight slice identity preserved on non-start steps, Petri-honest greedy frontier selection over co-fireable enabled transitions, and a persisted claim-set seam on `petrinaut/marking.json` that records the currently reserved firing set without treating it as lifecycle authority, now surfaces it live through execute/web update paths with explicit replay-reason clears when snapshot-backed updates replace replay hints plus boundary validation that rejects malformed or terminal-incoherent projection payloads instead of desynchronizing cached Petri metadata, now lets a restarted drive resume the same claimed firing order when the snapshot provenance still matches `run.json`, now routes snapshot reads and live `execute.run` projection patches through a shared Petri projection parser/normalizer so cache patches and snapshot reads enforce the same payload contract and normalization, and now rejects provenance-matching marking snapshots whose persisted `currentMarking`, `firedTransitionCount`, or terminal summary contradict checkable live truth before surfacing them on read paths while replay now fails closed on contradictory, under-specified, or post-terminal-resumed raw journal tails instead of trusting a stale terminal summary; durable parallel side-effect authority remains the next substantive Petri question.
- **Certainty:** proving.
