...
- **Acceptance:**
  ...
  - **Graph-model strand:** Phase 1 edge contract locked in `docs/design/GRAPH_MODEL.md` and materialized as type/policy stubs under `src/graph/` (✓ landed at commit `100585a1`). Phase 2 node contract locked similarly (per-plane node kinds, decision-as-node shape, framing/theses carrier resolution, node-side `provenance` and `basis` symmetry with edges) and materialized as `src/graph/schema/nodes.ts` (+ peers); the prior named-relation catalogue and brainstormed edge taxonomy are retired from active docs. A20-L spike produces a verdict on the persistence seam over one representative intent-plane slice: Drizzle line choice, row-schema derivation path (`drizzle-zod`, `drizzle-orm/typebox`, or equivalent), monotonic counter allocation, change-log writes, and Pi `registerTool` round-trip.
...
- **Current execution pointer:**
  ...
  - **Graph-model strand:** Phase 1 edge lock landed at commit `100585a1` (canonical doc `docs/design/GRAPH_MODEL.md`; stubs under `src/graph/atoms.ts`, `index.ts`, `schema/edges.ts`, `schema/reconciliation-need.ts`, `policy/category-policy.ts`; SPEC/PLAN reconciled via D51-L at commit `23a97f34`). Remaining: **(i)** Phase 2 node lock — per-plane node kinds, decision-as-node shape, `framing_as` vs. product-theses carrier resolution, node-side `provenance` and `basis` symmetry with edges — materialized as `src/graph/schema/nodes.ts` and reflected in `GRAPH_MODEL.md` §"Nodes" plus SPEC if a new D-L is warranted; **(ii)** A20-L persistence spike over one representative intent-plane slice (throwaway scope — verdict only): settle the Drizzle line and adapter, prove the monotonic counter / change-log scheme, and prove Pi `registerTool` binding against the chosen row schema.
