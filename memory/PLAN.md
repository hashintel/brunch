...
- `alpha-hardening` — collection frontier (def below) for three small residues from completed work: the dev-seed-fixtures all-seeds opt-in flag + seed disposition catalog (FE-848), the `src/session/schema/kinds.ts` runtime-vocab leaf (D73-L; `goal` axis dropped by D85-L), and the FE-893 deferred prompt-shape (`SKILL.md`/`references` + `.pi/agents/<agent>/SYSTEM.md` convention + `_generated/` vocab refs). One low-conflict branch, one commit each.
...
- `core-tools-adoption` (noted 2026-06-12): **web fetch/search landed 2026-06-19 inside FE-861** — Brunch now statically registers `web_fetch`/`web_search` in the sealed profile and base-allows them in `elicit-read-only`. The subagent portion is split into its own frontier (`subagent-adoption`).
- `subagent-adoption` (split from `core-tools-adoption` 2026-06-19) — decide how Brunch adopts a basic `subagent` tool inside the sealed profile. All surveyed reference implementations (canonical pi example, `amosblomqvist/pi-config`, `nicobailon/pi-subagent-enhanced`, `nicobailon/pi-subagents`, `ogulcancelik/pi-spar`) spawn a child `pi` process and rely on `~/.pi/agent/` discovery for agents/tools, which conflicts with D39-L. SDK-only alternatives (`shitty-extensions/oracle.ts`, `loop.ts`) use `complete()` from `@earendil-works/pi-ai` for single-call second opinions / loops, not tool-bearing subagents. This frontier resolves the execution model (child `pi` vs in-process SDK vs Brunch-CLI subprocess) and the agent-definition boundary before capture design can assume delegated acquisition.
...
- **Current execution pointer (2026-06-19):** first FE-861 slice landed (`b5c101bc`) — `src/.pi/skills/methods/capture.md` carries the D81-L commitment-gradient conduct body, and `src/graph/__tests__/capture-commitment-gradient-gate.test.ts` locks the routing gate over the real `mutate_graph` + `update_elicitation_gaps` adapters (high→explicit/implicit commit, low→one existing-or-new gap, structural answered derivation, manual gap close on the graph clock, illegal batch fail-loud). Second slice landed (`5e637ad4`) — `src/projections/session/sweep-watermark.ts` owns the D80-L transcript-position sweep watermark and un-swept-tail projection; `src/app/pi-extensions.ts` advances it at the live `before_agent_start` boundary; tests lock conversational/digest classification, raw-background exclusion, idempotent advance, and graph-LSN watermark separation. **Third slice landed** — submit-time labeled-prefix capture fossil retired: `graph/capture/structured-response.ts` + its `session.*` RPC wiring + the `capture-response-to-graph` / `submit-message-capture` proofs deleted, and the `capture` field removed from both submit result schemas (RPC-contract change; no web consumer, no observer-update regression — submit-time publish was redundant with the graph-tool path). SPEC A22-L/I30-L + the rpc/web/graph topology READMEs reconciled. **Core web-tool prerequisite landed** — `src/.pi/extensions/web/` statically registers `web_fetch`/`web_search`, `elicit-read-only` base-allows them, and `read-context` names them as context-gathering read tools. **D82-L acquisition/digest + situating gap landed** — `ea541220` registers four acquisition-mode resources (`elicit-by-question`, `ingest-paste`, `read-referenced-documents`, `explore-and-characterize`) as ungated `methods/` skills in the sealed manifest, with the two bulk modes directing an assistant-authored digest the landed sweep captures over (digest substrate was already watermark-classified); `6d4c8a75` seeds a grounding-band `manual` situating gap at `createSpec` carrying the orientation anchors (new-from-scratch / brownfield / continuation) that route the opening into the matching acquisition mode. Scope file `memory/cards/generalized-capture--acquisition-digest-situating.md` retired on exhaustion. **Remaining (continues on a fresh stacked branch atop `ln/fe-861-generalized-capture`, same FE-861 issue):** false-commit scenario-matrix completeness, and the contradiction→reconciliation_need outlet.
...
- **Objective:** Collect three genuinely-small residues left by completed frontiers into one low-conflict branch, each landing as its own commit:
  1. **dev-seed-fixtures residue (FE-848):** the explicit all-seeds opt-in flag/command (no ambient all-seeds default) + the captured-seed disposition catalog (`brunch-self`, `dumpchat`, `fable`, `rd-loop`, `yamlbase`, + Bilal/coverage sets → `test` / `preview` / `manual workbench` / `probe input` / `parked`).
  2. **runtime-vocab-leaf:** `src/session/schema/kinds.ts` as the drizzle-free, pi-free source-of-truth leaf for the runtime axes (`op_mode`, `strategy`, `lens`, `auto` sentinel — `goal` dropped by D85-L), mirroring `graph/schema/kinds.ts` (D73-L). Consumers (`runtime/state.ts` `MethodId`, `runtime-policy.ts`, `affordances.ts`, the display-only `PLANNED_OPERATIONAL_MODE_IDS`/`OperationalModeChoice`) import from it; the leaf imports nothing; must not recreate `READINESS_GRADES`.
  3. **FE-893 deferred prompt-shape:** the `SKILL.md` + `references/` presented shape + the `.pi/agents/<agent>/SYSTEM.md` + `[sub]` agent-definition convention (shape only — D39-L sealing + code-owned manifest preserved, no ambient discovery); and the `_generated/` typed-vocab reference destination + lock disposition (regenerated + drift-checked, distinct from the authored-body-lock) + trigger (build-step vs compose-time). Do not over-build the generated layer before a concrete stale-member need (`sourcing: strip-or-build`).
- **Why now / unlocks:** none is demo/alpha-critical and none merits its own branch; collecting them keeps tracker/branch overhead low while the demo lane (blocks 3–4) stays the focus. Each residue is independently scoped at build time; full historical source-frontier defs live in `docs/archive/PLAN_HISTORY.md` (2026-06-19 archive).
- **Acceptance:** each residue lands per its source frontier's original acceptance: seed all-seeds opt-in + disposition catalog; the runtime kinds leaf with its import-boundary test; the resolved-or-explicitly-deferred prompt-shape / generated-vocab-refs questions.
- **Verification:** Inner — seed CLI parse/mode tests; runtime-leaf import-boundary/architecture test; manifest-legality + compose goldens for any prompt-shape change (D39-L sealing preserved).
- **Cross-cutting obligations:** keep each residue inside its origin seam; do not let the collection branch grow into unrelated cleanup; the runtime leaf stays a pure constants module; any prompt-shape change preserves D39-L sealing and the code-owned manifest.
- **Traceability:** D39-L, D58-L, D59-L, D73-L, D79-L, D85-L / I1-L, I25-L, I38-L, I48-L.
- **Design docs:** `docs/archive/PLAN_HISTORY.md` (2026-06-19 archive); `src/session/README.md`; `src/.pi/skills/README.md`; `.fixtures/README.md`.
...
- **Status:** parallel / low-conflict (deferred below the demo line; not alpha-blocking)
- **Certainty:** proving
...
- **Objective:** Decide and implement how Brunch delegates isolated reasoning tasks to child agents without breaking the sealed profile or reintroducing ambient discovery.
...
- **Design context (reference survey):**
  ...
  - **Alternative execution models still to compare:** (a) SDK in-process with sealed ResourceLoader / explicit tool manifest; (b) Brunch-CLI subprocess with a dedicated `--mode subagent` or equivalent flag (higher isolation, but requires designing a headless mode and binary-spawn policy); (c) `pi --mode rpc` subprocess (ruled out by D39-L because it depends on a globally-installed `pi` and ambient discovery).
- **Acceptance:**
  - A documented decision on the child-agent execution model, with the SDK-based sealed-session path as the leading candidate and the Brunch-CLI subprocess path as the deliberate fallback.
  - If implemented: a single `subagent` tool registered in the sealed profile, backed by an SDK `createAgentSession()` call that uses an explicitly constructed ResourceLoader / tool manifest / settings manager (no ambient discovery), with agent definitions declared statically in the Brunch repo.
  - Runtime legality gate keeps `subagent` out of the ordinary `elicit` base-allowed set; it is available only when posture/capability explicitly permits delegated acquisition.
  - Static registration only; no import or discovery from `~/.pi/agent/extensions/` or `.pi/`.
- **Verification:**
  - Inner: architecture decision record or spec update; static audit confirming no ambient imports.
  - Middle/Outer: proof that the chosen execution model starts from the Brunch entrypoint and returns without relying on a globally-installed `pi` binary; manual end-to-end if a real LLM call is involved.
- **Topology:** new registrar in `src/.pi/extensions/subagent/`; child-session factory and sealed ResourceLoader helper in `src/.pi/extensions/subagent/session.ts`; agent definitions under `src/.pi/agents/subagent/`; policy in `runtime/state.ts`; wiring in `src/app/pi-extensions.ts`; decision recorded in `src/.pi/extensions/README.md` and `memory/SPEC.md` if durable.
...
nodes:
  ...
  alpha-hardening                [parallel · proving] collection: dev-seed residue (all-seeds flag + disposition catalog) + runtime-vocab leaf (D73-L) + FE-893 deferred prompt-shape/generated-vocab refs
  subagent-adoption              [parallel · proving · split from core-tools-adoption] how to adopt a basic subagent tool in Brunch's sealed profile
