...
- 2026-05-22 `web-shell` scoped review queue — Done: explicit session projection was split into a read-side resolver that validates durable bindings without opening selected workspace state; stdio/WebSocket share JSON-RPC parse/internal-error dispatch; browser RPC protocol failures close the attachment deterministically; static asset resolution rejects traversal; fixture capture uses the shared response type; and the React shell renders read-only transcript projection state through explicit `{ sessionId, specId }`. Verified: `npm run verify` after each slice. Watch: manual browser smoke / projection postconditions are still the remaining outer-loop evidence before tying off M3.
