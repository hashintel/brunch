...
- **Unified execution authority (2026-07-13):** D124-L now serializes same-process callers by canonical run identity across the full drive while preserving cross-run overlap. `RUN_MUTATION_ENTRY_INVENTORY` covers scheduler effects plus run create/supersede/abandon, replan retry/regeneration, and host apply entries. Epic verification has durable claim/replay/catch-up ordering; ordinary serial and parallel external-effect throws settle durably while journal/marking carriers remain fail-closed. Standalone start and executor/RPC/web observation recover from even one journal-only parallel claim, and malformed/unavailable nonterminal authority journals suppress all readiness as `authority_unreadable`. D123-L remains limited to concurrent isolated slice truth.
