...
- **Grounding:** concentricity is now materialized as code contract — `EXECUTOR_ALLOWED_TOOL_NAMES` (`src/agents/runtime/executor/active-tools.ts`) is composed from the live elicitor allowlist plus executor-only `orchestrator_stub`, with the blocked-tool floor still applied. The CODE-side J5 switch now records a `brunch.session_orientation` resolution and kicks the executor with a matching directive. The remaining conduct work is prompt/reference guidance: capability-readiness postures (Proceed / Proceed-advisory / Negotiate / Ask) in `src/agents/references/readiness-bands.md` §Elicitor Use extend to the executor; this frontier does not invent a new readiness model.
