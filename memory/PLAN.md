...
- **Current execution pointer (2026-06-19):** the D80-L banded sweep + watermark, the D81-L commitment-gradient routing gate, the submit-time labeled-prefix fossil retirement, the `web_fetch`/`web_search` prerequisite, and the D82-L acquisition-modes + digest + situating-gap layer have all landed on the `ln/fe-861-generalized-capture` stack (slice history in git; SPEC I30-L/A22-L + the rpc/web/graph topology READMEs carry the durable record). The two deterministic capture oracles (routing gate + sweep-watermark property) are green. **Remaining (continues on `ln/fe-861-generalized-capture-2`, same FE-861 issue):** false-commit scenario-matrix completeness, and the contradiction→`reconciliation_need` outlet.
