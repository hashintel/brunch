...
3. `harness-dep-install` — **(acceptance 1 landed — FE-872, classify + react)** dependency-delta capture + install-failure classification (the install *action* is agent-native via `bash` + FE-843 conventions; this owns lockfile capture for promotion + the fail/infra split). Slices 1–2 done (classify + infra-aware halt reason); dep-delta capture on promotion (acceptance 2) remains.
