...
- **Current execution pointer:** `memory/cards/context-seed-payload--startup-completeness.md` (chain: continuity carrier migration → seed payload + startup proof). Scoping finding (2026-06-11): pi's `appendCustomEntry` is ledger-only — custom entries never enter LLM context, so `worldUpdate`, drains, and the seed were never provider-visible. **Widened by user decision:** every continuity entry whose intention is model-visible migrates to the `CustomMessageEntry` carrier (`appendCustomMessageEntry` at the reconciler seam; `pi.sendMessage` reserved for future out-of-band injection) in one pass before the seed payload lands. The ledger-only set (`own_mutation`, `mention`, runtime state, binding, lifecycle) is a closed keep-list in the scope file.
- **Acceptance:**
