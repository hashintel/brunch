...
- **Current execution pointer:** Preflight hardening done 2026-06-09. The completed chain made the D67-L vite/vitest source alias portable/exact, moved ordering-proof faux-provider wiring out of build-excluded `src/dev/**`, and routed introspection artifacts to repo-root `.fixtures/scratch/introspection/<run-id>/` with fixture docs/gitignore reconciled. Next scope should start the live TUI `BRUNCH_DEV` wiring slice, then conversational self-report.
