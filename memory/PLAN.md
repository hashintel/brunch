...
- **Status:** active. Admitted 2026-07-07 (spike → grill → spec, same day); the model-allowlist tracer, packaging fixes, and release verification smoke are built, while no-auth gate and `brunch login` remain open.
...
- **Current execution pointer:** none for the consumed packaging/release scope. No-auth gate (D115-L/I59-L) and `brunch login` (D114-L) are scoped after the allowlist tracer — both consume `src/app/model-policy.ts` as their single allowlist source.
