...
- **Current execution pointer:** linear transcript policy hardening is complete; next slice should start the browser shell without weakening the thin `session.*` / `workspace.*` handler boundary.
...
- 2026-05-21 `web-shell` — Done: M3 linear transcript policy hardening landed before browser consumption: JSONL transcript loading rejects non-linear child graphs, `parentSession`, and `branch_summary`; `session.elicitationExchanges` returns a product-shaped JSON-RPC error for non-linear selected sessions; and the Brunch TUI extension cancels Pi `session_before_tree` / `session_before_fork` branch flows with stable explanatory copy while preserving `/new` coordinator binding behavior. Verified: `npm run verify` after each slice. Watch: next web-shell slices should consume the same thin `session.*` / `workspace.*` handlers rather than inventing browser-only session semantics.
