...
- Older completed frontiers (2026-06-17/06-18/06-19 window, the 2026-06-12 demo window, and earlier): `docs/archive/PLAN_HISTORY.md`.
...
- `structured-exchange-affordance` (`ln-induct` candidate + designed, 2026-06-22; surfaced across the FE-811 ship-gate runbook): the recurring **"enforced-but-untaught"** signal — the agent fights the structured-exchange grammar with low-information errors. Five+ live samples: `request_choice` after `present_question` (request-side mis-pairing), `present_question` used for a multiple-choice question (present-side selection error, the inverse), and review-set payload shape errors at successive nesting layers. Cheap stopgaps shipped (`present_review_set.payload` typing `64fe9a41`; pairing `.describe()` + guideline `95933e5f`). Durable design is the **request-side collapse to a single server-routed `respond`** — full rationale, three-design comparison, load-bearing claims, and tracer bullet in [`docs/design/STRUCTURED_EXCHANGE_COLLAPSE.md`](../docs/design/STRUCTURED_EXCHANGE_COLLAPSE.md). Three separately-scoped pieces: (1) the `respond` collapse (request-side; tracer = `respond`-for-`present_question`), (2) phase-2 present-side merge (a single `present`/`ask` whose `options[]` presence determines choice-vs-freeform, killing the selection error structurally), (3) a standalone review-set nested-payload describe-pass. Post-runbook structural work; an `ln-induct` pass would graduate the lens and systematize the fix across all three. Not demo/alpha-blocking.
