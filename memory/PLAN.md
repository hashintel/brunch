...
- `exchange-ask-refinement` ([FE-1164](https://linear.app/hash/issue/FE-1164/one-shot-ask-terminal-and-exchange-presentation-refinement)) — one-shot ask terminal (D116-L) + exchange presentation refinement, on `ln/fe-1164-ask-terminal` (stacked on the tied-off FE-1115 lane). Opened 2026-07-07 from grill + spec; definition below. Current execution pointer: `memory/cards/exchange-ask-refinement--ask-terminal.md`. Sliced execution: rich-body picker surface first (de-risks both halves), standalone ask cutover second, offer declared-continuations third.
