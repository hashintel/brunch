...
- **Live scope files:** none. The structured-ask witness landed 2026-08-07 and its scope file was consumed. The rival-refusal and shutdown/reopen slices stay unscoped and must run sequentially: both need the same shared PTY choreography module (`src/app/__tests__/session-runtime-contract-pty-journey.ts`) as a primary write target, so they are not write-disjoint from each other.
...
active:
  shared-session-host-tracer (FE-1321)
    ...
    next: scope and build rival refusal, then shutdown/reopen — both sequential on the shared
          PTY choreography module (src/app/__tests__/session-runtime-contract-pty-journey.ts)
