...
- **Status:** slice 1 done (FE-871) — `detectProfile(repoDir)` / `project-detect.ts`: a pure, evidence-first detector mapping manifests/lockfiles to a registry `ProfileId` (bun lockfile → bun; deno config → deno; `package.json` vitest/jest/none → node-vitest/node-jest/node-test), returning a loud `{detected:false, reason}` for unsupported stacks (Python/Go) and unrecognized dirs rather than silently defaulting to bun — already the cheap "which lockfile is present" check, not a language-detection engine. **Slice 2 remains:** wire `detected` into the `plan-emitter` selection chain (brownfield front) + the greenfield no-op + the greenfield-protecting before/after-identical check — `detected` is **not yet** wired into `plan-emitter.ts`. Stacked on `agent-extension-host`.
