...
- [2026-05-07] FE-698 prompt/context scenario substrate — Packaged markdown prompt registry + observer context-pack foundation. Server interviewer, observer, and side-chat role prompts now load from markdown assets through a typed prompt registry, and observer capture now renders its existing prompt context through the first typed scenario-specific context pack. Verified: `npm run verify`. Watch: next FE-698 slices still need broader context-pack scenarios and scenario-runner probes.
