...
- **Ownership model (deterministic boundaries around a non-deterministic core):** elicitation decides commitments (stack, framework, package manager, deployment, quality obligations); repository detection reports facts and never overrides approved commitments; the planner synthesizes and lowers; deterministic validation judges; bounded repair corrects or rejects with exact findings; the executor enforces only the admitted frozen plan; conflicts return to the user — approved truth wins over inference, contradictions block rather than silently substitute. Capability vocabulary is projected from elicited commitments and resolved through product-owned pluggable providers; no closed built-in catalogue of languages/frameworks/runners as planning truth; unrecognized elicited capabilities stay explicit blocked requirements.
...
- **Explicitly out:** named durable review-branch promotion; external Petrinaut HTTP interop beyond the existing observer/live-stream contract; split-process execution authority; richer browser authoring or side chats; post-execution semantic spec-to-code drift detection; generic human halt/replan/resume UX; actual YAML support or `plan.yaml` renaming; reimplementing PR #325 isolation/concurrency/fan-in/attempt-authority/epic-lifecycle semantics (the planner targets them, never re-owns them).
- **Slice A landed (2026-07-13, D130-L):** the plan artifact carries the typed execution contract (planning projection incl. constraint/invariant/decision commitments; provider-resolved actions; blocked/conflict findings), `execute_run_create` admits or rejects it before any artifact, brownfield projections feed manifest-detected capabilities, and the test-runner port lost its ambient default. Design-permutations record (contract home, capability vocabulary, action safety) preserved in the slice-A commits `31c264e6`/`300cce65`. Substrate remains a run-create param — plan-owned substrate lowering rides slice B/C.
...
- **Slice C fixture witness landed (2026-07-13):** `plan-synthesis-composition.test.ts` drives an admitted synthesized plan through the frozen topology under `petriScheduler` + `frontierFiringPolicy` — overlapping dependency-independent isolated slices (barrier witness), ordered fan-in, epic integration/verification, promotion preparation, contract-only verify commands on every slice/epic invocation, and the failed-verification gate (oracle 8).
- **Spec-commanded recipe (2026-07-14):** settled commitment nodes may declare `execute.setup|build|verify: <command>` lines; a generic extractor turns them into a recipe provider plus spec-mandated base requirements the planner cannot drop (argv-only, shell operators fail closed as `malformed_recipe`). This is the generic execution path for stacks without a built-in provider; the planner subagent registry omission that blocked the first live witness is fixed.
- **Open acceptance:** the live Specify → committed scope → synthesized/admitted plan → PR #325 execution → promotion witness (oracle 9) plus live-capability oracles 1–5 on real elicited specs; requires a model-backed session per `docs/praxis/manual-testing.md`. Re-entry trigger: first live FE-1197 walkthrough. All scope files consumed.
