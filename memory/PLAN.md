...
- **Kind:** structural
- **Status:** active — **slices 1–3 done** (slice 1: executability contract + repair + `Toolchain` descriptor; slice 2: toolchain-agnostic cook prompts + runner — `ToolchainTestRunner`, `sliceTestTask`/`epicVerifyTask`, `test-writer.md` de-hardcoded, resolved from `plan.profile`; slice 3: build-architect prompt — `projectPlanningContext` lifts spec relation edges into `req-*` slice space, each slice's acceptance criteria + the three reference fixtures (comment-stripped, `plan-exemplars.ts`) are inlined into the planning prompt, output schema unchanged; all 2026-06-09); **slice 4 (file-ownership/join + D160-K amendment + `Slice.writes`) next**. SPEC records A100-K (partially-validated), D167-K (+ base/emitted profile-split refinement), I129-K, I130-K, I131-K, Future Direction §Cook plan generation.
