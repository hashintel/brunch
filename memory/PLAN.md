...
- 2026-05-20 `walking-skeleton` / TUI boot-chrome slice — Done: replaced the bootstrap stub with Brunch-owned TUI boot, coordinator-first spec gating, an internal pi extension widget for persistent cwd / spec / phase / chat-mode chrome, and a bin shim. Verified: `npm run verify`. Watch: manual TUI smoke is still required to validate actual terminal presentation before frontier completion.
