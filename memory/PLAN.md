...
- **Verification:** each row binds a real entry point to a canonical artifact/query/contract oracle. Under D142-L, agent conduct uses public Brunch JSON-RPC, Pi RPC, or Claude stream-JSON/Agent SDK as appropriate; Herdr or the headless `tui-driver` is reserved for terminal-specific claims, and browser claims stay browser-driven. Qualitative UI/conduct claims use owned outer walkthroughs. Incidental simplicity/quality observations are evidence, never completion criteria unless promoted.
