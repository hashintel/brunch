...
- **Current execution pointer:** `memory/cards/exchange-rendering--sweep.md` (ledger). 9 of 12 rows built as of 2026-07-03; open rows scoped as three independent cards — `memory/cards/exchange-rendering--present-candidates-row.md`, `memory/cards/exchange-rendering--structural-illegal-recovery-row.md`, `memory/cards/exchange-rendering--rendercall-coverage-row.md` (build renderCall last; shared write paths) — plus the family-wide DoD residuals (family-completeness registry test not yet written; per-formatter honesty tests + preview entries for the request formatters). Head slice built 2026-07-02 (`beede8d5`, D104-L); its consumed scope card deleted per cleanup. The 2026-07-03 consolidation of core contracts into `src/exchanges/` is recorded as D108-L.
