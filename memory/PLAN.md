...
**Current closure lane.** FE-1187 is the sole remaining ship-gate frontier. The digest-questionnaire carrier, atomic local/RPC review settlement, and borderless Impact Ledger renderer with structural oracles are built. The `remediation-4` continuation now consolidates the remaining walkthrough notes into one branch-local closure bundle: terminal-adaptive TUI chrome, canonical spec/orientation menu treatment, a narrowly reopened persistent elicitation-style model, readiness-filtered action menus, full Ask evidence, Continue-language cleanup, and a 1.x data-model handoff for the KA stream. The Impact Ledger readability walkthrough and separate web graph audit follow those reshaped surfaces. The consolidated outer checkpoint remains paused until R6/R8–R10/R12/R13 have owned dispositions. Execute evidence O7–O9 belongs to the KA stream; FE-1187 coordinates but does not own it.
...
- `walkthrough-remediation-2` ([FE-1187](https://linear.app/hash/issue/FE-1187/walkthrough-remediation-sweep-2-wr18-follow-up-closure)) — remaining ship-gate closure: reconcile the persistent elicitation-style decision, then close the replanned TUI shell/orientation/Ask/Continue/KA-handoff bundle on `remediation-4`; run the Impact Ledger + separate web graph audit over the resulting surfaces; then finish R6/R12, provider/human evidence, and the paused outer checkpoint. Execute O7–O9 stay KA-owned. Closing member of arc `deterministic-orientation`. Definition below.
...
- **Branches:** `ln/fe-1187-walkthrough-remediation-2` (tied-off first tranche) → `ln/fe-1187-remediation-3` (tied-off Impact Ledger continuation) → `ln/fe-1187-remediation-4` (current continuation).
...
- **Built:** D123-L model/auth reversal; deterministic remediation rows; sweep-debt tripwire; digest feedback → bounded questionnaire/confirmation carrier; shared atomic local-TUI/RPC review settlement; discriminating seed variants for propose/project/review routing plus settlement visibility; the human-selected borderless Impact Ledger with narrow/normal/wide goldens, word-wrap-tolerant render honesty, a naive differential inventory oracle, and a deterministic 17-node/11-edge content-length variant gallery; and the R6 ask-only terminal status rail over unchanged canonical Markdown with fail-closed details validation. The exact review payload remains durable and approval produces one receipt-bearing terminal, one LSN, and one change-log entry.
- **Next action:** route R12's no-model/footer/working-state defect through authenticated `ln-diagnose`; do not infer a correction boundary before observing its owner and cause.
- **Replanned `remediation-4` bundle (same frontier / issue / branch):**
  ...
  - **Editor, footer, intro, and spec menu:** the editor border owns the operational-mode and specification-title labels. The stable dim footer is only the conditional web URL plus `model_info | thinking_level | context_percentage`; transient working/status display remains separate. Replace `/brunch:menu` and `ctrl+shift+b` outright with `/brunch:spec-menu` and `alt+s`; share one menu shell while keeping startup and in-session decision logic distinct. Move the welcome copy out of the header into a one-time borderless, non-transcript new-session card teaching spec/menu, mode, consult/continue, model, and thinking controls, including the vendor-neutral guidance that low/medium thinking often works best.
...
- **Remaining routes:** R13 disambiguation + SPEC revision are complete; implementation rides the next scope. R6 four-state result visuals → `ln-design`; R12 no-model behavior → `ln-diagnose`. O7–O9 are KA-owned D120-L Execute evidence. The [`consolidated outer checkpoint`](cards/walkthrough-remediation-2--consolidated-outer-checkpoint.md) remains paused until these routes and R8–R10 are dispositioned.
- **Live scope files:** completed D119-L [`Continue lexical closure`](cards/walkthrough-remediation-2--continue-lexical-closure.md) sweep, retained until its completed pointer can be retired; completed four-card [`deterministic remediation queue`](cards/walkthrough-remediation-2--deterministic-remediation-queue.md), now retained only as consumed build context until derivative cleanup; paused [`consolidated outer checkpoint`](cards/walkthrough-remediation-2--consolidated-outer-checkpoint.md). The independent [`Brunch 1.x data-model handoff`](../docs/architecture/BRUNCH_1X_DATA_MODEL_HANDOFF.md) is delivered. A48-L’s semantic/model preflight remains unscoped until a separate proving card can bind deterministic latency and quality acceptance.
...
group-1 (Active — walkthrough closure):
  walkthrough-remediation-2 (FE-1187, absorbs closed FE-1167)
    ...
    status: D98/D109 carriers, terminal-adaptive shell, deterministic menus, full Ask
      matrix, and KA handoff built; active D119 lexical-closure sweep precedes the
      remaining R6/R12, provider/human evidence, and outer walkthrough
