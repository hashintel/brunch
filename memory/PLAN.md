...
- `origination-follow-ups` (small FE-852 residue, 2026-06-12): (a) `kickTurnMessage` content still claims "a structured exchange offer was just presented" — canned-offer-era copy contradicting revised D78-L; needs a deliberate wording pass + walkthrough (live-prompt surface). (b) Optional: widen the `.brunch/debug/entry-contents.md` mirror to pi-side appends (`brunch.kick`) — never requested; smaller now the canned offer is gone. (c) **Defect noticed in manual testing (2026-06-12): the resume-path kick doesn't always fire.** New-session kick works; on resume the kick is intermittent. The 2026-06-12 `ln-induct` pass named three silent-skip suspects, none product-observable: (1) the model-availability guard on the kick in `src/app/brunch-tui.ts` skips with zero trace — prime suspect (unauthenticated/slow-auth resume); (2) kick failure lands in `.catch(console.error)` inside a TUI, an invisible error channel; (3) a no-debt `action !== 'start'` decision records no reason why no kick was owed. Repair class: make the origination decision/outcome a product observable (status chrome and/or debug cache); also verify the `rpc/methods/session.ts` manual_trigger site replicates the same guard set (two call sites, drift unchecked). Lower-line work (product wiring), not demo-polish — the demo-polish pending-indicator item is the presentation half of the same observable. **Scoped 2026-06-12:** prepared chain at `memory/cards/origination-follow-ups--kick-observability.md` (kick-completion seam + outcome sink → decision/outcome observables → D78-L wording lock); build deferred until the demo-lane stack merges to `next`. Scoping verified the RPC `triggerExchange` site is intentionally seed-only (one kick site, no drift defect) and that idle reasons already exist on `StartAssistantTurnDecision` but are discarded.
...
- **Design docs:** `src/renderers/README.md`; renderer feedback-loop rationale from the retired cross-cut ledger is archived in `docs/archive/PLAN_HISTORY.md`.
