...
- **Kind:** structural
- **Status:** active — slices 1 + 2 + 3a of 5 done (export reducer + identity-fold engine wiring + `--petrinaut-fold` cook CLI flag + in-process stream bus with replay buffer + `BrunchExecutionExportFrame` discriminated-union wire shape). Next: slice 3b — HTTP server (`http.createServer` + `listen(0)`) + `/stream` (text/event-stream) mounted on the 3a bus. Bristol-demo integration path; supersedes the earlier static single-file bundle idea.
