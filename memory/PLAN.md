...
- **Certainty:** proving.
- **Status:** active; inner writer-authority and TUI semantic-adapter contracts, production composition wiring, semantic-only canonical `/rpc`, and split-transport regressions are built in FE-1321. The deterministic PTY journey through the production real `InteractiveMode` is now witnessed end to end — production boot chrome and Pi editor, one ordinary turn, that exact exchange in the sole canonical JSONL, and bounded Ctrl-D cleanup — with the provider backend substituted through a `runBrunchTui` option that mirrors `runBrunchWeb`'s, never through a replacement launcher. Companion React convergence, structured ask, authority rivals, lifecycle transfer, and the colleague walkthrough are unscoped; re-run `ln-scope` against the child/report shape that journey established. The rejected independent-host evidence: Pi 0.83.0's public `InteractiveMode.stop()` leaves `AgentSessionRuntime` alive, but `run()` remains pending forever with its unresolved `getUserInput()` callback. Design history: [`docs/design/WEB_UI_ARCHITECTURE.md`](../docs/design/WEB_UI_ARCHITECTURE.md).
...
- **Live scope files:** [`companion React semantic convergence`](cards/shared-session-host-tracer--companion-react-convergence.md) — scoped 2026-08-07 against the landed PTY child/report shape; proves a TUI-typed ordinary turn reaches companion React through target-addressed semantic deltas alone and converges with a fresh canonical-JSONL projection. The structured-ask, rival-refusal, and shutdown/reopen slices stay unscoped until it lands.
