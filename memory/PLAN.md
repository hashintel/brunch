...
- **Next action:** build Card 2 of the active four-card [`deterministic remediation queue`](cards/walkthrough-remediation-2--deterministic-remediation-queue.md): terminal-adaptive shell → deterministic orientation rendering/fallback → deterministic Ask matrix. The vocabulary audit and SPEC revision are complete; historical/archive material remains provenance, not rewrite scope.
...
- **Live scope files:** active four-card [`deterministic remediation queue`](cards/walkthrough-remediation-2--deterministic-remediation-queue.md) with Card 1 done and Card 2 next; queued/independent [`KA 1.x data-model handoff`](cards/walkthrough-remediation-2--ka-data-model-handoff.md), which may land at any point before the outer checkpoint; paused [`consolidated outer checkpoint`](cards/walkthrough-remediation-2--consolidated-outer-checkpoint.md). Continue lexical closure re-enters `ln-scope` only after Card 4 exposes the final current-surface inventory; A43-L’s semantic/model preflight remains unscoped until a separate proving card can bind deterministic latency and quality acceptance.
...
group-1 (Active — walkthrough closure):
  walkthrough-remediation-2 (FE-1187, absorbs closed FE-1167)
    ...
    status: D98/D109 elicitation-style revision specified; next materializes its
      carriers/prompt/juncture seam, then terminal-adaptive shell, readiness-filtered
      menus, full Ask evidence, Continue lexical closure, KA handoff, outer walkthrough
