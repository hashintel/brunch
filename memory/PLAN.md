...
- `tool-schema-convergence` (FE-1163) — implementation landed across all 51 provider-facing Brunch tool surfaces, but the 2026-07-10 closure audit reopened the sweep: object-root enforcement plus the exchanges, dev-mode, graph, and context semantic differentials are now closed; the aggregate registry oracle still covers only 48 tools, and the remaining converted families still lack genuine pre/post differentials. Restored ledger: `memory/cards/tool-schema-convergence--ledger.md`. Definition below.
...
frontiers:
  Active:
    ...
    tool-schema-convergence (FE-1163)
      status: active closeout after 2026-07-10 closure audit; object-root + exchanges/dev/graph/context differentials closed; breadth/remaining differential rows remain partial
