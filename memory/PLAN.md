...
5. `app-runtime-probe` — **(slice 1 landed — FE-875, `runProbe`)** build + boot + exercise the host app; the concrete reachability mechanism `integration-oracle` depends on (without it, "reachable" collapses back to "a test that imports the module"). Slice 1: boot + HTTP probe + reachable/not-reachable/infra classification + teardown. Remaining: mode-awareness, integration-oracle gating, ProbeSpec sourcing.
