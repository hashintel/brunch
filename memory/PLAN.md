...
- `prompt-skill-topology` (FE-898, branch `ln/fe-898-prompt-skill-topology`) — active: realign `.pi/skills/` to the Agent Skills standard topology + frontmatter, reusing pi's `loadSkills` over a **code-owned path list** (D39-L sealing preserved, no ambient scan). Reverses the FE-893/`alpha-hardening` SKILL.md-shape deferral now that the user has decided to adopt the standard. Scoped: `memory/cards/prompt-skill-topology--agentskills-alignment.md`; D58-L/D85-L revision is part of this slice.
