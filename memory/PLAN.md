...
1. `sealed-pi-profile-runtime-state` — In-flight prep envelope on `ln/fe-776-graph-layer-prep-profile`. Two strands: **(a)** Pi harness sealing — substantially landed; remaining: best-effort lifecycle-generated session display names over Pi `session_info`. **(b)** Graph-model lock-and-materialize — Phase 1 (edges) and Phase 2 (nodes) both locked in `docs/design/GRAPH_MODEL.md` + stubs under `src/graph/` + SPEC reconciled (D51-L, D54-L–D57-L); ✓ A20-L Drizzle persistence spike validated (2026-06-01). Tie off when session display names land so `graph-data-plane` CRUD can start on settled foundations.
...
- **Current execution pointer:**
  ...
  - **Graph-model strand:** Phase 1 edge lock landed at commit `100585a1`. Phase 2 node lock landed at commits `b6ecec1e`–`8346e23d`: 11 intent kinds in 3 categories, common `GraphNode` shape with `detail` JSON column, `provenance` retired from both nodes and edges, `framing_as` retired, modality/source-question rubric + context promotion heuristic for agent prompting, code stubs at `src/graph/schema/nodes.ts`. Canonical doc is [`docs/design/GRAPH_MODEL.md`](file:///Users/lunelson/Code/hashintel/brunch-next/docs/design/GRAPH_MODEL.md) (both phases locked). SPEC reconciled via D54-L/D55-L/D56-L/D57-L. ✓ **(ii)** A20-L persistence spike validated (2026-06-01): `drizzle-orm@0.45.2` + `drizzle-kit@0.31.10` + `better-sqlite3@12.8.0` + `drizzle-typebox@0.3.3` + `@sinclair/typebox@0.34.14`. Batch `commitGraph`-shaped transaction (multi-node → ref resolution → multi-edge → LSN → change-log) atomic with full rollback. `drizzle-typebox` derives valid row schemas; Pi tool parameters use `typebox` v1.x separately; shared enum arrays bridge both.
  - **Tie-off:** session display names (sealing strand) is the sole remaining item. When both strands reach acceptance, mark the frontier done and let `graph-data-plane` (M4 CRUD) start with settled persistence/schema-derivation/graph-contract foundations.
