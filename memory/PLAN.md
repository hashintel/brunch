...
- **Audit reopening inventory (replacement ledger authored at `ln-scope`):**
  ...
  18. ● Focused outer closure evidence — satisfy the single authoritative O1–O10 end-of-frontier checklist in `memory/cards/walkthrough-remediation-1--closure-ledger.md` §WR18. `Not encountered` / `not observed` is not closure; WR5 conduct not naturally reached in Run B requires a focused conduct beat. New product defects route through explicit planning dispositions rather than being silently absorbed.
- **Aggregate DoD:** WR1–WR8 remain built; no required WR13–WR18 row remains open; D120-L/I62-L are materialized in topology/code; every WR18 O1–O10 obligation reaches the ledger's allowed terminal state with evidence in `TESTING_FINDINGS.md`; any failure has an explicit promoted owner and PLAN disposition.
