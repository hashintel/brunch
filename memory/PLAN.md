...
- **Deferred (deliberately not queued):** export/replay subcommand (`brunch petrinaut replay` / export flag) — JSON format inherits the A99 marking decision, scope after cross-team confirmation; the Petrinaut team is adding a Petrinaut-side export button in parallel. Semantic-lane-suppression flag (smaller demo graph) — projection design unsettled. Single-slice fixture — FE-815 territory. `--petrinaut-linger` rejected at the sync in favor of export/replay.
