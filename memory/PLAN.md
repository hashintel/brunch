...
- **Current execution pointer:** threads 2–6 scoped 2026-07-08 into five files (build in this order; parenthetical = why): `memory/cards/main-editor-chrome--ask-surface-ux.md` (settles content/goldens once) → `main-editor-chrome--details-driven-rendering.md` (consumes that form; carries the D104-L rewrite; review-set renderer deliberately unscoped until this lands) → `main-editor-chrome--mode-reactive-chrome.md` (getter seam + `ask.ts` merge order) → `main-editor-chrome--commands-and-menus.md` (shares `commands/index.ts` with mode-reactive; Card 3 needs ask re-present loop; Card 4 independent) → `main-editor-chrome--theme-demo-expansion.md` (witnesses the roles the others create).
