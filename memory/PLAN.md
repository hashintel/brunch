...
- `exchange-ask-refinement` ([FE-1164](https://linear.app/hash/issue/FE-1164/one-shot-ask-terminal-and-exchange-presentation-refinement)) — **built 2026-07-07; review pass 2026-07-08 found it not-done**: 22 silently skipped tests (the answering behavior suite + two live proofs), two fallback-ladder regressions (stubbed-custom → editor rung; multi-choice editor-envelope rung), the legacy present-question/request-response stack unretired, and the continuation declaration not actually driving dispatch (hardcoded review vocabulary + per-kind switch survive). Fix slice scoped: `memory/cards/exchange-ask-refinement--review-fixes.md` (current execution pointer). Branch `ln/fe-1164-ask-terminal`. Definition below.
