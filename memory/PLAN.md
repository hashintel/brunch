...
8. `plan-build-architect` — **FE-829**; **active, building on `ka/fe-829-plan-build-architect`** (stacked on FE-827). Evolves `brunch plan` from a sequencer into a build-architect emitting fixture-quality, cook-executable `plan.yaml` (target shape = reference fixtures `parallel-utils`/`layered-todo`/`resilient-pipeline`). Slice 1 (building now): self-contained `PlanContract` (schema-checkable executability invariants) + deterministic repair in the emitter — synthesizes the multi-slice-epic integration seam, closing FE-800's integration-blind / "green checks, no artifact" gap (D167-K, I129-K; no LLM, no D160-K change). Later slices: build-architect LLM stage (criteria+edges, schema-constrained, fixtures as few-shot exemplars), file-ownership/join + the D160-K amendment + `Slice.writes`, eval harness vs fixtures. Refinement on `spec-to-cook-plan` (FE-800).
...
- **Linear:** FE-829 (standalone) · branch `ka/fe-829-plan-build-architect` (stacked on FE-827)
- **Kind:** structural
- **Status:** active — slice 1 (executability contract + repair) building now. SPEC records A100-K, D167-K, I129-K, Future Direction §Cook plan generation.
...
- **Traceability:** Requirements 46–50; A97, A100-K; D158-K, D160-K (amended in slice 3), D161-K, D167-K; I129-K; Future Direction §Cook plan generation. Refinement on `spec-to-cook-plan` (FE-800) — resolves its integration-blind follow-on. Builds on FE-826, FE-827.
