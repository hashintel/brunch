...
1. `sealed-pi-profile-runtime-state` — ✓ **Done.** Both strands complete on `ln/fe-776-graph-layer-prep-profile`. **(a)** Pi harness sealing — fully landed including session display names via Pi `session_info`. **(b)** Graph-model lock-and-materialize — Phase 1 (edges) and Phase 2 (nodes) locked; A20-L Drizzle persistence spike validated (2026-06-01). Ready for `graph-data-plane` (M4 CRUD).
...
- **Kind:** structural
- **Status:** next / unblocked (prep envelope tied off 2026-06-01)
...
- **Current execution pointer:** **(1)** ✓ `src/db/` Drizzle schema + `initSchema` DDL push + graph_clock seed — landed with prep envelope + skeleton commit. **(2)** ✓ `CommandExecutor` result contract (`success | structural_illegal | needs_human | policy_blocked | version_conflict`) and one-transaction LSN/change-log skeleton in `src/graph/command-executor.ts` — landed with `createNode` proof-of-life, I26-L architectural boundary test, and NodeId/EdgeId corrected to `number`. **(3)** widen to single-edge CRUD (`createEdge` with stance/category structural validation per GRAPH_MODEL.md); **(4)** add `commitGraph` batch mutation with intra-batch and existing-node reference resolution (D53-L, I34-L); **(5)** add graph snapshot readers at cursory and neighborhood detail levels (I35-L); **(6)** add the reconciliation-need substrate and oracle-plane stub tables. Keep M4 thin enough to falsify A3-L / A4-L / A6-L / A8-L / A14-L-readiness before widening CRUD or coherence homes.
...
nodes:
  ...
  graph-data-plane                  [next · unblocked]       (M4 CRUD proper)
