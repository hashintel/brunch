...
2½. `origination-kick-live` (rides FE-857) — **done 2026-06-11**: the kick now fires live (the harness-as-false-proof defect is closed by the product-originated-turn oracle), Brunch entries are debug-observable without provider calls, and the `setLabel` look concluded "don't label" (see frontier definition).
2¾. `origination-native-elicitation` (FE-852) — **done 2026-06-12**: the deterministic structured-exchange fossil retired per the revised D78-L/D49-L — origination seeds and kicks only, the assistant authors the opening live; content-rich seed (workspace + spec + full canonical graph overview, never truncated); elicitor guidance carries the situating ground and live `present_*`/`request_*` authorship. Generator relocated to probe-land for R24 evidence.
...
- 2026-06-12 post-burst judo/refactor pass (FE-852, same branch) — seven behavior-preserving commits from the `ln-judo-review` of the 2¾ burst: pi-private `_rewriteFile` flush contract named once (`session/flush-session-manager.ts`, five copies replaced); present-side synthetic writers moved to probe-land (module boundary now enforces "product never mints offers"); `workspaceContext` required on origination/seed with one composing helper (`renderWorkspaceOverviewContext`); tier-2 harness env/faux boilerplate collapsed and the boot test file split (`tier-2-harness` kick suites / `tier-2-scaffold` I45–I47 / `tier-2-test-support` helpers); `formatGraphOverview` re-export shim retired. **Residual finding for sync/follow-up:** `kickTurnMessage` content still claims "a structured exchange offer was just presented" — canned-offer-era copy contradicting revised D78-L; live-prompt surface, so left unchanged pending a deliberate wording pass + walkthrough.
- 2026-06-12 `origination-native-elicitation` (FE-852) — demo block 2¾ done on `ln/fe-852-below-the-line`: canned deterministic exchange retired from product origination (generator → probe-land; `triggerExchange` = kick surface per revised D49-L); seed payload now carries workspace overview + spec header + full canonical graph overview + ranked-gap framing (one render dialect with `read_graph`); elicitor guidance covers the situating ground and live exchange-tool authorship. Tier-2 oracle proves the tool-call-free, no-canned-offer opening; opening quality pending next walkthrough.
...
- **Status:** **done 2026-06-12** (3-card chain consumed and retired)
- **Certainty:** earned
...
- **Traceability:** D18-L, D27-L, D65-L, D66-L, D80-L, D81-L, D82-L; A22-L.
- **Design docs:** `memory/SPEC.md` D80-L/D81-L/D82-L (mechanism/policy/acquisition), D65-L/D66-L; `src/projections/README.md`.
- **Near-future successor (named, not this block):** subagent acquisition — exploration/research modes delegated to side/sub-agents with the digest as handback (SPEC Future Direction §Subagent acquisition).
