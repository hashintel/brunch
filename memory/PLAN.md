...
- **Workflow + persistence seam (`src/server/core.ts`, `src/server/app.ts`, `src/server/db.ts`, `src/server/schema.ts`, `src/server/parts.ts`, `src/shared/api-types.ts`, `src/shared/project-state-turn.ts`)** — this stack still fabricates kickoff / recovery as durable `turn_kind` rows, auto-seeds frontier turns through `ensureProjectFrontier()`, exposes frontier/control meaning through turn records, and still trusts persisted `assistant_parts` / `user_parts` JSON more optimistically than a cross-restart boundary deserves. Active items 1, 2, and 3 must replace that with explicit stream projection, anchored phase-outcome authority, truthful hydration landings, projected control semantics, and a clearer runtime-authority seam for persisted parts.
...
1. **Merged stream projector cutover: truthful landings, slim charts, and projected controls** — make the center column honest about artifact types by projecting it from active-path turns, anchored workflow facts, and projected control / activity / phase-marker elements instead of treating every visible card as a durable turn, and make hydration land in those projected states through a pure reconciler rather than through kickoff/recovery turn heuristics.
   ...
   - What this slice must accomplish:
     ...
     - treat persisted `assistant_parts` / `user_parts` as a real runtime boundary during projection and resume: durable data-part payloads should be parsed through the shared schemas that already own them instead of relying on `JSON.parse(...) as ...` casts in read-model helpers
   ...
   - Execution guidance for this frontier:
     ...
     - preserve the current oracle stack while rewriting the seam: shared contract tests, SQLite-backed integration tests, fixture/corpus probes, routed controller/component tests, build-boundary tests, and manual seeded walkthroughs are the current verification backbone
     ...
     - keep runtime validation at true uncertainty seams only: HTTP ingress, LLM/tool I/O, route-param/search ingress, and cross-restart persisted payloads should validate at ingress; controlled in-process seams should flow by shared TypeScript owners and inferred types rather than mirrored internal parse layers
