...
- **Members:**
  ...
  - `exchange-answering-chrome` ✓ built (2026-07-06) — live answering UI: `choice`/candidate-choice/review use bordered decision pickers, multi-choice uses the bordered checkbox picker, and free-text `answer` uses `ExchangeAnswerEditorComponent` via `ctx.ui.custom` with the sealed `ctx.ui.editor` as fallback.
- **Done-definition:** every exchange kind in the closed inventory renders honestly in transcript and re-render; live local-TUI answer collection no longer routes primarily through raw Pi picker/editor chrome; each renderer and live answering component has a `dev:components` preview entry; `src/.pi/extensions/exchanges/TOPOLOGY.md`, `src/exchanges/schemas/TOPOLOGY.md`, `src/projections/TOPOLOGY.md` shape ledger, and `src/.pi/components/TOPOLOGY.md` reconciled; the formatter-home decision recorded in `memory/SPEC.md` (done: D104-L/D108-L). Arc residue: physical-terminal smoke for the new `Editor`-inside-`ctx.ui.custom` surface remains owed before marking the arc ✓ done.
...
- 2026-07-06 `exchange-answering-chrome` (FE-1138) — code DoD closed for the live answering chrome frontier: free-text `answer` now uses a Brunch-owned bordered `ExchangeAnswerEditorComponent` via `ctx.ui.custom`, the sealed `ctx.ui.editor` remains fallback, headless broker precedence is preserved by `hasUI`-first guards, empty/whitespace free-text answers are rejected at both TUI and RPC/schema boundaries, shared editor-rule stripping lives in `.pi/components/editor-lines.ts`, and `dev:components` exposes `exchange-answer-editor`. Manual physical-terminal smoke remains owed before closing the `exchange-presentation` arc.
...
- **Lane C — exchange presentation:**
  1. `exchange-answering-chrome` ([FE-1138](https://linear.app/hash/issue/FE-1138/bordered-answering-chrome-for-structured-exchanges)) — code frontier built; arc remains open only for the owed physical-terminal smoke of the free-text custom editor surface. Arc: `exchange-presentation`.
...
- **Status:** built 2026-07-06; **physical-terminal smoke confirmed 2026-07-06** (gallery + live session, both threads) — frontier build work complete, ready for arc closeout at next `ln-sync`. `choice` / candidate-choice / review local-TUI paths use Brunch-owned `ctx.ui.custom` bordered picker chrome, and `MultiChoicePickerComponent` is wrapped in the same bordered shell. Free-text `answer` uses `ExchangeAnswerEditorComponent` via `ctx.ui.custom` with the sealed `ctx.ui.editor` fallback and the existing headless broker third; empty/whitespace free-text answers are rejected at the TUI and RPC/schema boundaries.
- **Certainty:** proving.
...
- **Objective:** Two threads (formerly threads 2–3 of `bordered-chrome-production`; the main-editor thread moved to `main-editor-chrome`, Horizon). `ctx.ui.setEditorComponent` and `ctx.ui.editor(...)` are two structurally distinct mechanisms in `pi-coding-agent` (the former replaces the persistent main chat input editor; the latter is `request_response`'s own one-shot free-text answer dialog, `ExtensionEditorComponent`, entirely unaffected by `setEditorComponent`), which is exactly why the main-editor work could split away cleanly:
  1. **`answer` (free-text) response kind**: **built 2026-07-06** — `request_response`'s `collectAnswerFromSources` path uses a Brunch-owned bordered `ExchangeAnswerEditorComponent` via `ctx.ui.custom` (not `setEditorComponent`), following the custom-first + sealed-editor-fallback + broker-third chain; the component hosts pi-tui `Editor`, strips the editor's self-drawn rules, re-prompts on empty submit, and shares editor-line helpers with `BrunchEditorComponent`.
  2. **`choice` and review response kinds**: **built 2026-07-03** — `collectChoiceFromUi`/`collectReviewFromUi` use the same `ctx.ui.custom`-backed bordered-picker treatment, reusing `projectRoundedBox` + `projectScrollViewport`; raw `ctx.ui.select` is no longer a fallback for these paths. The `STRUCTURED_EXCHANGE_ANSWERING_PATHS.md` coverage matrix remains the non-regression proof that `session.submitExchangeResponse` (column B) bypasses local `ctx.ui.*` entirely.
- **Acceptance:** closed for both threads, including the manual physical-terminal smoke (confirmed 2026-07-06). No frontier residue.
- **Current execution pointer:** none; `memory/cards/exchange-answering-chrome--answer-editor.md` consumed 2026-07-06.
- **Carried design question (2026-07-02, from `exchange-rendering`):** GitHub-style per-item review commentary — widening the review answered payload (`comments: [{on: draft|edge|set, body}]`) plus the collection UI to gather it. The payload half is a SPEC decision; the collection half belongs to this frontier's review-answering surface. **Evaluated at scope (2026-07-03): deferred post-gate** — the widened payload would ripple into the review schema that `exchange-capture-contract` rows and `present-digest`'s reused review terminal consume mid-gate; ship-gate posture defers enhancements. Transcript sketch in `src/agents/contexts/exchanges/design-permutations.md` §Review-set evaluation.
