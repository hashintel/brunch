...
- **Next action:** scope the D98-L/D109-L materialization first: replace the mixed `brunch.session_orientation` carrier with persistent `brunch.elicitation_style` (`interrogate | disambiguate | propose`) plus one-shot `brunch.process_move`; wire active-branch style into every elicitor prompt; reduce automatic menu junctures; and preserve the readiness fallback boundary. The vocabulary audit and SPEC revision are complete; historical/archive material remains provenance, not rewrite scope.
