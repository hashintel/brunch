...
- **Live scope files:** none. The authority-and-transfer scope file landed both its slices on 2026-08-07 as `src/app/__tests__/session-runtime-contract-authority.slow.test.ts` and was consumed, as was the structured-ask scope file before it. Every automated limb A51-L names is now witnessed; what remains on this frontier is the colleague walkthrough, which is the user's to run and needs no scope file.
