...
- **Status:** active. First slice (the preview harness) shipped; refinement/new-component slices are open-ended and may use their own `memory/cards/component-dx--<slug>.md` card while in progress, but each card is retired once its durable state is reconciled into this definition plus the co-located `TOPOLOGY.md` homes rather than kept as a standing artifact. **Current execution pointer:** `memory/cards/component-dx--rounded-box-primitive.md` (active, `Mode: slices` — consolidates the three hand-rolled bordered-box implementations named in the harness/scroll-viewport work above into one `projectRoundedBox` primitive before the `request_*` picker work touches them).
