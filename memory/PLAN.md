...
- `subagent-adoption` (split from `core-tools-adoption` 2026-06-19; the web-tools half landed 2026-06-19 inside FE-861 — `web_fetch`/`web_search` registered in the sealed profile, base-allowed in `elicit-read-only`) — decide how Brunch adopts a basic `subagent` tool inside the sealed profile. All surveyed reference implementations (canonical pi example, `amosblomqvist/pi-config`, `nicobailon/pi-subagent-enhanced`, `nicobailon/pi-subagents`, `ogulcancelik/pi-spar`) spawn a child `pi` process and rely on `~/.pi/agent/` discovery for agents/tools, which conflicts with D39-L. SDK-only alternatives (`shitty-extensions/oracle.ts`, `loop.ts`) use `complete()` from `@earendil-works/pi-ai` for single-call second opinions / loops, not tool-bearing subagents. This frontier resolves the execution model (child `pi` vs in-process SDK vs Brunch-CLI subprocess) and the agent-definition boundary before capture design can assume delegated acquisition.
- `prompt-skill-topology` (new frontier 2026-06-19; def below) — realign `.pi/skills/` to the Agent Skills standard topology + frontmatter, reusing pi's `loadSkills`/`formatSkillsForPrompt`/`parseFrontmatter` over a **code-owned path list** (D39-L sealing preserved, no ambient scan). Reverses the FE-893/`alpha-hardening` SKILL.md-shape deferral now that the user has decided to adopt the standard. Scoped: `memory/cards/prompt-skill-topology--agentskills-alignment.md`. The user's chosen next frontier after FE-861's matrix; stacks on the FE-861 branch; needs an FE issue + branch + `ln-spec` (D58-L/D85-L revision) at pickup.
...
- **Current execution pointer (2026-06-19):** the D80-L banded sweep + watermark, the D81-L commitment-gradient routing gate, the contradiction→`reconciliation_need` outlet, the submit-time labeled-prefix fossil retirement, the `web_fetch`/`web_search` prerequisite, and the D82-L acquisition-modes + digest + situating-gap layer have all landed on the FE-861 stack (slice history in git; SPEC I30-L/A22-L + the .pi/extensions/rpc/web/graph topology READMEs carry the durable record). The two deterministic capture oracles (routing gate + sweep-watermark property) are green, now including fixed contradiction-tagged routing to one semantic-conflict recon need. **Remaining (FE-861's last item, on `ln/fe-861-generalized-capture-2`):** false-commit scenario-matrix completeness — scoped + in flight in `memory/cards/generalized-capture--false-commit-matrix.md` (re-aim the closed capture-quality-spike family to the D81-L gradient outcome; every scenario class a deterministic regression guard through the real adapters; probe stays fitness). On land, FE-861 is **complete** → tie off (PR description + `gt submit`). Next frontier on the stack: `prompt-skill-topology` (the Agent Skills realignment; def below).
...
### prompt-skill-topology

- **Name:** Prompt-resource topology + format realignment to the Agent Skills standard
- **Linear:** unassigned — needs an FE issue at pickup (reverses the D85-L deferral; **not** an FE-861 slice)
- **Branch:** tbd — stacks on the FE-861 branch (`ln/fe-861-generalized-capture-2`)
- **Kind:** structural / format adoption
- **Status:** parallel / near-term — the user's chosen next frontier after FE-861's matrix lands (2026-06-19)
- **Certainty:** proving (the standard-format-improves-LLM-use claim is a fitness bet; the realignment itself is deterministic and golden-verifiable)
- **Depends on:** `prompt-skill-consolidation` / D85-L (done — the `.pi/skills/` + `.pi/agents/<agent>/SYSTEM.md` topology this realigns); land after `generalized-capture`'s capture + acquisition skill bodies settle so the move does not churn an in-flight build
- **Objective:** Adopt the [Agent Skills standard](https://agentskills.io/specification.md) topology + format for Brunch's prompt resources — `skills/<family>/<name>/SKILL.md` with YAML frontmatter, parsed by pi's own loader over a code-owned path list (no ambient scan), advertised through a pi-format skills block. D39-L sealing preserved.
- **Why now / unlocks:** the standard "skill" naming/topology/frontmatter activates LLM training priors and is more reliably used; reverses the FE-893/`alpha-hardening` deferral of the SKILL.md shape now that the user has decided to adopt it. Reuses pi's exported `loadSkills`/`formatSkillsForPrompt`/`parseFrontmatter`, minimizing Brunch-owned surface.
- **Resolved decisions (2026-06-19):** description file-owned (frontmatter) via pi's loader over a code-owned `skillPaths` list (`includeDefaults: false`); path/legal-set code-owned in `state.ts`; **no** `capabilities/` rename; **no** skill-name prefixing; skills block copies pi's `formatSkillsForPrompt` element format. Open micro-decision (build-time): block wrapper (`<brunch-skills>` vs verbatim `<available_skills>`) + axis grouping.
- **Acceptance / full scope:** the committed scope card `memory/cards/prompt-skill-topology--agentskills-alignment.md`.
- **Verification:** Inner — compose manifest-name + ≥700-char readability over SKILL.md paths + sealing/no-discovery (extended: an unlisted SKILL.md is not advertised) + regenerated goldens; the SPEC D58-L/D85-L revision. Outer — fitness (does the standard format improve skill selection/use).
- **Cross-cutting obligations:** D39-L sealing (code-owned path list, no ambient scan); D58-L composition stays projection (name/description/location metadata only, bodies not inlined); free-rewrite (no alias for the old flat `<id>.md` paths); keep the slice to the manifest seam — no agent-body or family/id renames for symmetry.
- **Traceability:** D58-L (refined), D85-L (deferral superseded), D39-L, D52-L.
- **Design docs:** the scope card; `memory/SPEC.md` D58-L/D85-L; https://agentskills.io/specification.md; `src/.pi/skills/README.md`.
...
nodes:
  ...
  prompt-skill-topology          [parallel · proving · new 2026-06-19] adopt Agent Skills SKILL.md topology+frontmatter via pi loader over a code-owned path list; reverses the D85-L deferral; card scoped, FE issue/branch at pickup
...
edges:
  ...
  prompt-skill-consolidation -[enables]->     prompt-skill-topology  (the .pi/skills + .pi/agents/<agent>/SYSTEM.md topology this realigns to the Agent Skills standard)
  generalized-capture       -[before]->       prompt-skill-topology  (realign skill bodies after the capture/acquisition skills settle, not mid-build)
