...
- **Current execution pointer:** M3 scoped web-shell queue is code-complete: shared JSON-RPC protocol helpers/dispatch semantics, `ws`-backed `/rpc` transport, persistent browser RPC client with protocol-failure hardening, canonical built asset serving with traversal-safe asset resolution, stable React runtime, explicit read-only session projection by durable session id through a read-side projection reader, and read-only browser transcript rendering via explicit `{ sessionId, specId }`. Automated verification and HTTP/WebSocket projection postconditions pass. Remaining outer-loop debt is explicit: local Chrome/agent-browser launch was blocked by macOS sandbox/socket permissions during the manual smoke attempt, so a qualitative browser-open/render check still needs to be run before M3 tie-off/submission.
