...
- 2026-05-22 `web-shell` M3 tie-off queue — Done: explicit session projection now rejects ambiguous self-description (`brunch.session_binding` duplicates, missing/duplicate Pi headers, and binding/header session-id mismatch) with a product-shaped read error; `session.transcriptDisplay` includes text-bearing transcript-native `brunch.elicitation_prompt` custom-message rows while omitting operational custom entries; and M3 is marked done with the browser-open smoke debt explicitly adjudicated. Verified: `npm run verify` after each code slice; direct smoke host served built HTML, returned `404` for `/workspace.snapshot`, and served `workspace.snapshot`, `session.transcriptDisplay`, and `session.elicitationExchanges` over WebSocket RPC with explicit `{ sessionId, specId }`, including assistant, prompt, and user transcript text. Accepted deferral: qualitative browser-open smoke remains environment-blocked because `agent-browser` cannot create its socket directory under the current macOS sandbox (`Operation not permitted`), but technical HTTP/WebSocket postconditions passed.
- 2026-05-22 `web-shell` final hardening queue — Done: `brunch.session_binding` creation/parsing/validation is single-sourced; RPC handlers require `cwd` for explicit session projections; browser RPC socket errors are terminal; `session.transcriptDisplay` returns displayable assistant/user rows from linear Pi JSONL through durable `{ sessionId, specId }`; and the React shell renders read-only transcript text rather than IDs/counts only. Verified: `npm run verify` after each code slice; HTTP static asset smoke returned `dist-web/index.html` and `/assets/brunch-web.js`; direct WebSocket RPC smoke used `workspace.snapshot`, `session.transcriptDisplay`, and `session.elicitationExchanges` with explicit session/spec params and returned the seeded transcript text. Watch: superseded by M3 tie-off; qualitative browser-open smoke was adjudicated as environment-blocked rather than product-blocking.
- 2026-05-22 `web-shell` scoped review queue — Done: explicit session projection was split into a read-side resolver that validates durable bindings without opening selected workspace state; stdio/WebSocket share JSON-RPC parse/internal-error dispatch; browser RPC protocol failures close the attachment deterministically; static asset resolution rejects traversal; fixture capture uses the shared response type; and the React shell renders read-only transcript projection state through explicit `{ sessionId, specId }`. Verified: `npm run verify` after each slice. Watch: superseded by the final hardening queue; only qualitative browser-open smoke remains before tying off M3.
- 2026-05-22 `web-shell` hardening slices — Done: shared runtime-neutral JSON-RPC protocol helpers, `ws`-backed `/rpc` server transport with malformed/throwing-handler containment, persistent browser RPC multiplexing over one socket, canonical `dist-web/index.html`/asset serving with an explicit missing-bundle error, stable React runtime ownership of QueryClient/router/RPC client, and explicit read-only `session.elicitationExchanges` projection by durable `sessionId` with optional `specId` binding validation. Verified: `npm run verify` after each slice. Watch: next web-shell work should render transcript state from explicit session projections without introducing browser writes, REST reads, or connection-as-session semantics.
- 2026-05-21 `web-shell` slices — Done: M3 linear transcript policy hardening landed before browser consumption; the file-backed reader now requires one Pi session header plus well-formed non-header entries; shared exchange projection preserves non-linear failures for RPC/fixture reads; `--mode web` now starts a minimal native Brunch HTTP shell with no REST product reads; `/rpc` WebSocket JSON-RPC reuses the existing `workspace.*` / `session.*` handlers with matching non-linear transcript errors; and the native React/TanStack Router + Query shell renders `workspace.snapshot` chrome via a WebSocket RPC client. Verified: `npm run verify` after each slice. Watch: remaining web-shell slices should keep browser state as projection/query cache over WebSocket RPC rather than inventing browser-only session semantics.
