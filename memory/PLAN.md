...
- **Verification:** reducer/export unit tests (SDCPN-minus-scenario + count-reduced markings) with an **inverted frame-replay oracle** (FE-819 Card A superseded the original delta-based oracle: firings now carry full markings, so the oracle reconstructs per-firing deltas from consecutive full markings and asserts non-negativity + token conservation, with untouched pools/budgets visible in every frame); SSE replay-on-connect integration test (late joiner gets the full timeline); ephemeral-port + process-death lifecycle test; cross-team validation of a real export with the Petrinaut team.
