...
- **Status:** scoped 2026-07-04. Current execution pointer: `memory/cards/present-digest--exchange-kind.md` (4 cards: end-to-end tracer incl. terminal echo + render family member + grant; sweep single-carrier retirement; supersession/cancel probes; ingest conduct + walkthrough). Consolidates the parallel `present-digest--exchange-throughline-pi-scope.md` scope file (superseded). The decision-flow chart obligation is discharged in the scope file.
