...
3. **Turn-linked capture projection and compact identifiers** — structural `[status: not-started]`
   ...
   - Verification: inner — DB/API/client projection tests for turn → captured-item mapping. Middle — replay assertions on seeded turns with known captured entities. Outer — browser walkthrough confirming captured sections match the actual knowledge sidebar for the same turn.
   - Verification approach: inner — focused persistence/projection tests around `turn_knowledge_item`; middle — transcript replay checks against seeded scenarios; outer — manual comparison of answered cards vs entity inventory; see `memory/SPEC.md` §Verification Design.
