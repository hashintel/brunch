...
- `core-tools-adoption` (noted 2026-06-12): **web fetch/search landed 2026-06-19 inside FE-861** — Brunch now statically registers `web_fetch`/`web_search` in the sealed profile and base-allows them in `elicit-read-only`. Remaining: **basic subagent** tool adoption, which is a separate delegation surface with distinct legality concerns. Consumers: D82-L acquisition modes (`read-referenced-documents`, `explore-and-characterize`) can now use web read tools; the named subagent-acquisition successor on `generalized-capture` still waits on subagent adoption.
