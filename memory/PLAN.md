...
- **Status:** active; decision-picker slice built 2026-07-03. `choice` / candidate-choice / review local-TUI paths now use Brunch-owned `ctx.ui.custom` bordered picker chrome, and `MultiChoicePickerComponent` is wrapped in the same bordered shell. Free-text answer dialog remains unscoped.
- **Certainty:** proving.
...
- **Current execution pointer:** none. The decision-picker scope card is consumed; thread 1 (free-text answer dialog) remains unscoped.
