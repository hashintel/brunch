...
6. `integration-oracle` — **(Half A landed — FE-876)** oracle asserts product reachability via `app-runtime-probe`. Half A (off-seam, done): `Epic.probe?: ProbeTarget` folds a `runProbe` result into the `verify-epic` verdict — after slices merge into `__epic__/<epicId>/`, the epic is `done` only when tests pass **and** the feature is reachable; `not-reachable` is the FE-800 orphan, `infra` is a harness fault. Probe is gated behind tests passing (never boot a known-broken build); absent probe → unchanged unit verdict; reachability rides the existing `report.passed` pass/fail routing. Half B (remaining, **dispatch seam**): architect emits generic wiring intent, cook agent resolves real wiring (cook-time grounding) and emits the `ProbeTarget` as an `execute`-mode plugin — coordinated with the pi-harness thread, in the FE-738 semantic lane. Promotes FE-800's integration-blind follow-on to a frontier. *(Half B depends on `agent-extension-host`)*
