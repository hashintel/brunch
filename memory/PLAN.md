...
- **Status:** in progress (2026-06-23) — branch `ka/fe-1055-greenfield-git-substrate` (FE-1055, parent `main`); post-Arc-1. **Slice 1 (substrate seam) landed (`40aafd13`):** `createSandbox` fixture mode `git init`s the run worktree with an empty root commit on `brunch/run/<runId>`; both modes now run on one git-backed substrate (I139-K). The `.git`-leak risk was confirmed retired (skipped by name in all merge/promotion/CoW paths; 177 orchestrator + full-suite tests green). **Remaining:** slices 2 + 3 are a **coupled convergence pass** (promotion-via-plumbing rides the per-slice fold — see slice notes), best done together with an `ln-oracles` pass and the ~30-test contract migration; not piecemeal. **Outer-loop debt:** greenfield cook smoke + 3-fixture parity (I139-K) not yet run with real pi.
