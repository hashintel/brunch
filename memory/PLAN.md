...
- **Certainty:** proving.
- **Objective:** Recover + reconcile the retired `INTENT_GRAPH_SEMANTICS` content and adjacent heuristic docs into one SPEC-mode data-model reasoning substrate under `src/agents/`: runtime-eligible generated vocabulary in `src/agents/contexts/references/`, authored graph-writing judgment in `src/agents/skills/capture/references/`, and pruned/cited skill bodies. Generate the closed-vocabulary tables (planes / kinds / bands / edge-category policy / `detail` schemas) from typed graph sources so heuristics are **cited** (D97-L), not inlined and duplicated across skill bodies; align the result with D98-L's mode-only runtime posture.
- **Acceptance:**
  ...
  - ✓ Skill bodies cite the new home (D97-L); inlined heuristic copies collapse to one cite-target. Live `capture` and suspended `commit-graph` guidance cite `graph-heuristics.md` for shared graph-authoring judgment; oracle checkability conduct stays skill-local in `generate-proposal/references/oracle.md`.
...
frontiers:
  ...
  Recently Completed:
    renderer-golden-coverage
      ...
      depended_on: context-pipeline PULL+PROJECT, D83-L, D52-L, D58-L, D98-L
      coordinated_with: data-model-legibility (references substrate), elicitor-generate (present_candidates render already landed in house style)
