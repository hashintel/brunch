...
- **Status:** in-progress (command-containment, dynamic chrome semantics, hierarchical spec/session picker startup + in-session flow, RPC/headless initial-selection contract, pty startup oracle, centered branded overlay reuse, evidence-memo reconciliation, structured-question schema/builder, TUI/editor adapters, live Pi RPC editor fallback, and response-side projection have landed; current missing seams are full structured-exchange proof across TUI/RPC/web plus visual chrome recovery)
