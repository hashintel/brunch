...
- **Current execution pointer (2026-07-14 walkthrough pause):** Session B beats 1–3 promoted R5–R13 and the [`consolidated outer checkpoint`](cards/walkthrough-remediation-2--consolidated-outer-checkpoint.md) remains paused. R5 is closed by commit `daba4cda` plus live transient-notification evidence; R7 by `b882d70f` plus live controls-only continuation evidence; R11 by `1343a7c4`, composed-prompt/carrier oracles, and authenticated live confidentiality/disclosure evidence. R8–R10 completed `ln-design`/`ln-spec`: D27-L/D38-L/D99-L/D106-L/D110-L/D116-L now specify conversational digest feedback → one bounded questionnaire/confirmation capture terminal → direct advisory mapping → optional settled-only proposition-shaped review, with compact inspection and one shared TUI/RPC atomic acceptance seam. The digest-questionnaire tracer is built: the core recovery recognizer owns final-digest eligibility, bounded questionnaire and no-question `confirm` / `revise` asks mint one runtime-abstract carrier only on submission/confirmation, and a product-wired registered-ask oracle proves public no-UI RPC convergence with local UI terminals. Shared review settlement is built: local registered `ask` and `session.submitExchangeResponse` now converge through `src/session/review-set-settlement.ts`, which revalidates the persisted offer, commits approval through `acceptReviewSet`, and only then returns/builds the receipt-bearing terminal while each adapter retains its own append mechanics. Compact review rendering must now be separately re-scoped from this landed terminal shape. Unresolved separate routes remain R6 four-state result visuals through `ln-design`, R12 through `ln-diagnose`, and R13 through `ln-disambiguate`/`ln-spec`. Resume the checkpoint only after these promoted routes are dispositioned.
- **Remaining findings inventory:** R6/R12/R13 remain contract-unresolved; R8/R9/R10 have their digest-questionnaire carrier and atomic review settlement built, with compact review rendering re-scope pending; remaining consolidated LN outer evidence; O7/O8/O9 live D120-L Execute workflows (KA-coordinated).
...
- **Folded design rows (2026-07-13/14, promoted findings):** `exchange-visual-design` — the WR9–WR12 cluster (compact tool rendering, `/introspect` legibility, review-set/ask visual revamp, markdown/node-id polish) plus border distinctness and nested-ask chrome (findings A6/A9/A10), now also owning Pi-native P2 `transcript-ledger-rendering` (durable user-choice ledger entries visible in the transcript without entering model context; folded by user ruling 2026-07-14). R8–R10's ingest/review subset is contract-settled in D27-L/D38-L/D99-L/D106-L/D110-L/D116-L (2026-07-14); its next unit is oracle design + scope for the ingest-choreography tracer. `generative-flow-synthesis-shape` — design-it-twice + recommendation/synthesis conduct over existing `present_candidates`/review-set seams (finding C3).
...
- **Verification (R8–R10, designed 2026-07-14):** derive one normalized semantic fixture from the witnessed 17-node/11-edge ingest; inner schema/projection gates cover bounded questionnaire answers and compact review hierarchy; middle loop combines a tiny model-based lifecycle oracle, local-TUI/RPC differential settlement contract, live/persisted compact-render metamorphic, and JSONL/graph negative-space checks; conduct closes only when 3/3 controlled provider runs follow free-text → questionnaire → advisory-before-optional-review choreography while avoiding combinatorial options, mixed-settlement review, and post-review mutation; one representative human walkthrough judges materiality, proposition cohesion, inspectability, and fatigue. See SPEC §Diagnostic Assessment, §Oracle Strategy by Loop Tier, §Design Notes, and §Acknowledged Blind Spots. Proves D27-L/D38-L/D99-L/D106-L/D110-L/D116-L; I15-L/I57-L.
