...
- **Kind:** coverage-shaped dependency-seam upgrade over the closed Pi 0.80.8→0.84.2 change surface. Adaptation is required; adoption is dispositioned, not assumed.
- **Certainty:** proving.
...
- **Change-surface findings (planning pass 2026-08-11, refreshed 2026-08-14 against the incremental 0.84.2 type surface):**
  ...
  | 0.84.2 `SettingsManager.getDefaultTools()` / `getFullscreenExitOutput()` | `src/app/pi-settings.ts` deliberately audits every getter; session construction seals built-ins with `noTools`/explicit lists | **Required, loud.** Add both to the audited surface and prove the new ambient tool default cannot widen Brunch's tool authority; do not pin either setting without product need. |

- **New-API disposition rows (each needs an adopt / promote / decline verdict with rationale):** `PiClient`/`RemoteSession`/CBOR/Unix-socket transport (verdict feeds `shared-session-host-cutover` — see the arc obligation); consuming `assistantMessageEvent` deltas directly instead of prefix-diffing cumulative snapshots (upstream removed the cumulative field precisely because it grew quadratically, so Brunch's re-derivation now works against the grain); `pi.registerMarkdownTransformer()`; `tool_call` `terminate`; `AGENTS.override.md` per-directory context; `pi auth check`; `AgentOptions.shouldStopAfterTurn`; vendor-neutral telemetry contracts plus optional `ToolCall.namespace` / `AssistantMessage.endTurn` diagnostics; configurable `defaultTools` and experimental strict built-in sampling; explicit `sendUserMessage(..., { expandPromptTemplates })`; `samplingParams`; and the expanded fullscreen search/exit/theme surface. Adopt-now is limited to verdicts that remove an existing Brunch workaround or are required for parity; everything else promotes or declines.
- **Aggregate DoD:** no required row remains `spec` / `new` / `partial`; `npm run check` and `npm run verify:full` pass on 0.84.2; FE-1321's four slow session-runtime-contract witnesses and the required FE-1348 subset re-pass on the new baseline; every new-API area carries a recorded verdict; no required row's closure rests on an untyped structural cast; A25-L is updated to validated or falsified with evidence.
