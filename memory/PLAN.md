...
- **Current execution pointer (updated 2026-07-08):** built so far: A1+A2+A3 (`memory/cards/main-editor-chrome--ask-surface-ux.md` consumed: rows, hierarchical esc, compact `formatAsk` content/goldens), C1 (shift+tab cycle), C2 (mode-reactive border roles for main editor + ask surfaces), D1 (menu rename), the judo keybinding fix (`memory/cards/main-editor-chrome--keybinding-scope-fix.md` consumed: in-process thinking-cycle suppression, one-time cleanup of C1's file suppression, restored `alt+m` mode picker, footer/header shortcut hints derived from shared constants), and the ask-step judo refactor (`memory/cards/main-editor-chrome--ask-step-refactor.md` consumed: `StepResult`, shared comment step, choice-row accumulator, exported `collectAskResponse` loop). Remaining order: `memory/cards/main-editor-chrome--details-driven-rendering.md` (D104-L rewrite; consumes compact ask form; review-set renderer deliberately unscoped until this lands) → D2–D3 (`memory/cards/main-editor-chrome--commands-and-menus.md`) → D4 (independent, anytime) → `memory/cards/main-editor-chrome--theme-demo-expansion.md` last.
