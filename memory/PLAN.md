...
- `orchestrator-tool-port` (FE-1107) — **substantially delivered by the parallel executor lane (FE-1089–FE-1125, landed on `next` 2026-07-06); needs re-baseline, not build.** The `execute_*` tool seam, ports architecture, and run driver exist; its scope card was consumed by that lane's ln-sync. Remaining residue before closing: (1) `src/agents/prompts/executor.md` still teaches "`orchestrator_stub` is the honest execution boundary" while 25 `execute_*` tools are live — prompt/conduct alignment is unbuilt; (2) confirm with KA which of his six remaining `memory/cards/executor-*` cards are truly open vs exhausted (all read as built, every acceptance box checked).
...
- **Grounding:** concentricity is materialized as code contract — `EXECUTOR_ALLOWED_TOOL_NAMES` (`src/agents/runtime/executor/active-tools.ts`) is composed from the live elicitor allowlist plus executor-only grants (`orchestrator_stub` and, since the KA executor lane landed, the `execute_*` tool chain), with the blocked-tool floor still applied. The CODE-side J5 switch records a `brunch.session_orientation` resolution and kicks the executor with a matching directive. Executor prompt/reference guidance now opens CODE mode with capability-readiness postures (Proceed / Proceed-advisory / Negotiate / Ask) from `src/agents/references/readiness-bands.md` §Agent Use; this frontier does not invent a new readiness model.
...
- **Status (re-baselined 2026-07-06):** substantially delivered externally — the KA executor lane (FE-1089–FE-1125, merged to `next`) built the tool seam this frontier was scoped to open: `src/executor/` pure core over injected `ExecutionPorts`, thin `.pi/extensions/agent-runtime/execute-*` adapters, `execute_plan_check`, the run driver, run-local promotion, and explicit-acceptance host promotion (D111-L, D112-L, I58-L). The former scope card was consumed by that lane's sync. Do **not** rebuild; remaining residue is reconciliation-shaped.
...
- **Remaining residue (confirm with KA before closing FE-1107):**
  - `src/agents/prompts/executor.md` still teaches "`orchestrator_stub` is the honest execution boundary" (with a conduct test pinning the phrase) while the `execute_*` chain is live — executor conduct must be realigned to the real boundary (host promotion behind explicit acceptance), and `orchestrator_stub` itself likely retires.
  - Six `memory/cards/executor-*` cards remain in-tree with all acceptance boxes checked; confirm they are exhausted, then garbage-collect them.
