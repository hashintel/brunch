...
- **Lane A — deterministic orientation:**
  1. `session-entry-orientation` ([FE-1134](https://linear.app/hash/issue/FE-1134/session-orientation-dialog-at-deterministic-junctures)) — active, verification closeout. The mechanism question is answered (deterministic product-owned dialog, not an exchange); all scoped product junctures/chrome slices are landed, including J5 mode-switch and the RPC timeout floor, but the full gate exposed automated boot/web-driver harnesses blocking behind J1 until they explicitly answer or degrade the dialog. Outstanding outer-loop walkthrough evidence for the generative menu options remains blocked on `walkthrough-batch-2` seed variants. Arc: `deterministic-orientation`.
  2. `execute-entry-readiness` ([FE-1137](https://linear.app/hash/issue/FE-1137/executor-entry-readiness-and-concentric-authority)) — stacks on `session-entry-orientation` once the dialog/juncture seam is stable. Arc: `deterministic-orientation`.
...
- **Status:** active, verification closeout; opened 2026-07-02 from walkthrough beat-5 findings; **superseded in place 2026-07-03 (grill)** — the mechanism question this frontier's first tracer was meant to answer ("deterministic kick chrome vs prompt-directed agent behavior") is decided: deterministic, product-owned. Card 1 is landed via option-2 J1 (`session_start(startup)` after UI binding); Card 2 is complete across J2/J3/J4/J5/J6 plus the RPC timeout floor; Cards 3–4 are landed. Full-gate verification exposed the remaining inner-loop gap: automated boot/web-driver harnesses must explicitly answer or degrade J1 rather than timing out behind the orientation dialog. Remaining outer-loop work is walkthrough evidence for the menu's propose/project options, blocked on `walkthrough-batch-2` seed variants.
- **Certainty:** mechanism `earned` (Pi affordances verified in docs: `ctx.ui.select` works in TUI + RPC via the extension-UI sub-protocol; `session_start` carries `reason: "startup" | "reload" | "new" | "resume" | "fork"`; `session_tree` fires after `/tree`; `pi.registerCommand` covers `/consult`); menu *content/conduct* still `proving` (which options at which junctures, threshold gating). Current scope card records boot-order corrections and the full decision-flow chart: `memory/cards/session-entry-orientation--slices.md`.
