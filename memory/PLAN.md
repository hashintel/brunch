...
- **Status:** active; decision-picker slice built 2026-07-03. `choice` / candidate-choice / review local-TUI paths now use Brunch-owned `ctx.ui.custom` bordered picker chrome, and `MultiChoicePickerComponent` is wrapped in the same bordered shell. Thread 1 (free-text answer editor) scoped 2026-07-06 after the walkthrough surfaced the visual inconsistency (raw `ctx.ui.editor` dialog vs bordered family); its former design unknowns are retired by the decision-picker slice, `brunch-editor.ts`'s rule-strip technique, and the preview harness's Theme→EditorTheme derivation.
- **Certainty:** proving.
...
- **Current execution pointer:** `memory/cards/exchange-answering-chrome--answer-editor.md` (thread 1, scoped 2026-07-06; ready for `ln-build`). The decision-picker scope card is consumed.
