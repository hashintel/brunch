...
| 4 | `poc-live-ship-gate` | FE-811 (existing) | done — runbook composed; evidence promoted 2026-06-22 |
...
- **Current execution pointer:** Active on `ln/fe-811-poc-live-ship-tie-off`. Thinness prelude (2026-06-22) settled the gate's shape: the public `--mode rpc` server cannot produce a deterministic agent turn (faux provider is in-process-only; `PI_OFFLINE` = no model, not a fake), so the gate is a **two-tier split** scoped in `memory/cards/poc-live-ship-gate--composition-gate-and-runbook.md` — Card 1 deterministic public-entry composition gate + anti-cheat import guard (CI), Card 2 live `--mode tui` runbook + durable artifact schema (manual/outer). Prior hardening (on `ln/fe-811-ship-gate-residue-and-mentions`): stale graph-snapshot/report residue regenerated to the graph-overview/workspace.state contract, the related-edge formatter labels non-anchor edges `lateral`, and the live mention autocomplete sources selected-spec graph nodes.
