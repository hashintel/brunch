...
- **Kind:** structural
- **Status:** active — slices 1 + 2 of 5 done (export reducer + identity-fold engine wiring + `--petrinaut-fold` cook CLI flag; one folding per cook run is now shared by `serializeBlueprint` and `createPetrinautEventStream`; engine-driven frame-replay oracle round-trips a real run through `reduceBrunchExecutionExport`). Next: slice 3 — ephemeral SSE server in the cook process. Bristol-demo integration path; supersedes the earlier static single-file bundle idea.
