...
- **Linear:** FE-894
- **Kind:** hardening
- **Status:** in progress (2026-06-24) on `ka/fe-894-cook-latency` — **replanted onto `main`** after the FE-1049 merge (main now carries the in-process pi harness + unified runner + epic-remediate, so the work lands cleanly off main; the prior FE-884-stacked branch is retired). **P0 telemetry + lever ① landed**; a real multi-slice cook run (~85 min, FE-1049 graph-canvas) showed **75% of wall-clock is *test-authoring*** rediscovery, not verify/caching — so ① (harness prior-art prime) is the headline lever, and the topology-guess ranking (P3-first) is demoted. ①'s wall-clock payoff is still unproven (A102-K). See **Measured findings** + **Build order** below.
