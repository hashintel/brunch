...
The next active work comes from the dramaturgical audit of the grounding interaction model: a hardening pass for six code bugs/gaps (duplicate answered cards, inflated readiness, term visibility, stale markers, divider width, sidebar sync), followed by the grounding free-text question-format change (D115) and phase section headers (D116). After those, the existing near horizon continues around completion/closure surfaces, grounding/context-gathering capability, workflow ownership cleanup, and router/query refinement. Revisit/cascade and infrastructure/tooling remain on the true horizon.
...
1. **Dramaturgical audit bug and gap fixes** — hardening
   ...
   - Why now / unlocks: these are live bugs visible to users on every fresh specification; the duplicate-card and readiness issues erode transcript and workflow trust. All six are small, independent of the retired naming frontier, and can land immediately.
...
Near-horizon work is grouped by theme: grounding interaction-model fixes, then completion/closure surfaces, then grounding/context-gathering capability, then workflow ownership cleanup, then router/query refinement.

### Grounding interaction model

1. **Grounding free-text question format** — bounded feature
...
2. **Phase section headers** — bounded feature
...
3. **Output route and markdown export refinement** — first completion-surface follow-on once naming settles.
...
4. **Close Phase confirmation modal** — complete the remaining phase-exit UX after export/output cleanup.
...
8. **Workflow ownership extraction** — architectural cleanup after the user-visible completion and grounding seams land.
...
9. **Continuous workspace / phase-addressable interview surface** — user-facing continuity pass after workflow ownership is clearer.
...
10. **Router / query ownership refinement for interview surfaces** — final near-horizon cleanup after workflow ownership and workspace continuity are clearer.
...
- [2026-04-20] Alias deletion retired the naming frontier — Done: removed `/api/projects/...` compatibility entry points, deleted shared/server project-wrapper aliases, collapsed canonical transport and controller state to `specification` ownership, and confirmed `src/` no longer carries `project_id` or project-shaped compatibility payloads on the happy path. Verified: `npm run verify`. Watch: manual freshly reseeded resume/export walkthrough still matters after the destructive cut.
- [2026-04-20] Durable `specification` record identity landed under the naming frontier — Done: schema/migration ownership, DB helpers, shared transport contracts, state/entity payloads, and verification now treat `specification` / `specification_id` as the canonical durable identity instead of `project` / `project_id`. Verified: `npm run verify`. Watch: resolved by the subsequent alias-deletion pass; only the manual resume/export walkthrough remains.
...
```text
dramaturgical-audit-bug-and-gap-fixes  (active; no blockers)
grounding-free-text-question-format    (no blockers)
phase-section-headers                  (no blockers)
output-route-and-markdown-export-refinement  (no blockers)
