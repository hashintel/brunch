...
- [2026-05-08] **Side-chat V3.1 slice 4 — reconciliation classifier (schema + run-agent route)** (FE-674) — added three nullable agent_* columns on `reconciliation_need` (migration 0019); pure `classifyNeed()` over a stubbed-or-live AI SDK adapter; `POST /api/specifications/:id/reconciliation-needs/run-agent` walks every awaiting open need through the lifecycle `null → queued → classifying → classified | failed`; new `reconciliation-classifier.md` prompt asset registered in the prompt-loader; listing endpoint now exposes the three classifier columns with null defaults. SPEC.md gains I114 (lifecycle + label vocabulary + structural recoverability) and the `planned I114` references in §Verification Design swap to live I114 + name the seed corpus location at `src/server/__corpus__/reconciliation-classifier-seeds.json`. Verified: `npm run verify` — 1126 passed (+17 net new tests across schema/agent/route/wire-shape suites); 2 inherited `structured-list-view.test.tsx` failures unchanged from cascade-polish baseline. Watch: A88 stays open (slice 6 outer-loop walkthrough validates it); single-shot LLM call + in-process loop are deliberate MVP choices with documented promotion triggers; middle-loop golden-corpus runner is its own slice.
