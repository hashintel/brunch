...
- **Status:** built 2026-07-06; pending physical-terminal smoke before arc closeout. `choice` / candidate-choice / review local-TUI paths use Brunch-owned `ctx.ui.custom` bordered picker chrome, and `MultiChoicePickerComponent` is wrapped in the same bordered shell. Free-text `answer` now uses `ExchangeAnswerEditorComponent` via `ctx.ui.custom` with the sealed `ctx.ui.editor` fallback and the existing headless broker third; empty/whitespace free-text answers are rejected at the TUI and RPC/schema boundaries.
- **Certainty:** proving.
...
- **Acceptance:** code acceptance is closed for both threads; manual physical-terminal smoke for the `answer` editor remains the only frontier residue.
