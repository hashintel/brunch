...
- **Current execution pointer:** Web transport/client-attachment hardening is complete: shared JSON-RPC protocol helpers, `ws`-backed `/rpc` transport, persistent browser RPC client, canonical built asset serving, stable React runtime, and explicit read-only session projection by durable session id. Explicit session projection now resolves through a read-side projection reader that validates durable `brunch.session_binding` without opening or mutating workspace default session state, and stdio/WebSocket transports now share JSON-RPC parse/internal-error dispatch semantics. Continue the scoped hardening queue, then render the selected/explicit session transcript in the browser while keeping web reads explicit/read-only and deferring browser input until write ownership is designed.
