...
- ~~**merge deep-copies a clobbered node_modules → ENOSPC**~~ — **resolved (2026-06-18)**: an in-slice `npm install` replaces the shared `node_modules` symlink with a real tree, which `walkFiles` then deep-copied per-slice + per-epic until the disk filled. Fixed by skipping `SHAREABLE_TOP_LEVEL_ENTRIES` by name (not just by symlink-ness) in `epic-sandbox-merge.ts`; re-link via `linkSharedTopLevelEntries` unchanged. **Residual gap (open):** deps a slice installs into its own `node_modules` are therefore *not* reflected in the epic verify tree (which re-links the parent's install) — slice-installed new deps won't resolve under verify-epic. Correct fix = merge `package.json`/lockfile and reinstall into the shared store (the "reconstruct derived from source" half of the deferred git-merge remodel). Run-dir GC remains operator-owned (`worktree.ts`).
