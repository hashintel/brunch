...
- [2026-04-30] FE-650 streamed question cache promotion — interviewer streams now emit a post-finalize `frontier-turn-ready` event, and the client promotes the acknowledged question into the specification bundle query cache before refetch reconciliation. Verified: `npm run verify`. Watch: still needs a manual browser timing walkthrough against a real streamed turn to judge the former inert-card gap under actual latency.
