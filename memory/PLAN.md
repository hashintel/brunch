...
- **Members:** `shared-session-host-tracer` ✓ → FE-1348 `post-hardening-alpha-validation` A51-L row ✓ (the frontier's other rows are product-wide) → `shared-session-host-cutover` (wait-gated on the remaining relevant FE-1348 evidence).
...
- `post-hardening-alpha-validation` ([FE-1348](https://linear.app/hash/issue/FE-1348/validate-current-brunch-usage-and-testing-paths)) — **active, evidence-gated coverage frontier:** validate the frozen current alpha entry/usage/verification inventory through real source, product, dev, comparison-gate, and installed-package paths; disposition incidental audit findings without implementing them in the sweep. A51-L's colleague walkthrough is complete; four product rows remain evidence-gated. Definition and active sweep ledger below.
...
- `shared-session-host-cutover` — tracer and A51-L are complete; remain wait-gated on FE-1348's standalone-web and cross-surface settlement evidence, then close the enumerated shared-contract surface and delete the raw sidecar relay and `/rpc/driver` without forcing physical host unification. Definition below.
...
- **Current execution pointer:** [`standalone-web reload and cleanup repair`](cards/post-hardening-alpha-validation--standalone-web-reload-cleanup.md) (`Mode: slices`), promoted from SW1 before the standalone-web row reruns.
...
- **Classification:** tracer and A51-L complete; wait-gated on FE-1348's standalone-web and cross-surface settlement rows.
...
active:
  post-hardening-alpha-validation (FE-1348)
    ...
    open: execute interaction | standalone web | stdio RPC | cross-surface settlement
    ...
    relevant_rows: standalone web + cross-surface settlement -[hard]-> shared-session-host-cutover
