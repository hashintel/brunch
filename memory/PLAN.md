...
- **Current execution pointer (2026-07-14 walkthrough pause):** Session B beats 1–3 promoted R5–R13 and the [`consolidated outer checkpoint`](cards/walkthrough-remediation-2--consolidated-outer-checkpoint.md) remains paused. R5 is closed by commit `daba4cda` plus live transient-notification evidence; R7 by `b882d70f` plus live controls-only continuation evidence; R11 by `1343a7c4`, composed-prompt/carrier oracles, and authenticated live confidentiality/disclosure evidence. R8–R10 completed `ln-design`/`ln-spec`: D27-L/D38-L/D99-L/D106-L/D110-L/D116-L now specify conversational digest feedback → one bounded questionnaire/confirmation capture terminal → direct advisory mapping → optional settled-only proposition-shaped review, with compact inspection and one shared TUI/RPC atomic acceptance seam. Next route those three findings through `ln-oracles`, then `ln-scope` to the deterministic ingest-choreography tracer. Unresolved separate routes remain R6 four-state result visuals through `ln-design`, R12 through `ln-diagnose`, and R13 through `ln-disambiguate`/`ln-spec`. Resume the checkpoint only after these promoted routes are dispositioned.
- **Remaining findings inventory:** R6/R12/R13 remain contract-unresolved; R8/R9/R10 are contract-settled with oracle design + scope/build pending; remaining consolidated LN outer evidence; O7/O8/O9 live D120-L Execute workflows (KA-coordinated).
