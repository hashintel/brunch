...
- **Kind:** structural
- **Status:** active — slices 1 + 2 + 3a + 3b of 5 done (export reducer + identity-fold engine wiring + `--petrinaut-fold` cook CLI flag + in-process stream bus with replay buffer + ephemeral HTTP/SSE server on localhost). Next: slice 4 — wire the 3b server into `runCook` behind `--petrinaut-stream`, multi-tier base-URL resolution, auto-open Petrinaut launcher URL. Bristol-demo integration path; supersedes the earlier static single-file bundle idea.
