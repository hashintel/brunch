...
- `petri-execution-parity` ([FE-1195](https://linear.app/hash/issue/FE-1195/petri-execution-parity-isolated-slice-fan-in-and-epic-integration)) — reopened on PR #325 after `ln-review`: harden run/epic effect authority, close topology/readiness edge cases, make production epic authoring and observers load-bearing, and consolidate duplicated serial/parallel lifecycle mechanics. Scope: [`memory/cards/petri-execution-parity--review-remediation.md`](cards/petri-execution-parity--review-remediation.md).
...
- **Final-finding closure (2026-07-13):** every drive dispatch now requires a readable prepared journal and full durable transition parity with lifecycle projection; serial lifecycle-ahead gaps halt without repeating effects, while active parallel and journal-backed transitioned epic claims retain explicit recovery authority. Epic marking alone cannot manufacture verification. The production execute-tool classifier now pins every semantic mapping and treats `execute_launch` as read-only.
- **Current execution pointer:** [`memory/cards/petri-execution-parity--review-remediation.md`](cards/petri-execution-parity--review-remediation.md) — all review-remediation slices complete, including final-finding closure Slice 9.
