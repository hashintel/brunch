...
- **Current execution pointer:** Web transport/client-attachment hardening is complete: shared JSON-RPC protocol helpers, `ws`-backed `/rpc` transport, persistent browser RPC client, canonical built asset serving, stable React runtime, and explicit read-only session projection by durable session id. Next scope should render the selected/explicit session transcript in the browser while keeping web reads explicit/read-only and deferring browser input until write ownership is designed.
