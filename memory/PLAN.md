...
- **Live scope files:** [`structured ask + TUI-only interaction witness`](cards/shared-session-host-tracer--structured-ask-witness.md) — scoped 2026-08-07, buildable. It carries the observe-only ask-announcement design and its rejected alternatives. The rival-refusal and shutdown/reopen slices stay unscoped and must follow it sequentially: both need the same shared PTY choreography module as a primary write target, so they are not write-disjoint from it.
...
active:
  shared-session-host-tracer (FE-1321)
    ...
    next: build memory/cards/shared-session-host-tracer--structured-ask-witness.md
          (structured ask + TUI-only interaction over the shared PTY choreography module),
          then rival refusal, then shutdown/reopen
    finding: the ask extension resolves collectors UI-first, so a real-TUI ask never reaches
             D125-L's live registry and no ask_opened delta currently reaches companion React;
             the scoped slice closes that with an observe-only announcement channel
