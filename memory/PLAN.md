...
- **Status:** first slice landed 2026-07-13 — `attempt_failed` journal facts (non-marking; replay/export skip them), bounded in-run agent retry with `run.json` attempt counter, HITL retry resets the bound, Petrinaut frame contract unchanged. Remaining before frontier close: `test_run_failed` coverage decision and the Petri-native attempt topology decision (build or explicitly defer to the fan-in frontier).
