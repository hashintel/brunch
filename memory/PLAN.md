...
- 2026-07-14 `petri-execution-parity` (FE-1195) — completed the old-`main` live-parity closure: timestamped firing wire, structural pass/fail branches with complete failed-slice terminal evidence, stale serial-state retirement at parallel admission, restart-stable terminal authority, and a view-only Petrinaut projection with mechanically pruned isolated places, preserved connected IDs/arcs, contextual labels, and compact deterministic execution bands. Raw executor topology/markings and SSE firing order remain unchanged; manual Rust fixture comparison remains pre-PR outer evidence.
