...
Prerequisite contract (parallel track, coordinated with the unpublished pi-harness thread): `agent-extension-host` — the pi harness as a dual-mode (`elicit`/`execute`) extension host; cook capabilities register as `execute`-mode plugins. Gates only the dispatch-seam frontiers below (`integration-oracle`); the rest do not depend on it. Sits over the FE-841 core, not in the linear FE-843 cook stack.

1. `brunch-detect` — read the project toolchain from the repo; brownfield-only front of the FE-843 resolution chain. First branch off FE-843. *(seam-independent)*
...
- **Linear:** unassigned (create on start) · coordinated with the unpublished pi-harness thread
- **Kind:** structural (shared contract / serialization point)
- **Status:** not-started (drafted 2026-06-15) — the contract both this work and the unpublished pi-harness thread target; the pi thread owns the core implementation.
...
- **Depends on:** FE-841 (in-process pi core), FE-813 (tool-scoping), FE-853 (confinement); coordinated with the unpublished pi-harness thread (owns the core). Sits over the FE-841 core, parallel to the linear FE-843 cook stack. Gates `integration-oracle`, `interactive-recovery`, `adaptive-replan`.
