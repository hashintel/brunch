...
| A7-L `framing_as` modality | Product framings need a node-shape carrier that `framing_as` cannot express. | Phase 2 node lock (see `docs/design/GRAPH_MODEL.md` §"Nodes — deferred") revisits the carrier itself (`framing_as` vs. product-theses nodes vs. first-class kinds); the prior "promote on relation-policy pressure" path is retired with the named-relation catalogue. |
...
| A14-L review-set structural legality | LLMs cannot produce dry-run-valid entity drafts and category-typed edge drafts (per `docs/design/GRAPH_MODEL.md`) reliably enough. | M5 must measure structural-legality rate and retry/fallback behavior before depending on proposal-heavy UX. |
...
1. `sealed-pi-profile-runtime-state` — In-flight prep envelope on `ln/fe-776-graph-layer-prep-profile`. Two strands: **(a)** Pi harness sealing (profile boundary, in-memory settings policy, transcript-backed runtime-bundle state) — substantially landed; remaining: best-effort lifecycle-generated session display names over Pi `session_info`. **(b)** Graph-model lock-and-materialize — Phase 1 (edges) landed (canonical `docs/design/GRAPH_MODEL.md` + stubs under `src/graph/` + SPEC/PLAN reconciled via D51-L); remaining: Phase 2 (nodes) lock + materialize, then the A20-L Drizzle 1.0-beta + `drizzle-orm/typebox` + `better-sqlite3` + Pi `registerTool` round-trip spike. Tie off when both strands are at acceptance so `graph-data-plane` CRUD can start on settled foundations.

### Next

1. `graph-data-plane` — M4 CRUD layer over the prep-envelope-locked graph model: `CommandExecutor` result contract, one-transaction LSN/change-log skeleton, durable intent-plane node/edge CRUD per `docs/design/GRAPH_MODEL.md`, reconciliation-need substrate, oracle-plane stub tables — all forward-compatible with oracle, design, and plan planes.
2. `agent-graph-integration` — M5. Graph tools, synchronous elicitor capture, review-set acceptance, and reviewer advisory writes through pi extension seams; all writes via the shared command layer.
...
- **Acceptance:**
  ...
  - **Graph-model strand:** Phase 1 edge contract locked in `docs/design/GRAPH_MODEL.md` and materialized as type/policy stubs under `src/graph/` (✓ landed at commit `100585a1`). Phase 2 node contract locked similarly (per-plane node kinds, decision-as-node shape, framing/theses carrier resolution, node-side `provenance` and `basis` symmetry with edges) and materialized as `src/graph/schema/nodes.ts` (+ peers); the prior named-relation catalogue and brainstormed edge taxonomy are retired from active docs. A20-L spike produces a verdict on Drizzle 1.0-beta + `drizzle-orm/typebox` + `better-sqlite3` + Pi `registerTool` round-trip over one representative intent-plane table (or documents the 0.x fallback path of standalone `drizzle-typebox` + `drizzle-orm/typebox-legacy`).
- **Verification:** Inner — profile/runtimestate unit tests, prompt-composition snapshot tests, tool-policy contract tests, edge/node schema unit tests, category-policy unit tests. Middle — ambient `.pi/` fixture/audit tests proving disabled discovery and sealed settings; explicit Brunch resource-injection test proving extension factories may inject Brunch-owned skills/prompts despite ambient `noSkills`/`noPromptTemplates`; JSONL reload/projection tests for runtime init/switch entries; before-agent-start/tool-call policy tests for `elicit`; Drizzle round-trip test landing one table, one insert/select cycle, one Pi `registerTool` parameter binding; I26-L grep-based architectural test wired alongside the first Drizzle import so the single-schema-vocabulary boundary stays enforced. Outer — manual TUI/RPC smoke that active role/lens/strategy changes are inspectable in transcript and reflected in prompt/tool posture rather than hidden UI state.
- **Cross-cutting obligations:** Do not expose Pi's generic extension/skill/prompt/theme configuration to Brunch users; do not make Pi skills the primary authority for core operational prompts; keep raw Pi RPC behind Brunch adapters; keep runtime state linear-transcript-backed and compatible with compaction/session-boundary lifecycle hooks (`session_start`, `resources_discover`, `before_agent_start`, `context`, `tool_call`, `session_before_switch`, `session_before_compact`, `session_shutdown`). Graph-model lock work must trace to `docs/design/GRAPH_MODEL.md`; node lock must preserve the closed-edge-set invariants (immutable accepted-edge identity, `dependency`-only auto-cascade, separate `ReconciliationNeed` substrate with `{kind:'edge'|'node_pair'}` target). Drizzle spike is throwaway scope — one table, one tool, no broad imports until the verdict lands; if the beta line blocks (migrations, SQLite fidelity, schema-derivation bugs), document the 0.x fallback and continue without re-opening the version question.
- **Traceability:** R25, R26 / D2-L, D16-L, D23-L, D39-L, D40-L, D41-L, D51-L / I24-L, I25-L, I26-L / A19-L, A20-L
- **Design docs:** [GRAPH_MODEL.md](file:///Users/lunelson/Code/hashintel/brunch-next/docs/design/GRAPH_MODEL.md) (canonical graph contract; Phase 1 locked, Phase 2 nodes pending), [pi-seam-extensions.md](file:///Users/lunelson/Code/hashintel/brunch-next/docs/architecture/pi-seam-extensions.md), [pi-ui-extension-patterns.md](file:///Users/lunelson/Code/hashintel/brunch-next/docs/architecture/pi-ui-extension-patterns.md)
- **Current execution pointer:**
  - **Sealing strand:** profile resource/settings/runtime slices have landed a `src/brunch-pi-profile.ts` boundary used by TUI launch, preserving `noContextFiles`/`noExtensions`/`noPromptTemplates`/`noSkills`/`noThemes`, explicit Brunch extension factories, quiet startup, offline default, and an in-memory Brunch settings policy that ignores hostile ambient global/project `.pi/settings.json` before and after reload. Runtime-state transcript helpers append/project/switch full `brunch.agent_runtime_state` snapshots, reject impossible mode-role-strategy-lens combinations including after real Pi JSONL reload, and drive session-start/before-agent-start prompt/tool posture from transcript projection without extension-local memory. Remaining: best-effort lifecycle-generated session display names over Pi `session_info`.
  - **Graph-model strand:** Phase 1 edge lock landed at commit `100585a1` (canonical doc `docs/design/GRAPH_MODEL.md`; stubs under `src/graph/atoms.ts`, `index.ts`, `schema/edges.ts`, `schema/reconciliation-need.ts`, `policy/category-policy.ts`; SPEC/PLAN reconciled via D51-L at commit `23a97f34`). Remaining: **(i)** Phase 2 node lock — per-plane node kinds, decision-as-node shape, `framing_as` vs. product-theses carrier resolution, node-side `provenance` and `basis` symmetry with edges — materialized as `src/graph/schema/nodes.ts` and reflected in `GRAPH_MODEL.md` §"Nodes" plus SPEC if a new D-L is warranted; **(ii)** A20-L Drizzle 1.0-beta + `drizzle-orm/typebox` + `better-sqlite3` + Pi `registerTool` round-trip spike over one representative intent-plane table (throwaway scope — verdict only).
  - **Tie-off:** when both strands reach acceptance, mark the frontier done and let `graph-data-plane` (M4 CRUD) start with settled persistence/schema-derivation/graph-contract foundations.
...
- **Kind:** structural
- **Status:** next / blocked on `sealed-pi-profile-runtime-state` prep envelope tie-off (Phase 2 node lock + A20-L Drizzle spike)
...
- **Why now / unlocks:** Pins I1-L, I6-L. Unlocks all agent ↔ graph work (M5+) and lets oracle / design / plan planes be added later without re-foundation. The graph contract and persistence toolchain are settled by the prep envelope, so M4 is pure CRUD/transaction/CommandExecutor work rather than mixed design-and-mechanics.
- **Acceptance:** Graph CRUD + change-log replay tests pass through the `CommandExecutor` public mutation boundary; command results already include success, `needs_human`, `policy_blocked`, `version_conflict`, and `structural_illegal` shapes even if pre-M6 policy classification is minimal; reconciliation-need substrate accepts inserts/updates/resolutions with LSN invariants enforced; oracle-plane stub tables exist (Check, Validation Method, Evidence, Obligation) even if unused; the persistence layer proves the one-transaction protocol that couples authority/result classification, version checks, structural validation (including the closed edge-category set and immutable accepted-edge identity per D51-L), LSN allocation, change-log append, and any coherence updates.
- **Verification:** Inner gate plus command/result schema/type tests. Middle — property/model-based tests on LSN monotonicity, graph replay, reconciliation invariants (target shape `{kind:'edge'|'node_pair'}`), framing matrix, edge structural legality (closed category set, stance scoping, supersession acyclicity), and `CommandExecutor` transaction/result behavior; architectural no-bypass tests. Outer — fixture property invariants on reconciliation-substrate begin running.
- **Cross-cutting obligations:** Reuse the Drizzle + `better-sqlite3` persistence shape settled by the prep-envelope A20-L spike; do not re-open the version pin. `CommandExecutor` result contract and no-bypass transaction rule become shared infrastructure for later direct-agent, elicitor-capture, deferred observer/auditor, side-task, migration, and UI-attributed writes. Derive row/insert/update runtime schemas from Drizzle table definitions via the TypeBox path chosen during the spike — do not hand-author parallel row schemas. The I26-L grep-based architectural test should already be live from the prep envelope; M4 widens its coverage as new Drizzle imports land.
- **Traceability:** R7, R9, R13 / D3-L, D4-L, D6-L, D8-L, D9-L, D16-L, D20-L, D41-L, D51-L / I1-L, I6-L, I7-L, I11-L, I26-L / A3-L, A4-L
...
- **Current execution pointer:** Blocked on prep-envelope tie-off. When unblocked, scope the narrow `CommandExecutor` result contract and one-transaction LSN/change-log skeleton first, then widen to intent-plane node/edge CRUD against the locked schema, then add the reconciliation-need substrate and oracle-plane stub tables. Keep M4 thin enough to falsify A3-L / A4-L / A6-L / A8-L before widening CRUD or coherence homes.
...
- 2026-06-01 `pi-ui-extension-patterns` (FE-744) — Done: implementation-complete and tied off. All Pi extension seam evidence for M5/M6/M7 has landed — command-containment + lifecycle effect blocking, dynamic Brunch-owned chrome through one wrapper, hierarchical spec/session picker (startup + in-session) with `WorkspaceSessionCoordinator` ownership, RPC/headless initial-selection contract, pty/ANSI-stripped startup oracle, structured-exchange schema/builder + Zod-authored present/request/capture details schemas under `src/tui-client/.pi/extensions/structured-exchange/schemas/`, registered sequential `present_*`/`request_*` tools, JSON-editor RPC fallback, public Brunch JSON-RPC structured-exchange parity (`rpc.discover`, `workspace.*`, `session.*`, `elicitation.*`) over the current deterministic permutation set with committed probe artifacts under `.fixtures/runs/public-rpc-parity/`, web real-time observation of RPC-originated transcript updates, private code-composed prompt-pack topology under `src/tui-client/.pi/context/`, shared Brunch identity primitives, and branded persistent + startup chrome. Strict built-in command suppression (A18-L) carried forward to the prep envelope as a Pi API residue. Verified across the frontier through `npm run verify`, deterministic public-RPC parity probe runs, pty startup oracle, contract tests, and manual TUI smoke. Detailed frontier definition archived to [docs/archive/PLAN_HISTORY.md §2026-06-01 Sync archive](file:///Users/lunelson/Code/hashintel/brunch-next/docs/archive/PLAN_HISTORY.md).
- 2026-05-31 `graph-data-plane` (Phase 1 — edge lock-and-materialize) — Done: locked the closed eight-category edge model (`dependency`, `proof`, `support`, `realization`, `boundary`, `composition`, `association`, `supersession`) with `stance` only on proof/support, `basis ∈ explicit | accepted_review_set` (no `inferred`), no edge `status` field, immutable accepted-edge identity (`category`, `sourceId`, `targetId`, `stance`), dependency-only auto-cascade, and a separate `ReconciliationNeed` substrate whose target is `{kind:'edge'} | {kind:'node_pair'}`. Materialized as canonical doc [`docs/design/GRAPH_MODEL.md`](file:///Users/lunelson/Code/hashintel/brunch-next/docs/design/GRAPH_MODEL.md) plus type/policy stubs under `src/graph/` (`atoms.ts`, `index.ts`, `schema/edges.ts`, `schema/reconciliation-need.ts`, `policy/category-policy.ts`); retired the prior named-relation catalogue in `docs/architecture/pi-seam-extensions.md` §"Edge types" and archived the source brainstorm at `archive/docs/design/GRAPH_EDGE_CATEGORIES.md` with a superseded preface. Captured as D51-L in SPEC.md and reflected in D7/D8/D9/D27/I6/lexicon. Phase 2 (per-plane node kinds, decision shape, framing/theses carrier, node-side provenance/basis) is deferred and tracked in GRAPH_MODEL.md §"Nodes — deferred". Verified at commit `100585a1`: `npm run verify` (check + test + build) passed.
- 2026-05-22 `web-shell` — Done: M3 now serves the native React web shell over one persistent WebSocket RPC client, blocks/adjudicates branchy transcript shapes for session-consuming reads, serves only static HTTP assets (no REST product reads), projects explicit durable sessions through a canonical Brunch session-envelope reader, renders assistant/user/prompt transcript rows, and keeps browser state as a read-only client attachment rather than a durable session. Verified: `npm run verify` after each slice plus direct host/WebSocket smoke for static HTML, missing REST product reads, explicit `{ sessionId, specId }` projections, transcript display, and exchange projection.

Older history (including `jsonl-session-viability`, `mode-shell-and-fixture-driver`, `walking-skeleton`): `docs/archive/PLAN_HISTORY.md`

## Dependencies

```text
nodes:
  sealed-pi-profile-runtime-state   [active · in-progress]   (M4 prep envelope: sealing + graph-model lock)
  graph-data-plane                  [next · blocked-on-prep] (M4 CRUD proper)
