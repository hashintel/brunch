...
- **Status:** implementation and review hardening complete on draft PR #322. Run creation freezes one plan and materializes attachable observer artifacts; live delivery uses subscribe-before-snapshot catch-up, journal-ordered completion, metadata-only abandonment wake-ups, single-flight refresh, reconnect equivalence, and host-shutdown cleanup. `npm run verify` passed after review fixes; review/merge pending.
