...
- **Current execution pointer:** the profile resource/settings slices have landed a `src/brunch-pi-profile.ts` boundary used by TUI launch, preserving `noContextFiles`/`noExtensions`/`noPromptTemplates`/`noSkills`/`noThemes`, explicit Brunch extension factories, quiet startup, offline default, and an in-memory Brunch settings policy that ignores hostile ambient global/project `.pi/settings.json` before and after reload. Runtime-state transcript helpers now append/project/switch full `brunch.agent_runtime_state` snapshots and reject impossible mode-role-strategy-lens combinations, including after real Pi JSONL reload. Next build the prompt/tool posture wiring so session-start and before-agent-start derive active tools and Brunch prompt material from the transcript-projected state. Follow-up slices should add any best-effort lifecycle-generated session display names over Pi `session_info`.
