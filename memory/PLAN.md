...
- 2026-04-18 — **Review-authoritative requirement and criterion capture now stays draft-only until acceptance** — `src/server/observer.ts` now keeps requirements and criteria in `data-observer-result` draft review inputs instead of pre-acceptance `knowledge_item` rows, `src/server/db.ts` now derives draft review inventory from those turn-owned observer artifacts, and `src/server/interview.ts` / `src/server/app.ts` now synthesize runtime `data-review-set` payloads from that draft inventory so durable requirement / criterion entities first appear only when accepted review materializes them. `src/server/observer.test.ts`, `src/server/app.test.ts`, `src/server/parts.test.ts`, and `src/server/fixtures/corpus.ts` now cover the draft-vs-durable contract across observer transport, replay, runtime review-set synthesis, and corpus probes. Done: `npm run verify`. Watch: Active item 2 may still want a follow-on cut that moves the *initial* review-set authority fully onto the interviewer review-generation seam instead of relying on synthesized fallback inventory.
- 2026-04-18 — **Runtime review turns now persist an explicit review-set payload derived from the current review inventory** — `src/server/interview.ts` can now synthesize `data-review-set` payloads for requirements and criteria from the current review inventory, and `src/server/app.ts` now persists that synthesized review set onto runtime review turns even when the model only emits the structured question itself. `src/server/app.test.ts` and `src/server/interview.test.ts` now prove runtime review turns carry the review-set payload and that the payload shape stays aligned across requirements and criteria. Done: `npm run test -- src/server/app.test.ts src/server/interview.test.ts`. Watch: Active item 2 still needs observer-side pre-acceptance persistence removal so these review sets stop depending on pre-accepted project-wide requirement / criterion entities.
- 2026-04-18 — **Accepted review now materializes only the persisted review-set items instead of blanket-accepting all project-wide review entities** — `src/server/db.ts` can now materialize requirement / criterion items from a persisted `data-review-set`, reusing matching existing rows when present, and `src/server/app.ts` now prefers that accepted-review path over blanket-linking every project-wide requirement / criterion row. `src/server/app.test.ts` now proves accepted review carries forward only the review-set items on the active path while leaving unrelated project-wide items unaccepted. Done: `npm run test -- src/server/app.test.ts src/server/db.test.ts`. Watch: Active item 2 still needs observer-side pre-acceptance persistence removal plus draft review-set synthesis before the ontology lane is retired.
- 2026-04-18 — **Requirement and criterion durability authority is now explicit in shared code** — `src/shared/knowledge.ts` now codifies the intended lifecycle boundary as a first-class durability policy: exploration kinds stay observer-captured, while `requirement` and `criterion` are review-authoritative and tied to their accepted review phases. `src/shared/knowledge.test.ts` now proves that contract directly, so the upcoming behavior change has one shared source of truth instead of an implicit reading of the spec. Done: `npm run verify`. Watch: Active item 2 still needs the actual persistence-behavior cutover and follow-on test sweep before the ontology lane is retired.
- 2026-04-18 — **Observer prompt and output schema now derive from shared ontology policy** — `src/server/observer.ts` now builds its output schema from the canonical knowledge registry instead of restating every collection by hand, and its phase bias / kind-semantics prompt text now derives from the shared observer ontology policy and per-kind semantic-role map rather than a second drift-prone ontology description. Done: `npm run verify`. Watch: Active item 2 still needs the explicit requirement/criterion durability decision and the resulting behavior change / test sweep before the ontology lane is retired.
- 2026-04-18 — **Shared observer ontology policy now declares phase-valid kinds and kind semantics** — `src/shared/knowledge.ts` now exports one phase-by-phase observer ontology policy plus per-kind semantic-role text, and `src/shared/knowledge.test.ts` proves the observer-facing contract covers every canonical kind and explicitly encodes the current scope/design/review phase allowances in one shared place. Done: `npm run verify`. Watch: Active item 2 still needs the observer prompt/schema rebuild plus the explicit requirement/criterion durability decision before the ontology lane is actually retired.
- 2026-04-18 — **Story, fixture, and demo samples now teach the canonical reference-code contract** — seeded review-set fixture scenarios and the chat/review/turn-lifecycle story fixtures now derive visible knowledge codes from `createKnowledgeReferenceCode()`, so non-runtime surfaces show the same `G` / `CTX` / `CON` / `R` / `AC` / `D` / `A` scheme as runtime and test code. Done: `npm run verify`. Watch: Active item 2 now moves on to the remaining observer-ontology-policy and requirement/criterion durability cleanup; reference-code drift from stale sample literals is no longer the blocker.
- 2026-04-18 — **Canonical reference-code prefixes now emit the intended short-form contract** — the shared knowledge registry now emits `G` / `T` / `CTX` / `CON` / `R` / `AC` / `D` / `A`, `knowledge-card` fallback badges now consume the shared prefix map instead of a drift-prone local copy, and the contract tests now prove the new visible codes end-to-end through the shared helper. Done: `npm run verify`. Watch: Active item 2 still needs the remaining story/demo/fixture sample normalization so non-test surfaces stop teaching the retired codes.
- 2026-04-18 — **Reference-code test fixtures now derive from the shared generator instead of freezing literals** — high-churn UI/server/shared tests now call `createKnowledgeReferenceCode()` for incidental requirement, criterion, goal, context, decision, and assumption codes, so the next contract-prefix change is isolated to the shared registry and a few true boundary assertions instead of a repo-wide literal sweep. Done: `npm run verify`. Watch: Active item 2 still needs the actual contract-prefix cutover plus story/demo normalization before the reference-code lane is retired.
