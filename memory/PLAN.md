...
- **Current execution pointer:** Slice 1 done 2026-06-09. The real CLI now parses `--cwd`, `BRUNCH_DEV` is the sole dev gate for CLI dev RPC and TUI introspection registration, the TUI owns a dev introspection store and passes the read-only tap only when opted in, and the offline default/lift is save/restore-scoped. The consumed scope file `memory/cards/dx-introspection-live--tui-dev-gate-wiring.md` was deleted. Conversational self-report (`tool_call`/`tool_result` tap + structured self-report + in-chat-vs-artifact surface, the open A26-L unknown) remains slice 2, scoped after this lands.
