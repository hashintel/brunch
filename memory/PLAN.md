...
- **Members:** `shared-session-host-tracer` (inner contracts, production wiring, and the production PTY boot/turn witness built; companion, exchange, rival, and lifecycle witnesses remain) → `shared-session-host-cutover` (wait-gated).
...
active:
  shared-session-host-tracer (FE-1321)
    ...
    next: build memory/cards/shared-session-host-tracer--authority-transfer.md in order —
          slice 1 rival refusal, then slice 2 shutdown/reopen; one shared PTY journey, so
          they are sequential on session-runtime-contract-pty-journey.ts, not parallel
