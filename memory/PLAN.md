...
- **Why now / unlocks:** FE-744 proved multiple Pi extension seams and exposed the exact weak point: ambient resource discovery and settings policy had to be sealed before future `elicit` vs `execute` work could depend on product-owned prompt/tool posture. The Brunch profile now isolates settings through an in-memory policy, but runtime bundle state still needs to be transcript-backed rather than hidden extension memory. This frontier de-risks M5/M6/M7 before graph tools, capture/reviewer jobs, and authority gating depend on the embedded harness.
...
- **Current execution pointer:** the profile resource/settings/runtime slices have landed a `src/brunch-pi-profile.ts` boundary used by TUI launch, preserving `noContextFiles`/`noExtensions`/`noPromptTemplates`/`noSkills`/`noThemes`, explicit Brunch extension factories, quiet startup, offline default, and an in-memory Brunch settings policy that ignores hostile ambient global/project `.pi/settings.json` before and after reload. Runtime-state transcript helpers append/project/switch full `brunch.agent_runtime_state` snapshots, reject impossible mode-role-strategy-lens combinations including after real Pi JSONL reload, and drive session-start/before-agent-start prompt/tool posture from transcript projection without extension-local memory. Follow-up work should add any best-effort lifecycle-generated session display names over Pi `session_info` and then reassess whether `sealed-pi-profile-runtime-state` is ready to tie off before `graph-data-plane` resumes.
...
- **Traceability:** R7, R9, R13 / D3-L, D4-L, D6-L, D8-L, D9-L, D16-L, D20-L, D41-L, D51-L / I1-L, I6-L, I7-L, I11-L, I26-L / A3-L, A4-L, A20-L
- **Design docs:** [GRAPH_MODEL.md](file:///Users/lunelson/Code/hashintel/brunch-next/docs/design/GRAPH_MODEL.md) (canonical edge layer; Phase 1 locked, Phase 2 nodes pending), [pi-seam-extensions.md §1 Async side-chain sub-agents](file:///Users/lunelson/Code/hashintel/brunch-next/docs/architecture/pi-seam-extensions.md#1-async-side-chain-sub-agents), [pi-seam-extensions.md §Graph clock, §Reconciliation-need substrate, §Oracle plane](file:///Users/lunelson/Code/hashintel/brunch-next/docs/architecture/pi-seam-extensions.md) (note: §"Edge types" in pi-seam-extensions.md is retired and superseded by `docs/design/GRAPH_MODEL.md`)
- **Current execution pointer:** Phase 1 edge lock-and-materialize has landed — the canonical edge contract is captured in `docs/design/GRAPH_MODEL.md` (closed eight-category set, `stance` only on proof/support, `basis ∈ explicit | accepted_review_set`, no edge `status`, immutable accepted-edge identity, dependency-only cascade, separate `ReconciliationNeed` substrate with edge or node-pair target), the corresponding `GraphEdge`, `ReconciliationNeed`, and category-policy stubs live under `src/graph/` (atoms, schema, policy), and the retired named-relation catalogue in `docs/architecture/pi-seam-extensions.md` §"Edge types" plus the brainstorm at `archive/docs/design/GRAPH_EDGE_CATEGORIES.md` are annotated as superseded. Phase 2 (per-plane node kinds, decision shape, framing/theses carrier, node-side provenance/basis) is still TBD and owned by GRAPH_MODEL.md §"Nodes — deferred". Next slice: scope the narrow `CommandExecutor` result contract and one-transaction LSN/change-log skeleton, paired with the A20-L spike (Drizzle 1.0 beta + `drizzle-orm/typebox` + `better-sqlite3` + Pi `registerTool` round-trip) so the version pin and schema-derivation path are settled before later slices import them broadly. Keep M4 thin enough to falsify A3-L/A4-L/A6-L/A8-L/A20-L before widening CRUD or coherence homes.
