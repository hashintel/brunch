...
- `kick-working-indicator` — the boot kick shows no activity indicator for 3–5s (provider latency window). Diagnosed 2026-07-06: `brunch-tui.ts` captures the kick-status `ui` before `InteractiveMode` binds (lands on the noOp context, silently dropped), and pi's `InteractiveMode` only shows the working indicator on `agent_start`, which fires before the TUI subscribes; upstream pi fix = backfill on attach when `session.isStreaming`. Re-diagnose on the braided base first — FE-1134 moved J1 onto the live `session_start(startup)` handler after Pi binds UI, which is the natural home for a Brunch-side backfill (`setWorkingVisible(true)` while the kick is in flight).
