...
- **Status:** all 3 slices landed (2026-06-23) — branch `ka/fe-1055-greenfield-git-substrate` (FE-1055, parent `main`); post-Arc-1; orchestrator suite green (765). **Slice 1 (`40aafd13`):** `createSandbox` fixture mode git-inits the run worktree (empty root commit on `brunch/run/<runId>`); one git-backed substrate (I139-K). **Slice 2 (`4438fa16`):** greenfield parallel per-slice now uses git worktrees + the `materializeEpicVerifyTree` fold (I124-K updated; `mergeSlicesIntoEpicSandbox` dead); ~47 contract tests migrated to git-backed sandboxes (no assertions weakened). **Slice 3 (`c655f24b`):** greenfield auto-promotes onto `brunch/run/<runId>` via plumbing (`harvestGreenfieldRun`); `--out` is now optional export; `promotionSourceDir` file-copy path dead. **Follow-ups:** (1) a focused cleanup commit to retire the dead file-copy machinery (`promotionSourceDir`, `mergeCompletedSlicesIntoTree`, `mergeSourceDirsIntoTree`, `mergeSlicesIntoEpicSandbox` + their tests); (2) **outer-loop debt** — greenfield cook smoke + 3-fixture parity (I139-K) not yet run with real pi.
...
- **Slices (value-ranked; queue to `memory/CARDS.md` at ln-scope):**
  ...
  - **Slice 2 — per-slice isolation convergence:** make greenfield *parallel* per-slice use git worktrees (`ensureSliceWorktree`) so verify-epic folds via `materializeEpicVerifyTree` instead of `mergeSlicesIntoEpicSandbox`. Preserve serial `'shared'` verify-in-place + greenfield halt-on-fail (D170-K). Realizes the D165-K convergence note. **Blast-radius finding (2026-06-23):** this tightens the contract for the *entire default per-slice path* — every per-slice run now requires a git-backed parent + `runId`, which broke ~30 `engine-contract.test.ts` cases that run greenfield against a bare temp dir with no `runId`. The orchestrator change itself is small (~3 forks: `resolveSliceCwd`, the runId precondition, the verify-epic fork); the cost is a mechanical migration of those ~30 tests to `createSandbox` + `runId`. **Route through `ln-oracles`** (merge correctness: add/add fail-closed proven at the `foldSliceBranches` level, different-hunk survival, dep-seeded-as-edits) before re-attempting.
  - **Slice 3 — promotion convergence (COUPLED to slice 2):** retire greenfield `--out` tree copy for FE-877's plumbing branch commit (reviewable, like brownfield). **Finding (2026-06-23):** promotion is a *separate* path from verify-epic — `promotionSourceDir` (per-slice) file-copies via `mergeCompletedSlicesIntoTree`, and the plumbing-promotion equivalent (`harvestCookRun`) folds *slice branches*, which only exist once slice 2 makes greenfield slices git worktrees. So slice 3's parallel path **depends on slice 2**; only serial-greenfield promotion (commit the git-backed shared run worktree) is standalone. **Do 2 + 3 together** as one coupled convergence pass, not piecemeal. Realizes the D166-K convergence note.
