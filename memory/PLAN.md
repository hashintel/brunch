...
9. `toolchain-profile-expansion` — **FE-843**; **active, building on `ka/fe-843-toolchain-profiles`** (stacked on FE-841). Expands the FE-829 toolchain contract so cook builds on more technologies: data-driven profile registry (+ `node-vitest`, `node-test`, `node-jest`, `deno`), live + strict profile selection (plan-time chain `--profile` flag ≫ `snapshot.profile` ≫ architect-classified-from-spec-prose ≫ `bun`; resolved id always persisted into `plan.yaml`; cook errors on unknown ids, stays lenient on absent for fixtures), and an architect-schema profile enum (D160-K intact — classification reads spec prose only). No harness install verb (the cook agent scaffolds per A98). 3-card queue in `memory/CARDS.md`. Deferred follow-ons: fail/infra test-outcome split, brownfield drift-check at cook open, `brunch detect`. Refinement on `plan-build-architect` (FE-829).
