...
- **Current execution pointer:** Web mode HTTP shell is complete; next slice adds the WebSocket JSON-RPC bridge over existing `workspace.*` / `session.*` handlers without adding REST product reads.
...
- 2026-05-21 `web-shell` slices — Done: M3 linear transcript policy hardening landed before browser consumption; the file-backed reader now requires one Pi session header plus well-formed non-header entries; shared exchange projection preserves non-linear failures for RPC/fixture reads; and `--mode web` now starts a minimal native Brunch HTTP shell with no REST product reads. Verified: `npm run verify` after each slice. Watch: remaining web-shell slices should consume the same thin `session.*` / `workspace.*` handlers over WebSocket rather than inventing browser-only session semantics.
