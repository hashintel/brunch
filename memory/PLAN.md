...
- [2026-05-07] FE-698 prompt/context scenario substrate — Packaged markdown prompt registry. Server interviewer, observer, and side-chat role prompts now load from markdown assets through a typed prompt registry, with explicit interpolation and build-boundary asset copying. Verified: `npm run verify`. Watch: next FE-698 slices still need typed context-pack builders and scenario-runner probes.
