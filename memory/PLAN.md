...
- **Kind:** structural
- **Status:** active — **slices 1–2 done** (slice 1: executability contract + repair + `Toolchain` descriptor; slice 2: toolchain-agnostic cook prompts + runner — `ToolchainTestRunner`, `sliceTestTask`/`epicVerifyTask`, `test-writer.md` de-hardcoded, resolved from `plan.profile`; both 2026-06-09); **slice 3 (build-architect LLM stage) next**. SPEC records A100-K (partially-validated), D167-K (+ base/emitted profile-split refinement), I129-K, I130-K, Future Direction §Cook plan generation.
