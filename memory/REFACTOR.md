...
4. [done] Split advanced code-highlighting and diagram-capable rendering from the text-first message path, keeping immediate plain rendering available and loading richer enhancement only when the content or user action actually needs it.
5. [done] Introduce a workspace data adapter that clearly separates durable project state, durable entity state, and ephemeral chat state, while preserving the current user-visible behavior. The goal of this commit is clarity of ownership, not changed semantics.
