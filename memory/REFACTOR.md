...
- [ ] Replace hard-coded reference-code expectations in tests and helper fixtures with derived expectations where possible, so later prefix changes become a contract update instead of a broad brittle rewrite.
- [ ] Switch the canonical reference-code contract to the intended prefixes (`G`, `T`, `A`, `D`, `CTX`, `CON`, `R`, `AC`) and update runtime expectations to match.
