...
2. Import and use the exported transition error-kind union types in the HTTP status helpers, and give both helpers deterministic fallback returns.
3. Narrow the turn-response route exception message to a generic transition failure message while preserving explicit domain error responses.
