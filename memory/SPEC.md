...
4. Structured responses support turn-appropriate option selections or explicit action submissions, an explicit `none of the above` path where relevant, and one attached response note. The interviewer autonomously chooses whether to include options on each question based on conversational trajectory; grounding requires free-text on every submission (options, when present, are optional enrichment), while design preserves the current selection-required gate with a structural "none of the above" path. A single turn may carry multiple assistant-part artifacts (e.g. a grounding card followed by a question card, or a revision card followed by a review set) rendered as stacked cards with one unified response submission.
...
20. The interviewer may invoke context-gathering capabilities such as workspace analysis during grounding; their outputs appear as visible grounding cards paired with question cards within the same turn.
21. Grounding cards are provisional context rendered as turn-internal artifacts paired with a question card within the same turn, so the observer captures from the whole validated unit (grounding context + question + user response) rather than from unvalidated provisional content alone.
...
28. Observer capture treats the full turn — including any turn-internal grounding card or revision card plus the question or review set plus the user response — as one atomic validated unit for knowledge extraction.
...
| A56 | A turn-internal grounding card paired with a question card within the same turn is enough user control for provisional context: the user qualifies the gathered context through their response to the paired question rather than through a separate review workflow. | medium | open | D83, D89, D91, D98, D117 | Manual walkthroughs on greenfield and brownfield kickoff variants once turn-internal grounding cards land. |
...
| A61 | A turn carrying multiple assistant-part artifacts (grounding card + question, or revision card + review set) renders legibly as stacked cards with one response submission without confusing the user about where to respond. | medium | open | D117, D118, D119 | Manual walkthroughs on grounding turns with paired cards and review revision turns. |
| A62 | Per-item commenting on review sets is sufficient for change-request expressiveness without needing inline editing, drag-to-reorder, or per-item accept/reject toggles. | medium | open | D90, D119 | Manual review walkthroughs with varied change-request scenarios (remove, rewrite, split, merge). |
...
83. **Grounding cards are provisional context, not durable knowledge** — context-gathering results remain visible orientation artifacts; only user reactions and subsequent conversational turns feed the observer's durable knowledge extraction. Grounding cards render as turn-internal artifacts paired with a question card so the observer captures from one validated unit. Depends on: D32, D117. Supersedes: brownfield kickoff grounding as a prompt-shaped handoff only.
...
90. **Requirements and criteria resolve through synthesized review turns with per-item commenting** — the interviewer proposes a full candidate set from prior knowledge, presents stable item reference codes, and supports per-item commenting (inline comment toggle on each item) plus one optional global review note, resolved through explicit `accept review` / `request changes` submission. On `request changes`, the submission payload carries per-item comments plus global note; the interviewer regenerates a revised full-set successor turn, treating uncommented items as implicitly approved and interpreting commented items as change requests. Revisions stack in the turn lineage with a version badge and a revision card (changelog summary) above the review set within the successor turn; prior revisions collapse to compact answered-turn summaries. Provenance is revision-level, not per-item. Runtime-generated review turns persist their own interviewer-owned review metadata (`reviewActions` plus the presented `reviewSet`) on the turn so submit, replay, and accepted-review materialization consume one durable review-turn contract. Active review affordances resolve button meaning from that persisted `reviewActions` metadata rather than assuming option order. Earlier requirement-like or criterion-like material remains synthesis input rather than durable canonical requirement / criterion state until review acceptance. In V1, accepting a review uses the same durable phase-progression seam: the accepted review output becomes authoritative carry-forward state, the phase closes through the shared closure mechanism, and the next phase opens into its projected entry affordance without a dead state. Depends on: A44, A52, A61, A62, D118, D119. Supersedes: free-text-only review note as the sole change-request mechanism.
...
97. **Workspace and specification are distinct product concepts** — the workspace is the cwd-backed software context containing `.brunch/`; each specification is one elicitation run within that workspace. Browser routes, HTTP paths, shared transport contracts, and durable DB/storage should all converge on canonical `specification` terms rather than preserving `project` as a long-lived compatibility identity. Depends on: —. Supersedes: overloaded product use of `project`.
98. **Grounding is the canonical first-phase term and workflow key** — user-facing language, shared contracts, persistence identifiers, and future interaction design should all speak in terms of grounding sufficiency; `scope` is a legacy term to remove rather than preserve on the happy path. Depends on: D97. Supersedes: scope / framing copy drift.
...
100. **Phase labels and workflow keys are canonical together** — the workflow stages are Grounding (`grounding`), Elicitation (`design`), Requirements (`requirements`), and Acceptance Criteria (`criteria`). Product labels and internal identifiers should no longer drift. Depends on: D98. Supersedes: previous label mapping (Framing, Elicitation, Requirements Review, Acceptance Review).
...
112. **Turn-artifact persistence is server-owned and interviewer-shaped** — durable review-set, grounding-card, activity-summary, and phase-summary artifacts materialize from interviewer output through one server helper, so the chat-runtime finalize path acts as orchestration glue instead of reconstructing artifact semantics ad hoc. Replay, accepted-review materialization, and seeded walkthroughs therefore consume the same persisted artifact contract the interviewer produced. Depends on: D90, D92, D99, D110. Supersedes: the ownership split where runtime finalization re-derived grounding/review artifacts outside one authoritative persistence seam.
...
115. **Question format is interviewer-autonomous; interpretation and submit gating are phase-aware** — the interviewer chooses whether to include options per-question based on conversational trajectory, not rigid phase rules. The `structuredQuestionSchema` keeps `options` as a required array that may be empty; prompts suggest tendencies (grounding often starts open-ended, design benefits from structured alternatives) but do not mandate format. Phase-awareness lives in two downstream seams: (1) **Observer interpretation** — in grounding, option selections are resonance signals captured as context/goals/constraints; in design, they are commitment signals captured as decisions/assumptions. (2) **Submit gating and "none of the above"** — grounding always requires free-text regardless of whether options are present (options are optional enrichment); design preserves the current gate (selection required, or none-of-the-above + free-text) and "none of the above" remains a structural fixture. Review phases are untouched (fixed accept/reject). The schema, response persistence contract, and card rendering of options (checkboxes, recommendation badges) do not change. Depends on: A59, D89. Supersedes: the rigid phase-format binding where grounding mandated free-text-only and design mandated options.
...
117. **Grounding cards are turn-internal, not standalone turns** — a grounding card (context-gathering result) renders within the same turn as the question it informs, so the observer captures from one validated unit (grounding context + question + user response) rather than from unvalidated provisional content. The interviewer's context-gathering tool call produces the grounding card, then the same turn lifecycle continues to formulate and present the paired question card. In the degenerate brownfield case where the interviewer gathers context but doesn't yet have a rich question, the paired question may be a thin confirmatory prompt ("does this match your understanding?"). Depends on: A61, D83, D89, D99, D112, D115. Supersedes: grounding cards as standalone turns with their own observer capture pass.

118. **Review sets support per-item commenting and iterative regeneration** — each item in a review set has a comment toggle that expands an inline comment field below the item. On `request changes`, the submission payload carries per-item comments plus an optional global review note. The interviewer regenerates the full set as a successor review turn, incorporating uncommented items as implicitly approved and interpreting commented items as change requests (augment, rewrite, remove, split, merge). Revisions stack in the turn lineage with a version badge; prior revisions collapse to compact answered-turn summaries. Provenance is revision-level, not per-item. Depends on: A61, A62, D90, D94, D112. Supersedes: free-text-only review note as the sole change-request mechanism.

119. **Review revision turns carry a revision card above the review set** — when the interviewer regenerates a review set after `request changes`, the successor turn carries a revision card (changelog summary + version badge) as a turn-internal artifact above the review set card, paralleling how grounding cards sit above question cards. The revision card explains what changed without requiring per-item traceability across versions. Depends on: A61, D90, D118. Supersedes: —.

120. **Grounding prompts are hint-guided, not unconstrained** — the grounding system prompt provides a priority-ordered topic list (concept, users/audience, existing constraints, scope boundaries) with example question shapes. The interviewer adapts and merges based on user responses but does not generate questions from scratch. Brownfield grounding adds tool-use hints for workspace scanning (README, package files, src tree) and poses confirmatory questions based on findings. Depends on: A63, D115. Supersedes: the unconstrained grounding prompt that burns extended thinking on option generation.
...
122. **The homepage surfaces workspace binding** — the root route contextualizes the specification list and "new specification" affordance with workspace (CWD) identity, so the user understands that all displayed and created specifications are scoped to the project directory in which they launched the application. Depends on: D97. Supersedes: —.
...
| **grounding card** *(implementation name: preface)* | A turn-internal artifact that presents provisional context from interviewer-invoked context gathering, rendered above a paired question card within the same turn. The observer captures from the whole turn (grounding context + question + user response) as one validated unit rather than from the grounding card alone. The implementation uses `preface` / `PrefaceCard` as the phase-agnostic component and data name; the product concept remains "grounding card" when referring to context gathering during grounding. The preface renders as a simple `bg-tint` rounded box with italic subdued text, not as a DrawerCard. |
...
| **revision card** | A turn-internal artifact on a review revision turn that summarizes what changed from the prior version and displays a version badge (v2, v3, etc.), paralleling how grounding cards sit above question cards. |
...
| **turn-internal artifact** | An assistant-part artifact rendered as its own visual card within a turn but sharing the turn's single response submission. Grounding cards and revision cards are turn-internal artifacts that render above their paired question or review set card. |
...
14. Grounding cards render as turn-internal artifacts paired with question cards, so the observer captures from the whole validated turn rather than from unvalidated provisional content alone.
