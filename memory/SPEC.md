...
| **agent loop**        | The stream → tool_use → execute → tool_result → re-submit cycle. Implemented in `agent-loop.ts`. Modeled after `badlogic/pi-mono` `packages/agent/src/agent-loop.ts`. Yields `DomainEvent`s during streaming. Configurable with tools, system prompt, maxTurns. See D31 |
