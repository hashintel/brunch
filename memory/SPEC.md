...
34. The side-chat is a popover-to-panel chat surface anchored to spec items in the structured spec view, with two entry modes (per-row `chat-with` button and text-selection floating menu) and three user-facing intents (Explore · Edit · Annotate). Proposed changes stage in a patch list visible in the persistent app top-bar; the user reviews and applies in batch. The side-chat is the unified user-driven mutation surface — it subsumes the prior trigger-popover composer concept and the modal revisit-thread flow defined in `docs/design/REVISIT_MODULE.md`.
35. First-run setup detects missing expected LLM provider credentials before the user starts a specification, makes the missing-key state visible on the dashboard, and offers a guided setup path rather than requiring README / shell-env debugging.
36. If Brunch accepts an API key through the UI, it stores credentials outside the project workspace in XDG-compliant user auth/config state; project `.env` files and `.brunch/` never become the default secret-storage target.
37. LLM provider configuration is owned by a shared AI runtime provider seam, so interviewer and observer model creation do not encode direct provider imports or environment-variable reads as product truth. That seam must preserve provider-specific capabilities such as Anthropic thinking / reasoning options or degrade them explicitly.
38. Workspace hygiene detects whether the local `.brunch/` directory is git-ignored and, with explicit user confirmation, can add an idempotent `.gitignore` entry, creating `.gitignore` when absent.
...
| A71 | Multi-chat containers can supersede the old side-chat conceptual model while preserving the shipped V1.1 side-chat interaction as an implementation stepping stone. | medium | open | D130, D139 | Prototype chat containers and reconcile existing side-chat entry points into the multi-chat substrate without keeping a separate side-chat roadmap. |
| A72 | Semantic mutations will eventually need changeset / patch-ledger history distinct from conversational turn ancestry, but the first implementation should prove reconciliation needs before committing the full ledger shape. | medium | open | D139 | Build chat containers plus reconciliation needs first; revisit whether turn-linked provenance remains sufficient before adding full semantic changesets. |
| A73 | Autonomous architect / generator loops can propose useful graph mutations only after human-driven multi-chat and reconciliation surfaces prove the shared mutation pipeline. | low | future | A72, D139 | Run architect proposals in shadow mode after multi-chat / reconciliation seams stabilize, then compare proposed changes against user-driven edits. |
| A74 | OpenRouter will reduce first-run friction for Brunch's likely users compared with requiring direct Anthropic keys, but model capability parity and AI SDK support need proof before making it the default provider path. | medium | open | D134, D135 | Spike provider configuration against interviewer/observer calls, especially model naming, structured output, tool use, and reasoning/thinking support. |
| A75 | XDG-compliant user-scoped auth/config storage is acceptable for UI-entered API keys and safer than writing secrets to the project workspace, while environment variables remain useful for automation and CI. | medium | open | D134, D136 | Prototype key save/load/delete precedence and inspect OS/XDG paths; manual first-run walkthrough verifies users understand where the key is stored. |
| A76 | Users will accept Brunch editing `.gitignore` when the action is explicit, previewable, and idempotent; doing so should reduce accidental commits of `.brunch/` without feeling like surprising repo mutation. | high | open | D137 | Unit-test ignore detection / append behavior and manual dashboard walkthrough with absent, present, and already-covering `.gitignore` states. |
| A77 | Progressive checkability will improve generated specs more than a binary "formal / not formal" framing, because the weakest sufficient witness may be prose, example, test, runtime contract, invariant, proof obligation, or explicit unresolved ambiguity depending on the claim. | medium | open | D138 | Prototype claim-to-witness review on a small corpus and compare whether users can validate meaning without being forced into formal-methods terminology. |
| A78 | Adding `invariant` and `example` / counterexample as product ontology candidates will make intent drift easier to detect without overwhelming early interviews, provided they enter through focused disambiguation and review flows rather than broad questionnaire expansion. | medium | open | D138 | Run transcript probes for examples, counterexamples, and state/transition rules; check whether items improve export and review quality or create noisy capture. |
| A79 | Behavioral kernels can generate higher-yield disambiguating questions than generic elicitation prompts, but only if kernels stay as interviewer machinery that emits checkable artifacts rather than user-visible formalism. | low | open | D138 | Try one or two kernel-card prototypes, likely state/lifecycle and authority/capability, and compare question value against current prompt-only interviewing. |
...
130. **Graph-anchored chat V1.1 proved the user-driven mutation surface, but the side-chat roadmap is superseded by multi-chat** — the shipped side-chat Explore slice proved that graph rows can launch an anchored chat surface through the existing workflow/runtime constraints. The older side-chat conceptual roadmap (V2 Edit / Drill-down / Propose-edge, V3 hard-edit cascade, V4 multi-thread / architect loop) is no longer the forward planning model. Future graph-anchored chat, annotation, edit, revisit, and architect-loop work should route through durable chat containers, reconciliation needs, and semantic changesets / patch ledger rather than extending side-chat as its own product concept. Depends on: A69, A71, A72, D50, D80, D89, D113, D125, D127, D128, D139, Requirement 34. Supersedes: trigger-popover composer (A51, D89 framing), the old side-chat V2/V3/V4 roadmap, and the modal revisit / edit mode framing as independent conceptual tracks. Design docs: `docs/design/PATCH_LEDGER.md`; historical side-chat implementation notes may remain as shipped V1.1 context.

131. **Patch list is the canonical staging surface in the persistent app top-bar; user-driven annotations bypass the explicit Apply step** — proposed mutations from the side-chat (and, later, from the architect loop in A73) stage in a patch list whose **canonical** UI lives in the persistent top-bar (`N Edits · Undo · Apply`), visible regardless of whether the side-chat panel is open. Bulk-apply is the canonical commit affordance for kinds that benefit from review-before-commit: V2's `edit` / `drill-down` / `edge` patches (where the user is proposing a mutation to durable spec content) and V4's architect-loop emissions (where the system, not the user, is the producer). **User-driven `annotate` patches auto-apply on stage** — the user typed the note in the moment and re-review adds nothing; the patch list still records the patch, provides Undo on the resulting batch, and shows it transiently. Auto-apply is suppressed if the previous attempt failed (the patch sits in staged with the explicit Apply button as a retry path); per-entry apply is intentionally not provided. System-driven future annotations (architect-loop in V4) do not auto-apply — system producers always traverse the explicit review step. The side-chat panel surfaces patches inline as a secondary acknowledgment, but the in-panel inline list is convenience UI, not source of truth. Depends on: A71, D113, D130. Supersedes: per-action promote semantics where each chat exchange fires exactly one durable mutation; uniform-staging-required framing where every patch kind goes through an explicit Apply click.

132. **Patch-list module is React-context-native publicly, event-log-shaped internally** — the patch-list module's public API is a `PatchListProvider` component plus three hooks (`usePatchList` for mutations, `usePatchListState` for reactive reads, `useStagedPatches` for filtered selectors), mirroring the `SideChatHost` pattern (D130 sibling). The `Patch` type is a **closed discriminated union** keyed on `kind`; new patch kinds (V2 `edit` / `edge` / `drill-down`, V4 architect-loop emissions) extend the union and force every consumer to acknowledge. Internally, state is a `useReducer` over a typed `PatchEvent` log (`PatchStaged`, `PatchDiscarded`, `PatchSummaryEdited`, `BatchApplied`, `BatchUndone`); derived state is a pure fold over events. The event-log primitive is **internal only** — never exposed publicly — but is shaped to match A71's eventual server-side `appendPatch(spec, patch[])` primitive so migration is a reducer swap rather than a public-API rewrite. Apply fan-out is injected via an `appliers` prop keyed by patch kind; V2 patch kinds typecheck-fail at the provider mount until their applier is supplied. Provider mount order at the spec route layout is `<PatchListProvider><SideChatHost><Outlet /></SideChatHost></PatchListProvider>` — the side-chat is the primary patch producer, so it sits inside the patch-list scope. Depends on: A71, D113, D130, D131. Supersedes: a generic patch-list store / registry / event-log façade as the public surface.

133. **Annotation is a new durable entity, item-anchored in V1, span-anchor-ready in schema** — a new `annotation` table stores user-authored notes anchored to knowledge items via foreign key, distinct from the transient per-turn `itemComments` field on review-set responses (D94). V1 ships with item-level anchoring only — `selectionRange` (start / end offsets) columns are part of the schema from day one but are NULL for V1 annotations and remain NULL until V2/V3 lights up span-anchored annotation as a side-chat entry path. Annotations cascade-delete when the anchor item is deleted; bulk-apply through the patch list (D131) is the only V1 creation seam, and undo deletes through the same id. The annotation entity supersedes the optimistic "comment-store extension" framing — there is no prior comment store to extend; the per-turn `itemComments` field on review-set responses is a separate seam scoped to one review event and stays unchanged. Depends on: D80, D113, D130, D131. Supersedes: the framing of Annotate as a comment-store extension.

134. **First-run setup becomes a product surface, not README-only configuration** — the dashboard should expose provider credential status before specification creation and route users toward setup when no supported key is available. CLI logs and README env instructions can remain, but they are no longer the only supported onboarding path. Depends on: A74, A75. Supersedes: `ANTHROPIC_API_KEY` in project `.env` as the sole user-facing setup contract.

135. **Provider access moves behind one AI runtime provider seam** — interviewer and observer model construction should consume a shared provider/model resolver instead of importing Anthropic directly and reading model environment variables in each caller. The seam may keep Anthropic as the current implementation while testing OpenRouter as the preferred onboarding provider, but provider choice must be explicit and testable. Depends on: A74. Supersedes: direct Anthropic imports in interviewer/observer code as product truth.

136. **UI-entered credentials are user-scoped auth state, not workspace state** — if the app collects an API key, it writes to an XDG-compliant user auth/config location, never to `.brunch/` or the project `.env` by default. Existing environment-variable configuration remains supported as an override path for scripted use. Depends on: A75. Supersedes: project-local `.env` as the only persistent setup mechanism.

137. **`.brunch/` gitignore support is confirm-gated deterministic workspace mutation** — Brunch may inspect the workspace repository and offer to add `.brunch/` to `.gitignore`, but it must not mutate repository files without explicit confirmation. The mutation should be idempotent, preserve existing file content, and create `.gitignore` only when the user accepts. Depends on: A76. Supersedes: relying solely on user memory / docs to ignore the generated workspace directory.

138. **Brunch specs evolve toward intent graphs with progressive checkability** — the product direction is to preserve meaning as typed claims, semantic edges, examples / counterexamples, verification witnesses, unresolved ambiguity, and user validation status rather than treating the spec as a planning document or prose inventory. Requirements and criteria remain distinct product items for now: a requirement is a commitment and a criterion is an oracle / witness. A shared `Property`-like claim primitive, top-level `invariant`, and top-level `example` are live ontology candidates, not committed storage changes. Behavioral kernels are candidate interviewer machinery for surfacing latent state, authority, containment, concurrency, migration, and evidence questions while emitting the weakest useful checkable artifact for the claim. Depends on: A77, A78, A79, D50, D125. Supersedes: the implicit framing that requirements / criteria review is the terminal semantic model of product intent.

139. **Semantic mutation history should split from conversational turn history when graph editing becomes first-class** — turns remain conversational provenance and replay; the intent graph remains current semantic truth; durable chat containers supersede the old side-chat conceptual model; a future changeset / patch ledger records semantic mutation history; and reconciliation needs record semantic debt caused by changes that may stale existing graph truth. The first implementation should favor chat containers plus durable reconciliation needs before a full patch ledger, keeping turn-linked provenance as compatibility while making room for patch-backed provenance later. This explicitly reshapes older side-chat and revisit-session drafts: graph-anchored chat and revisit / cascade remain product capabilities, but the side-chat V2/V3 roadmap and `revisit_session` table shape are no longer the preferred conceptual or persistence foundation once multiple chats, direct graph edits, and reconciliation review sets are in scope. Depends on: A71, A72, D80, D110, D112, D125, D128, D138. Supersedes: turn ancestry as the only plausible semantic history spine, the old side-chat conceptual roadmap after V1.1, and the `docs/design/REVISIT_MODULE.md` table shape as canonical persistence design.
...
| I106 | Provider credential discovery, precedence, dashboard status, and model-provider resolution stay explicit without exposing raw secret values through `/api/config`, logs, persisted specification state, or client-visible payloads. | planned: `runtime-config.test.ts`, `app.test.ts`, `ProjectList.test.tsx` | Requirements 35, 36, 37; D134, D135, D136 |
| I107 | `.brunch/` gitignore hygiene is idempotent and confirmation-gated: existing ignore coverage is detected, missing entries are appended only after user confirmation, and absent `.gitignore` files are created only through that same accepted action. | planned: `project-gitignore.test.ts`, `app.test.ts`, `ProjectList.test.tsx` | Requirement 38; D137 |
...
| **intent graph** | The semantic product truth formed by knowledge items, review-authoritative requirements / criteria, graph edges, examples / counterexamples, validation status, and reconciliation state. Chat and graph views are projections over this truth. |
| **progressive checkability** | The stance that each claim should receive the weakest sufficient witness: human-readable claim, concrete example, counterexample, regression test, runtime contract, state-machine rule, invariant, proof obligation, or explicit unresolved ambiguity. |
| **property** *(candidate ontology)* | A normalized claim primitive that requirements could commit to and criteria could observe. It is a design candidate, not a committed storage or UI surface. |
| **invariant** *(candidate ontology)* | A property that must remain true across relevant states, transitions, executions, versions, or semantic revisions. Candidate top-level product kind. |
| **example** *(candidate ontology)* | A concrete scenario, trace, input/output, edge case, approved example, rejected example, or counterexample that disambiguates or witnesses intent. Candidate top-level product kind. |
...
| **chat** *(future persistence seam)* | A conversation container inside one specification. The primary interview, side-chats, reconciliation chats, verifier feedback, and review discussions may all own turns without owning semantic truth directly. |
| **semantic changeset / patch** *(future persistence seam)* | An atomic set of semantic mutations to the intent graph. It records what changed and why, separate from the conversational turn that may have initiated it. Naming remains open between `changeset` / `change` and `patch` / `patch_change`. |
| **reconciliation need** *(future persistence seam)* | Durable semantic debt saying existing graph truth may require renewed judgment because an upstream item, relation, verifier, or historical premise changed. It is process state, not a knowledge edge. |
