...
45. The workspace runtime can host secondary conversational threads (`side`, `reconciliation`, `qa`) inline with the main interview chat while keeping thread context, replay, and semantic mutations server-authoritative.
...
| A94 | A unified thread substrate can replace independent side-chat persistence while preserving reloadable side, reconciliation, and qa conversations inside one workspace chat surface.                                           | medium     | open   | D138, D153, Requirement 45       | Track 2 thread-substrate RFC plus in-stream rendering/reload walkthroughs.                                          |
| A95 | Thread-scoped context specs, durable `#` mentions, TOON graph serialization, and turn-zero prompts will make non-primary threads useful without ad hoc prompt context.                                                       | low        | open   | D139, D140, D153, Requirement 45 | Track 5 context-provision tests and manual prompt/context walkthroughs per thread kind.                             |
| A96 | Async-by-default reconciliation can move Pending review into an in-stream target-grouped thread without hiding judgment work or surfacing auto-confirmed noise.                                                              | medium     | open   | D135, D137, D138, D146, D153     | Track 3 classifier scheduling, target-ordering tests, and dense reconciliation walkthroughs.                        |
...
153. **Conversational Workspace Runtime supersedes independent side-chat persistence** — continuous workspace is the host; side, reconciliation, and qa work should converge into inline threads in the unified chat surface, while `changeset` / `change` remains the semantic mutation spine.
154. **Thread substrate must be reconciled with strategy and context ownership before schema commitment** — `parent_chat_id`, a new `thread` table, and UI-only rendering remain alternatives until Track 2 proves how chat-local strategies, one-open-frontier semantics, thread context specs, and prompt/context-pack assembly compose without duplicate authority.
...
| I120 | Threads remain conversational process containers, not workflow or semantic truth: inline thread rendering, collapse/reload state, and context replay may organize discussion, but accepted mutations still flow through Brunch-owned handlers and changesets.            | planned: thread-substrate, context-provision, changeset/app tests                             | Requirement 45; A94, A95; D143, D149, D153, D154 |

## Future Direction Register

### Conversational workspace runtime

- The Conversational Workspace Runtime is the next architectural umbrella after continuous workspace Track 1: the shipped shell hosts unified chat, inline threads, reconciliation, changesets, and thread context provision. See `docs/design/CONVERSATIONAL_WORKSPACE_RUNTIME.md` and PLAN items `chat-runtime-threads`, `reconciliation-runtime`, `changeset-ledger`, and `thread-context-provision`.
- Persistent side-chat history is no longer an independent V4a frontier. It is absorbed by the thread runtime; Track 2 must decide whether threads persist as child chats, a new thread table, or UI-rendered metadata.
- Track 2 is a coordination/RFC frontier, not a schema implementation mandate. It must align with chat-local strategies, one-open-frontier semantics, and prompt/context-pack ownership before accepting a `thread` table or any equivalent substrate.
- Reconciliation absorption and thread-context provision stay downstream of Track 2; changeset/change history can proceed in parallel and remains the semantic mutation authority.
...
| **thread**                                 | User-visible sub-run inside the unified chat surface. Thread substrate shape is still open (`parent_chat_id`, new table, or UI-only metadata), but threads do not own workflow state, strategy authority, prompt/context assembly authority, or semantic truth directly. |
| **thread context spec**                    | Replayable context selection for a thread: scope, roots, include rules, and durable mention-derived context items used by prompt assembly.                                                                                                                               |
| **turn-zero**                              | Assistant-authored kickoff turn that seeds a new thread with kind-appropriate context and options before the user responds.                                                                                                                                              |
