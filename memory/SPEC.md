...
| I19-L | Brunch-controlled flows do not create or navigate Pi session branches, and Brunch transcript readers fail fast on non-linear JSONL rather than flattening, migrating, or branch-selecting. | partially covered (M3 transcript loader rejects non-linear child graphs, `parentSession`, and `branch_summary`; `session.elicitationExchanges` returns a product-shaped error for non-linear selected sessions; Brunch TUI extension cancels `session_before_tree` and `session_before_fork`) | D24-L, D6-L, D11-L, D13-L |
