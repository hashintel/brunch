...
131. **Patch list is the canonical staging surface in the persistent app top-bar** — proposed mutations from the side-chat (and, later, from the architect loop in A73) stage in a patch list whose **canonical** UI lives in the persistent top-bar (`N Edits · Undo · Apply`), visible regardless of whether the side-chat panel is open. Bulk-apply is the single canonical commit affordance: the user applies all staged patches in dependency order or discards what they don't want first; per-entry apply is intentionally not provided in V1. The side-chat panel surfaces patches inline as a secondary acknowledgment when staging happens from inside the chat, but the in-panel inline list is convenience UI, not source of truth. The same patch list serves the architect loop in V4 — there is no second review surface to invent for system-driven patches. Depends on: A71, D113, D130. Supersedes: per-action promote semantics where each chat exchange fires exactly one durable mutation.
