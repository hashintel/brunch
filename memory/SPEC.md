...
| A71 | A unified `spec → chat → turns` data model with diff-shaped patches as the persistence primitive will simplify the side-chat's patch-list, item versioning, and the architect loop without sacrificing the durable workflow guarantees of the current store-of-stores. | low | future | D113, D125, D130, D131 | Prototype the patch / event-stream model on a side branch and compare apply-step latency, audit-log clarity, and refactor surface area against the current per-store mutation fan-out. |
| A72 | Knowledge-graph items can carry version history without breaking the active-path durable-truth contract: each version is the result of an applied patch, prior versions are queryable for diff/comparison/audit, and the active-path projection always reflects the latest version for each item. | low | future | A71, D80, D130 | Prototype item versioning behind A71's patch model; verify that revisit cascades, span-anchored annotations, and soft-edit audit trails behave correctly across versions. |
| A73 | An autonomous architect / generator agent can produce useful patch proposals against the knowledge graph and surface them through the same patch list the side-chat uses, so HITL review is a unified surface regardless of patch origin. | low | future | A71, D130, D131 | Run the architect in shadow mode against seeded specs once V4 ships; compare proposed patches against subsequent user-driven edits to gauge precision/recall. |
...
130. **Side-chat is the unified user-driven mutation surface** — graph-launched chat anchored to spec items, with two entry modes (per-row `chat-with` button and text-selection floating menu), three user-facing intents (Explore · Edit · Annotate), and four internal patch kinds (`edit` · `edge` · `drill-down` · `annotate`) routed through one shared mutation pipeline. **Edit is a router**, not a single action: applied edit patches route by anchor-item phase status (open → Refine via successor turn; closed → impact tier `none` / `soft` / `hard`). Span-level pinned context is supported as a prompting hint, but patch granularity stays item-level in V1; only `annotate` patches carry an optional `selectionRange` field for span-anchored notes. The side-chat replaces the trigger-popover composer concept and absorbs the modal secondary thread defined in `docs/design/REVISIT_MODULE.md` into its panel; both prior horizon items are subsumed. Phasing: V1 ships Explore + Annotate; V2 adds Edit + Drill-down + Propose-edge; V3 absorbs REVISIT_MODULE's cascade preview and batch-resolution secondary-thread; V4 unlocks multi-thread + architect-loop integration once A71 lands. Depends on: A69, D50, D80, D89, D113, D125, D127, D128, Requirement 34. Supersedes: trigger-popover composer (A51, D89 framing) and revisit / edit mode + cascade preview (Requirement 10, D50, D80 secondary-thread modal). Design doc: `docs/design/SIDE_CHAT.md`.
