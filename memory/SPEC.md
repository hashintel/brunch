...
125. **Observer capture is relation-first across the ontology** — graph edges should be emitted for any knowledge kinds when the turn or accepted review makes the relationship reasonably traceable; sparse abstention is preferable to speculative edge inflation, but decisions and assumptions are no longer the only kinds eligible for structured relationship capture. Depends on: A66, D50. Supersedes: the decision/assumption-only relationship extraction instruction in the current observer seam.
