...
94. **Phase progression is frontier-anchored** — every open phase bottoms out in exactly one actionable frontier turn or a visible generation state; fixed kickoff turns seed newly opened phases, successor generation avoids closed-without-frontier gaps, and recovery turns exist only to repair broken frontier state. If a phase is closed, the transcript bottoms out in a handoff or completion artifact. Depends on: A51, A54. Supersedes: —.
