...
- **D21-L — Workspace session coordination is the spec/session boot seam.** Brunch owns a narrow `WorkspaceSessionCoordinator` for boot, spec selection, and `/new` session creation. It is the only product module allowed to create or open Pi sessions for Brunch user flows and the only module allowed to write `brunch.session_binding`; callers receive `ready | select_spec | needs_human` workspace-session state and never mutate a session's bound spec. The coordinator hides `SessionManager.create(cwd, ".brunch/sessions/")`, `switchSession`/new-session setup callbacks, `.brunch/state.json` current-spec acceleration, binding validation, and chrome-state derivation. Because pi defers writing session JSONL files until an assistant message exists, the coordinator explicitly flushes the initial header + `brunch.session_binding` custom entry so Brunch sessions are self-describing before the agent loop starts. Depends on: D6-L, D11-L. Supersedes: the loose `SpecRegistry` + caller-orchestrated session-binding mental model.
...
| I8-L | Spec selection persists across pi `switchSession` (i.e. `/new`); each session has exactly one `brunch.session_binding`, and a session's bound spec never changes. | partially covered (M0 coordinator integration tests + store-only runbook checker; TUI integration and JSONL reload viability still planned) | D11-L |
