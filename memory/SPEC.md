...
45. The workspace runtime can host secondary chats (`side`, `reconciliation`, `qa`, `strategy`) inline with the primary interview chat while keeping transcript replay, explicit context insertion, and semantic mutations server-authoritative. Schema-level `thread` is deferred.
...
| A95 | Transcript-first context with explicit context snapshots and active graph-item handles can keep secondary chats useful across multi-chat item changes without a persisted context-spec table.                                 | medium     | open   | D139, D140, D154, Requirement 45 | Context-provision tests for snapshot insertion, stale-handle refresh, and prompt/context-pack rendering.            |
...
154. **Chat context is transcript-first with explicit snapshots and handles** — a chat primarily uses its own transcript as prompt context. Additional graph/workspace context enters through explicit context snapshot artifacts inserted into turns; active handles track referenced graph items across turns and cause fresh snapshots to be inserted when current item versions advance, including changes made by other chats. Do not introduce a persisted context-spec table by default.
...
| I120 | Secondary chats remain conversational process containers, not workflow or semantic truth: inline rendering, collapse/reload state, context snapshot replay, and stale-handle refresh may organize discussion, but accepted mutations still flow through Brunch-owned handlers and changesets.                   | planned: chat-runtime, context-provision, changeset/app tests                                 | Requirement 45; A94, A95; D143, D149, D153, D154 |
...
- Context provision should be transcript-first: context builders may derive initial snapshots from chat kind/strategy/anchors, but durable prompt context is represented by inserted context snapshots and refreshed graph-item handles rather than a hidden persisted context-spec table.
...
| **context snapshot**                       | Turn artifact that records graph/workspace context explicitly inserted into a chat transcript at a point in time, including subject id, version/fingerprint, snapshot content, reason, and provenance. Snapshots do not mutate when source truth changes. |
| **context handle**                         | Durable or transcript-derived reference saying a chat is tracking a graph subject across turns. Before new assistant turns, stale handles may cause fresh context snapshots to be inserted when the subject's current version/fingerprint has advanced.   |
