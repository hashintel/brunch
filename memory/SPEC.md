...
| I23-L | Every structured elicitation interaction that owns the response surface persists exactly one terminal structured result (`answered`, `skipped`, `cancelled`, or `unavailable`) in Pi JSONL before the next agent turn consumes it. For structured-question/questionnaire tools, `toolResult.details` is self-contained enough for Brunch projection (status, mode, prompts/questions, options, answers, and transport metadata); the assistant tool-call args are correlation/position rather than the only semantic source. | planned (FE-744 structured-question tool tests + JSON-over-editor RPC fallback + projection contract) | D12-L, D13-L, D17-L, D37-L, D38-L |
