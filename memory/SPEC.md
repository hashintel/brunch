...
4. Structured responses support turn-appropriate option selections or explicit action submissions, an explicit `none of the above` path where relevant, and one attached response note. Grounding questions use a free-text-only format (question + why + response note) without requiring option selections; elicitation and later phases use the option-selection format. A single turn may carry multiple assistant-part artifacts (e.g. a grounding card followed by a question card, or a revision card followed by a review set) rendered as stacked cards with one unified response submission.
...
| A64 | Replacing coarse `router.invalidate()` with granular TanStack Query domains eliminates the scroll-jank cascade without introducing coordination complexity or stale-data bugs. | medium | open | D121 | Prototype the query decomposition and measure scroll stability plus data freshness during observer updates. |
...
115. **Grounding questions use a free-text-first format distinct from elicitation option questions** — the grounding phase asks open, exploratory questions where the user provides context through a response note and optional free-text answer rather than choosing between interviewer-proposed options. The `structuredQuestionSchema` makes `options` optional (or a phase-aware variant omits them), the grounding system prompt asks open context-gathering questions without mandating 2-4 options, and the response schema accepts `freeText`-only submissions. Elicitation and later phases continue to use the existing option-selection format. Depends on: A59, D89. Supersedes: the uniform option-mandatory question format across all phases.
...
121. **Client data ownership migrates from coarse loader invalidation to granular query domains** — transcript/turn data, knowledge state, and workflow state become independently invalidable TanStack Query domains scoped to the specification. Mutations and SSE events invalidate only the relevant query key. The router loader becomes a thin shell that ensures the specification exists and provides the ID. The decomposition boundary is a design-first concern (query hook count and shape) before implementation. Depends on: A64, D87. Supersedes: monolithic `router.invalidate()` after every mutation.
...
| **grounding card** | A turn-internal artifact that presents provisional context from interviewer-invoked context gathering, rendered above a paired question card within the same turn. The observer captures from the whole turn (grounding context + question + user response) as one validated unit rather than from the grounding card alone. |
...
| **query domain** | An independently invalidable TanStack Query scope within a specification (e.g. transcript/turns, knowledge state, workflow state). Mutations and SSE events invalidate only the relevant domain rather than the entire route tree. |
