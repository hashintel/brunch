...
| A56 | A turn-internal preface card paired with a question card within the same turn is enough user control for provisional context: the user qualifies the gathered context through their response to the paired question rather than through a separate review workflow. | medium | open | D83, D89, D91, D98, D117 | Manual walkthroughs on greenfield and brownfield kickoff variants plus non-grounding phases with preface cards. |
...
| A61 | A turn carrying multiple assistant-part artifacts (preface card + question, or revision card + review set) renders legibly as stacked cards with one response submission without confusing the user about where to respond. | medium | open | D117, D118, D119 | Manual walkthroughs on turns with paired preface + question cards and review revision turns. |
...
83. **Preface cards are provisional context, not durable knowledge** — context-gathering results remain visible orientation artifacts; only user reactions and subsequent conversational turns feed the observer's durable knowledge extraction. Preface cards render as turn-internal artifacts paired with a question card so the observer captures from one validated unit. Depends on: D32, D117. Supersedes: brownfield kickoff grounding as a prompt-shaped handoff only.
...
99. **Interviewer-invoked context gathering is a reusable capability, not a brownfield-only startup ritual** — workspace analysis, future web research, and similar moves may appear as preface cards in any phase whenever the interviewer needs more context for the next move, gated only by cwd availability rather than by phase or grounding strategy. Depends on: D30, D32, D83. Supersedes: one-shot brownfield kickoff exploration.
...
117. **Preface cards are turn-internal, not standalone turns** — a preface card (context-gathering result) renders within the same turn as the question it informs, so the observer captures from one validated unit (preface context + question + user response) rather than from unvalidated provisional content. The interviewer's context-gathering tool call produces the preface card, then the same turn lifecycle continues to formulate and present the paired question card. In the degenerate case where the interviewer gathers context but doesn't yet have a rich question, the paired question may be a thin confirmatory prompt ("does this match your understanding?"). Depends on: A61, D83, D89, D99, D112, D115. Supersedes: preface cards as standalone turns with their own observer capture pass.
