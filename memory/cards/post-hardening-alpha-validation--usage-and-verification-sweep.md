...
| Cross-surface graph/session settlement | `partial` | ● | `proving` | FE-1348; re-enter only with fresh explicit authorization in an execution/handoff context that proves its sole host/browser remain live for the user's decision | The latest one-shot witness passed the repaired pre-approval semantic gate: one exact instruction produced one successful one-node/zero-edge settled review set; React and canonical JSONL agreed on visible `settled`, exactly one each of Approve/Request changes/Reject, no generic review Answer, no duplicate proposal/narration, and no pre-approval graph effect. The delegated host then exited before the user could perform the authorized Approve action. This is WI1 witness-infrastructure lifetime divergence, not recurrence of SW2 or failure of SW3. Exact 13-entry JSONL, public pre-approval presentation/graph reads, browser AX/render evidence, host lifecycle, and read-only SQLite prove no approval, effect, receipt, A/B projection, or reload occurred; graph stayed empty at LSN 1 with only `create_spec`. Evidence: [`cross-surface-graph-session-settlement-post-sw3-final/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-post-sw3-final/walkthrough.md); prior SW3, SW2, and CS1 provenance: [`cross-surface-graph-session-settlement-witness/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-witness/walkthrough.md), [`cross-surface-graph-session-settlement-rerun/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-rerun/walkthrough.md), [`cross-surface-graph-session-settlement-final/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-final/walkthrough.md). |
