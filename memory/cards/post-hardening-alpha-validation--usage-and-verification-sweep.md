...
| Stdio public RPC | `partial` | ● | `proving` | FE-1348; re-enter only after clone activation/projection portability is separately accepted and repaired, or a fresh supported pending target supersedes the retained input | `rpc.discover`, workspace/session activation, one read and one structured exchange through `npm run dev-cli -- rpc`; schemas, method results, JSONL, and projections agree without raw Pi RPC. Owned gate: the retained provider-authored `fe1348-anchor-2` ask remained byte-identical in a disposable clone, but public `workspace.activate` first returned `needs_human` because the selected copied session was unavailable; later read-only projections disagreed with canonical pending state, so no response was submitted and no repair/retry occurred. Evidence: [`testing/walkthroughs/2026-08-10/stdio-public-rpc-owned-gate.md`](../../testing/walkthroughs/2026-08-10/stdio-public-rpc-owned-gate.md), [`stdio-public-rpc-reentry/`](../../testing/walkthroughs/2026-08-10/stdio-public-rpc-reentry/). |
