...
| Standalone-web driven session | `partial` | ● | `proving` | FE-1348; fix the reload/cleanup defect in a separate row-sized scope, then rerun the bounded browser journey | The 2026-08-11 rerun cleared the browser/provider gates: real React opened the target, a production-provider ask settled through the browser, and JSONL/public reads agreed. Two normal reloads then rendered `Session transcript cannot be displayed`, and bounded host termination left a stale writer owner after the process/listener were gone. No state was repaired manually. Evidence: [`testing/walkthroughs/2026-08-10/standalone-web-driven-session.md`](../../testing/walkthroughs/2026-08-10/standalone-web-driven-session.md). |
