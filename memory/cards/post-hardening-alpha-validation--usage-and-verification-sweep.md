...
**Excluded:** feature implementation except the separately scoped deterministic production availability repair promoted from the final Execute row; inline incidental fixes/refactors; broad provider/model quality campaigns; FE-1187 R8–R10/A48-L; KA-owned Execute evidence; saved-mission and execution-comparison reruns; legacy 0.x migration without a disposable fixture; cross-platform claims beyond the available witnessed host; shared-session-host cutover deletions; actual npm publication.
...
| Execute mode interaction | `partial` | ● | `proving` | FE-1348; build [`post-hardening-alpha-validation--execute-availability.md`](post-hardening-alpha-validation--execute-availability.md), then rerun the owned gate | Real mode switch and `/brunch:consult` consume capability-honest deterministic Prepare/Compile/Execute availability on a fresh supported target; session runtime state and public plan/run readbacks match visible promises. Do not manufacture state, invoke a provider during the witness, or execute a plan. The retained thin-seed witness honestly advertised Prepare only and its runtime read agreed, but repository inspection then showed production callers always pass the static Prepare-only menu even though a state-aware builder exists. User direction transferred only that bounded deterministic production wiring from FE-1187 to FE-1348; A48-L's model-backed improvement/campaign remains excluded. Evidence: [`testing/walkthroughs/2026-08-10/execute-mode-interaction-owned-gate.md`](../../testing/walkthroughs/2026-08-10/execute-mode-interaction-owned-gate.md). |
...
| Cross-surface graph/session settlement | `built` | ● | `proving` | FE-1348 | At commit `cad69d602`, a persistent user-owned standalone host carried one settled one-requirement/zero-edge proposal through exactly one Approve action and exactly one reload. React showed the canonical three-choice set before approval, then Decision: Approve and receipt LSN 2 with no remaining controls. Canonical JSONL, byte-equal public presentation A/B, byte-equal graph A/B, and stopped-host read-only SQLite agree on exactly `create_spec` then one `accept_review_set`, one settled explicit REQ1 created/updated at LSN 2, zero edges, no `mutate_graph`, and no second acceptance. The JSONL's `Do  │ not` is blockquote-gutter witness-harness contamination, recorded as byte-exact conduct `met-with-divergence`, not a product finding. This successful path satisfies WI1's re-entry trigger and outer-witnesses CS1/SW2/SW3 without erasing prior failure provenance. Evidence: [`cross-surface-graph-session-settlement-post-sw3-success/walkthrough.md`](../../testing/walkthroughs/2026-08-13/cross-surface-graph-session-settlement-post-sw3-success/walkthrough.md); prior WI1/SW3/SW2/CS1 provenance remains at [`cross-surface-graph-session-settlement-post-sw3-final/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-post-sw3-final/walkthrough.md), [`cross-surface-graph-session-settlement-witness/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-witness/walkthrough.md), [`cross-surface-graph-session-settlement-rerun/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-rerun/walkthrough.md), and [`cross-surface-graph-session-settlement-final/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-final/walkthrough.md). |
...
Record evidence in `TESTING_FINDINGS.md`. Do not edit production code while executing an evidence row; the promoted Execute repair lives in its separate scope file and must land before that row resumes.
...
src/                                                               ?  # only through the promoted Execute repair scope; no inline row fixes
```
