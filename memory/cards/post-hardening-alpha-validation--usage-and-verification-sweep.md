...
| Cross-surface graph/session settlement | `partial` | ● | `proving` | FE-1348; re-enter only after `shared-session-host-cutover` repairs settled review/continuation semantics and a fresh journey is explicitly authorized | The consumed final witness reached exactly one successful one-node/zero-edge settled review set after one exact user instruction, with no SW2 duplicate actionable ask or proposal. React did not visibly label settlement, collapsed canonical Approve/Request changes/Reject continuation choices to a generic Answer/input, and rendered offer plus continuation copy as two confirmation-like blocks. The user correctly stopped before approval. Exact 13-entry JSONL, screenshot, and read-only SQLite audit prove the canonical review/choices existed but no approval, effect, receipt, A/B projection, or reload occurred; graph stayed empty at LSN 1 with only `create_spec`. This is a remaining React semantic-projection contradiction inside the closed row, not SW2 recurrence or a new inventory row. Evidence: [`cross-surface-graph-session-settlement-witness/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-witness/walkthrough.md); prior SW2 and CS1 provenance: [`cross-surface-graph-session-settlement-rerun/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-rerun/walkthrough.md), [`cross-surface-graph-session-settlement-final/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-final/walkthrough.md). |
