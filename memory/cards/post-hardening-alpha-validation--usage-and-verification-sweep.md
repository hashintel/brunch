...
| Stdio public RPC | `partial` | ● | `proving` | FE-1348; re-enter on authorized provider-backed present or a deterministic supported product path | `rpc.discover`, workspace/session activation, one read and one structured exchange through `npm run dev-cli -- rpc`; schemas, method results, JSONL, and projections agree without raw Pi RPC. Owned gate: discovery, activation, reads, and canonical agreement pass, but `session.triggerExchange` intentionally mints no exchange; provider turns and test-only transcript minting are excluded. Evidence: [`testing/walkthroughs/2026-08-10/stdio-public-rpc-owned-gate.md`](../../testing/walkthroughs/2026-08-10/stdio-public-rpc-owned-gate.md). |
