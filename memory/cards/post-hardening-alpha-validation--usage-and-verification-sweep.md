...
| --- | --- | --- | --- | --- | --- |
| Authless bare-workspace entry | `partial` | ● | `proving` | `src/app/`, workspace dialog, I59-L | Fresh scratch `PI_CODING_AGENT_DIR` + bare workbench: creation remains available, no provider turn fires, `/login` recovery is named; DB/workspace/session reads confirm no hidden auth or premature turn. |
| Populated-workspace posture entry | `partial` | ● | `proving` | workspace/session coordinator, D118-L | Fresh populated workbench with no `.brunch/`: dialog establishes kind/origin before the agent; `workspace.state` and DB readback prove persisted posture and no invented authority. |
| Specify session interaction | `partial` | ● | `proving` | TUI + elicitor/exchange/graph owners | Real TUI path performs orientation, one ask/answer, one proposal or digest/review outcome, and graph readback; JSONL, graph overview, receipt/LSN, and debug mirrors agree. |
| Execute mode interaction | `partial` | ● | `proving` | TUI + executor extensions | Real mode switch exposes capability-honest Prepare/Compile/Execute availability on a suitable existing seed; session runtime state and plan/run read methods match visible promises. Full provider execution remains excluded. |
| Session resume and active-tree continuity | `partial` | ● | `proving` | `src/session/`, coordinator, D24-L | Quit/relaunch and one Pi-valid branch/tree change preserve selected spec, active-leaf transcript, mode/style, and canonical JSONL projection without append-order leakage. |
...
| Cross-composition writer transfer | `partial` | ● | `proving` | session writer guard + standalone host, I64-L | While TUI owns the target, standalone open is refused without mutation; after normal TUI shutdown, standalone web reopens and continues the same JSONL; owner/lock and projection evidence agree. |
...
| Stdio public RPC | `partial` | ● | `proving` | `src/rpc/` | `rpc.discover`, workspace/session activation, one read and one structured exchange through `npm run dev-cli -- rpc`; schemas, method results, JSONL, and projections agree without raw Pi RPC. |
| Print projection | `partial` | ● | `proving` | `src/app/print-workspace-state.ts` | Source and installed-package foreign-cwd `--mode print` both project the selected workspace without mutation; before/after canonical files are byte-stable. |
| Cross-surface graph/session settlement | `partial` | ● | `proving` | CommandExecutor + projections, I15-L/I32-L/I65-L | Reuse the Specify/RPC/browser journeys: one accepted effect yields one receipt/LSN/change-log entry, exact settlement, and normalized-equivalent fresh projections; no UI/debug mirror is treated as authority. |
...
| TUI-driver lifecycle fallback | `partial` | ● | `proving` | `src/dev/tui-driver.ts` | Start/wait/send/screen/stop/remove one bounded session; `list` shows no live or residual session and the captured screen reflects the real entry path. |
...
| Debug-mirror legibility | `partial` | ● | `proving` | `src/dev/`, `.brunch/debug/` | A provider-triggering or deterministic supported path produces the documented mirrors; each answers its named operator question and agrees with JSONL/runtime state. Absence is accepted only where canonical docs declare it optional. |
| Read-only repository gate | `partial` | ● | `earned` | `package.json`, CI | `npm run check` passes; warnings are classified as known or promoted, never silently fixed outside the sweep manifest. |
| Full retained local gate | `partial` | ● | `earned` | Vitest/build scripts | `npm run verify:full` passes, proving default, core-slow, comparison, and build paths through the declared aggregate command. Record counts/skips and compare with policy. |
| Comparison lane entry | `partial` | ● | `proving` | comparison controllers/oracles | `npm run test:comparison` passes its closed current suite and leaves bounded cleanup. This validates the lane, not a fresh provider comparison campaign. |
| Conditional CI lane selection | `partial` | ● | `earned` | `scripts/ci-test-lanes.mjs`, workflow | Focused selector tests prove closed allowlist omission and fail-open behavior for runtime, unknown/incomplete, and merge-group events; workflow and SPEC policy agree. |
...
| Current testing guidance | `partial` | ● | `earned` | `TESTING_PLAN.md`, manual-testing docs, SPEC verification policy | Reconcile stale branch/PR/command/runtime assumptions; link rather than duplicate canonical command policy; every retained concern maps to a current row or named deferred owner. Markdown-link check passes. |
