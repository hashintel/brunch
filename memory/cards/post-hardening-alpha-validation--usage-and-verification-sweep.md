...
| Cross-surface graph/session settlement | `partial` | ● | `proving` | FE-1348; re-enter after an explicitly scoped repair makes one-node/zero-edge review sets structurally legal and guards the consequential projection failure, then obtain new user authorization | The failed authorized journey retained the exact 13-entry canonical JSONL and proved no graph effect: public overview and read-only SQLite remained `nodes: []`, `edges: []`, LSN 1. The provider authored exactly one settled requirement with `edgeDrafts: []`, but `present_review_set` rejected the legal minimum batch as `structural_illegal` (`edgeDrafts must be non-empty`), then emitted an unanswered standalone approval ask; React subsequently could not display the transcript. CS1 is promoted to this row; the projection failure is consequential evidence, not a second inventory finding. Existing SW2 ownership is unchanged. Evidence: [`testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-final/walkthrough.md`](../../testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-final/walkthrough.md). |
