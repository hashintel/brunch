...
Build the first open required row whose gate is satisfied. Current next ready row: **WR8 · No-auth onboarding surface**.
...
| WR6 | Themed exchange-tool failure rendering after raw JSON leak diagnosis | `built` | ● | proving | `.pi/extensions/exchanges` shared validation-failure result helper plus active exchange tools | Inputs: A6 ask invocation JSON leak, C3 `present_candidates` retry JSON leak; D104-L render-honesty. Diagnosis: active exchange tools were throwing Zod parse errors at the params boundary, leaving Pi's generic tool-error path to expose raw validation payloads instead of a Brunch exchange result. Built 2026-07-09: `ask`, `present_candidates`, `present_digest`, and `present_review_set` now return a themed markdown `TOOL_INPUT_INVALID` tool result with field diagnostics on validation failure; regression coverage proves ask and present-candidates failures render human-legible text without leaking raw payload values. |
| WR7 | Seed insertion legibility after no-auth → post-login continuation diagnosis | `built` | ● | proving | `src/.pi/extensions/commands/index.ts` `/brunch:continue` → `src/.pi/extensions/session-orientation/juncture.ts` manual-trigger origination | Inputs: A5; D102-L as amended by D118-L, I59-L. Diagnosis: WR3's general `/brunch:continue` seam already owns the no-auth-suppressed boot recovery path; it resolves live kick context after auth, calls manual-trigger origination with `forceSeed`, delivers `brunch.context_seed` via `sendMessage` before the trigger-turn `brunch.kick`, and forwards the origination decision to the debug mirror callback. Built 2026-07-09: command regression coverage now proves the no-declared-ask continue path emits seed-before-kick and exposes a manual-trigger origination decision containing the seed entry, so `.brunch/debug/{entry-contents.md,origination.md}` have the insertion point/trigger evidence. This stayed row-sized and did not promote into `spec-posture`. |
