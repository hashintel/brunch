...
| --- | --- | --- | --- | --- | --- |
| Dismissal is absence, not an orientation pseudo-choice | `partial` | `●` | `earned` | `src/.pi/extensions/session-orientation/` | Remove internal `'dismissed'` from `SessionOrientationChoice` and dialog results; Escape/timeout returns no choice and still writes/fires nothing. Repair stale command/juncture comments and the command test that fabricates a retired mixed-carrier entry. Keep parser tests that deliberately reject `dismissed`. Closure: `session-orientation/__tests__/index.test.ts`, `juncture.test.ts`, and `commands-runtime-switch.test.ts`. |
| Orientation menu and mirror copy use work/move vocabulary | `partial` | `●` | `earned` | `src/.pi/extensions/session-orientation/index.ts` | Replace **Choose how Execute mode should continue** and stale Continue=Wait component fixtures with current style/process-move or neutral component ids. Update the component preview and consequential-fact campaign title/locator only; do not broaden into parked campaign conduct repair. Closure: orientation index, consult-menu, component-preview registry, and campaign-runner tests. |
| Branch-flow policy copy names the actual `/new` action | `partial` | `●` | `earned` | `src/.pi/extensions/commands/policy.ts` | Replace “Use `/new` to continue within the selected spec” with direct start-new-session wording. Closure: `src/app/__tests__/brunch-tui.test.ts`. |
| Agent prompt resources do not teach Continue as generic progression | `partial` | `●` | `earned` | `src/agents/skills/` | Replace `continue through map-plans` / `Continue in …` handoff prose with direct `use` / `read` wording. This row covers `project/SKILL.md` and `map/references/map-nodes.md`; ordinary prose outside product prompt resources remains out. Closure: `src/agents/skills/__tests__/registry.test.ts` plus aggregate lexical oracle. |
| Canonical event records point at materialized current state | `partial` | `●` | `earned` | `memory/SPEC.md` | Update D98-L/D109-L/D119-L/D120-L materialization status/pointers without changing their decisions; correct the Continue/Wait glossary so inertness is Escape dismissal, not a no-kick menu option. PLAN points to this active ledger. Closure: `npm run check:markdown-links`, exact decision/pointer review, and topology cross-check. |
