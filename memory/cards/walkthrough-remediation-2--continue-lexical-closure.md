...
| Closed live-surface lexical negative-space oracle | `built` | `●` | `earned` | `src/.pi/extensions/__tests__/continue-lexicon.test.ts` | Added a focused static inventory over current `src/.pi/components`, `src/.pi/extensions`, and `src/agents` source/prompt files plus the two in-boundary mirrors `src/dev/component-preview/registry.ts` and `src/dev/consequential-fact-campaign-runner.ts`, excluding tests, fixtures, topology prose, and the rest of `src/dev`. Historical rivals pin stale Continue=wait/dismiss/proceed/next-step menu/progression shapes; explicit negative-space examples preserve recovery commands, declared continuations, workspace resume state, parser rejection rivals, and TypeScript keywords. No new in-boundary stale occurrence was found and no source copy changed. Closure: `continue-lexicon.test.ts` and `npm run verify`. |
