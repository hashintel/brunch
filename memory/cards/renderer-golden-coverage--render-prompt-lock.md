...
| --- | --- | --- | --- | --- | --- |
| Foreground prompt-body topology is canonical | `partial` | ● | earned | `src/agents/registry.ts`, `src/agents/prompts/README.md`, prompt body tests | Closure oracle: one foreground body home matches the D98 target vocabulary (`elicitor` / executor direction) or a narrow explicit deferral says which CODE row owns the remaining rename. No compatibility alias from old role/body names remains without an owner. |
| Background subagent body topology is canonical | `partial` | ● | earned | `src/.pi/extensions/subagents/agents.ts`, prompt/subagent topology docs | Closure oracle: background bodies are not ambiguously presented as foreground prompts; either rehome under an earned `src/agents/subagents/` surface or lock the current `prompts/<id>/SYSTEM.md` convention with README/tests that explain why no rehome is needed now. |
