...
- ✓ `src/dev/execution-comparison/__tests__/brunch-lane.slow.test.ts` — one real Opus 4.8 Brunch `empty_dir` run starts from the frozen empty base/public packet, reaches `promotion_prepared`, never reaches `landed`, preserves run/Petri evidence and the complete `runBaseSha..reviewSha` tree, passes the same controller build/browser/reference-model suite, and leaves no live process/session.
