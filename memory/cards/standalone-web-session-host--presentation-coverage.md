...
| B2 · `present_review_set` offer (review proposal) + `request_review` continuation + committed receipt | `built` | ● | earned | `session-presentation.ts` + `session.tsx` | Shared projection validates the present and terminal details once, preserves ordered proposal nodes/edges/display/continuation plus every terminal outcome and exact approved `MutateGraphSuccess`; React renders proposition-first semantic HTML and no acceptance controls. A distinct production approve → one commit → settlement → reconnect witness preserves the separate B1/B3 witnesses. FE-1187 retains visual-polish ownership. |
