...
| A3 · `ask` single-select choice (options + selected `choice` + Other/`comment`) | `spec` | ● | earned | `session-presentation.ts` + `session.tsx` | Answered detail carries `answered.choice` + `options[]`; projection currently ignores it and React renders a free-text input. Close: project choice + options; render a selected-choice control. Source: `projectAsk` choice branch. Oracle: projection choice no-loss + adapter renders selection and can answer via `session.answerExchange`. |
