...
| A4 · `ask` multi-select choices (options + `choices[]` + Other/`comment`) | `built` | ● | earned | `session-presentation.ts` + `session.tsx` | Shared projection preserves canonical `multiple: true` as explicit multi-select mode plus `answered.choices[]`, option echo, and optional Other/comment. React renders the durable selection list and a live checkbox control; listed choices answer through the existing comma-delimited `session.answerExchange` string contract. Oracles: projection no-loss, adapter render/answer, focused headless, production-entry, differential, concurrency, and RPC contract suites. D125-L ceiling remains: live Other/comment collection is interactive-only. |
| A5 · `ask` bounded questionnaire terminal | `built` | ● | earned | `session-presentation.ts` + `session.tsx` | Shared projection preserves the canonical fixed questionnaire order, each keyed free-text/single-select/multi-select question-answer pair, and `accepted_abstract`; React renders ordered readable answer labels/text plus the accepted abstract from that semantic projection. No second live answering path was added. Oracles: projection order/no-loss over all answer kinds + React read-back; focused questionnaire, differential, concurrency, and RPC regressions. |
...
| Capability | Status | Req | Fill | Owner / next | Notes |
| --- | --- | --- | --- | --- | --- |
| B1 · `present_candidates` offer (proposal cards) + `request_choice` continuation | `built` | ● | earned | `session-presentation.ts` + `session.tsx` | Shared projection validates `zPresentCandidatesDetails` once, preserves ordered candidate/rubric/reference semantics plus the declared same-exchange `ask`/`request_choice` linkage, and classifies malformed details honestly. React renders accessible proposal cards while the separate canonical `ask` entry remains the answer owner. Settlement/refetch is used rather than adding an offer-live delta. Oracles: projection no-loss + malformed classification; React card render + existing ask answer contract; production-host continuation settlement/reconnect; concurrency regression. |
| B2 · `present_review_set` offer (review proposal) + `request_review` continuation + committed receipt | `spec` | ● | earned | [`standalone-web-session-host--review-receipt-presentation.md`](standalone-web-session-host--review-receipt-presentation.md) | Blocked-row re-orient found validated `request_review` details drop the `MutateGraphSuccess` receipt that D27-L/I15-L already require. User chose to keep closure inside FE-1200 rather than promote a new frontier. Two serial slices first restore the receipt-bearing terminal contract, then project/render the review set + decision + receipt and return here to mark B2 built. FE-1187 retains visual-polish ownership. |
| B3 · `present_digest` offer (digest prose) + feedback / questionnaire-confirmation continuation | `built` | ● | earned | `session-presentation.ts` + `session.tsx` | Shared projection validates `zPresentDigestDetails` once, preserves digest abstract/analysis/recommendation plus its declared `ask` continuation, and classifies malformed details honestly. Confirmation and questionnaire terminals retain `accepts_digest` + `accepted_abstract`; digest `request_review` terminals retain decision/comment/accepted abstract where present. React renders accessible digest prose and terminal feedback while the existing `session.answerExchange` ask path owns live answering. Settlement/refetch remains authoritative; no offer-live delta. Oracles: projection no-loss/malformed tests, React render/answer, production settle/reconnect, and concurrency regression. |
