...
Promotion enforcement (I52-L: "not treat as globally settled until a command-layer mutation promotes") is implemented as a monotonic transition rule: `patch_node` may set `settlement: 'settled'` on an advisory node, but rejects `settlement: 'advisory'` on an already-`settled` node as `structural_illegal`; `patch_edge` enforces the identical rule (`validateEdgePatch` now takes the existing edge row, matching `validateNodePatchAgainstExisting`'s shape — the row is already fetched by `graph-mutation-planner.ts`'s patch-edge loop, so no new query was needed).
...
`queryGraph`'s `GraphFilter` gained a `settlement` filter so projection/plan/commitment readers can request settled-only reads (`queryGraph(db, specId, { settlement: ['settled'] })`) — this is the concrete mechanism promised by "context/projection rendering exposes settlement so capability-readiness can consult it," since no bespoke "plan reader" or "commitment reader" module exists yet to hard-code the exclusion into. The filter applies to **both** nodes and edges: a settled-only read excludes advisory nodes (`nodeMatchesFilter`) and independently excludes advisory edges even when both endpoints are settled (`edgeMatchesSettlementFilter`) — an edge's own settlement is not implied by its endpoints'. `graph-slice.ts`'s rendering labels advisory nodes inline in the title column (`title (advisory)`) and advisory edges inline in the relation column (`relation (advisory)`, e.g. `required by (advisory)`), so any consumer of the rendered overview sees settlement on both graph-item kinds without a render-shape change.
...
Test coverage: `src/graph/__tests__/settlement.test.ts` — schema defaults, `createSettlement` batch creation (nodes and edges), node and edge promotion, rejected node and edge regression, and two `queryGraph` filter cases (excludes advisory nodes; independently excludes an advisory edge between two settled nodes); two `graph-slice.test.ts` cases for advisory node and advisory edge labeling; a `brunch-data-graph.test.ts` end-to-end case proving the `mutate_graph` tool surface persists and surfaces advisory settlement.

**Fixup round (2026-07-01, same-day scoping-agent review):** the first pass left edge settlement half-enforced — `queryGraph`'s settled-only filter checked only node settlement (an advisory edge between two settled nodes still leaked through), advisory edges weren't labeled in rendering, and `patch_edge` had no monotonic-regression guard (a stray `ceiling:` comment had deferred it). All three are now closed to parity with the node path; the `ceiling:` comment is removed since the deferred work is done, not just re-scoped.
