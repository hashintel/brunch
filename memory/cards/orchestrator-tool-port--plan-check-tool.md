...
- Containing seam: `execute` mode's foreground `orchestrator` agent and the `.pi/extensions` adapter boundary; this slice replaces the standup stub with the first real cook-orchestrator tool.
...
The execute-mode orchestrator can inspect a cook plan through a product-registered, read-only `cook_plan_check` tool whose result contains plan shape plus contract findings.
...
```text
→ execute-mode foreground `orchestrator` prompt
→ runtime policy tool grant / block list
→ `.pi/extensions/orchestrator` Pi tool adapter
...
This is a proving tracer. It scores on proof of life by making execute mode call real orchestrator-derived product code, on invariants by locking the foreground no-direct-write boundary while still exposing orchestration capability, and on uncertainty by testing that the external `brunch cook` plan contract can be ported without shell-wrapping the CLI.
...
- Preserve D90-L-D93-L/I49-L authority: foreground `orchestrator` remains low-privilege; any future write-capable worker must be code-owned and explicitly allowlisted.
