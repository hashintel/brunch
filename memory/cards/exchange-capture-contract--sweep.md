...
| CC-05 | Approved review-set terminal consumes the final accepted proposal payload only and routes persistence through `acceptReviewSet`. | have | ● | earned | RPC/probe + SPEC cross-check | Witness: existing `src/rpc/__tests__/handlers.test.ts` approval case drives `session.submitExchangeResponse` over a pending `present_review_set`, returns `review.status: approved` with `lsn`/`createdNodes`, invalidates graph reads, and observes the accepted graph node as `basis: explicit`; `src/graph/TOPOLOGY.md` / `src/rpc/TOPOLOGY.md` already name `CommandExecutor.acceptReviewSet` as the review-set approval mutation path. |
| CC-06 | Review `request_changes` captures no prior proposal payload; the comment is direct user material and the successor proposal is the next offer. | built | ● | earned | D108-L recovery-seam probe | Witness: `src/agents/skills/ingest/SKILL.md` says `request_changes` captures only the comment and treats the regenerated review set as the next offer; `src/probes/__tests__/exchange-capture-contract-proof.test.ts` pins `request_review` details as decision/comment only, with no proposal payload or graph result fields. |
| CC-07 | Review `reject` kills the offer rather than demoting it to scratchpad. | built | ● | earned | conduct guidance + exchange-proof probe | Witness: `src/agents/skills/ingest/SKILL.md` states that `reject` kills the offer and must not become a scratchpad obligation/live graph material; `src/probes/__tests__/exchange-capture-contract-proof.test.ts` pins reject details as decision/comment only, with no proposal or scratchpad payload. |
| CC-08 | Cancelled proposal chains contribute no offer payload; only unresolved intent may become scratchpad. | built | ● | earned | conduct guidance + exchange-proof probe | Witness: `src/agents/skills/ingest/SKILL.md` states cancelled proposal-chain reviews carry no offer payload and only unresolved intent can become an `open` obligation; `src/probes/__tests__/exchange-capture-contract-proof.test.ts` pins cancelled `request_review` details with no answered/proposal/graph result payload. |
| CC-09 | Superseded prior proposal entries stay transcript history; projection/capture consume only the accepted terminal payload. | built | ● | earned | SPEC I57-L + chain probe | Witness: `memory/SPEC.md` I57-L already states accepted-terminal-only superseding proposal-chain capture; `src/agents/skills/ingest/SKILL.md` now carries the same model-facing conduct; `src/probes/__tests__/exchange-capture-contract-proof.test.ts` pins the SPEC + conduct wording. |
| CC-10 | `present_candidates` accepted terminal is recognition/fan-in material, not graph truth; non-picked candidates remain offer history. | built | ● | earned | propose/project/map conduct guidance + exchange-proof probe | Witness: `propose` and `project` already state recognition-only conduct; `src/agents/skills/map/references/routing.md` now prevents map/routing from treating accepted candidate selections as graph truth; `src/probes/__tests__/exchange-capture-contract-proof.test.ts` pins all three homes. |
...
| CC-12 | Sweep trigger remains per-turn/watermark-shaped: request terminal toolResults and user/assistant messages are sweepable; present offers are not. | built | ● | earned | `src/projections/session/sweep-watermark.test.ts` | Witness: `src/projections/session/sweep-watermark.test.ts` now pins `present_*` and reserved `capture_*` toolResult exclusion while `request_answer` / `request_choice` remain sweepable; implementation stays prefix-only and outcome-blind. |
...
| CC-14 | The five governing invariants are stated in the live model-facing homes that perform capture/ingest/elicitation. | built | ● | earned | `src/agents/skills/{ingest,map,elicit}/` + `map/references/routing.md` | Witness: `src/agents/skills/ingest/SKILL.md`, `src/agents/skills/elicit/SKILL.md`, and `src/agents/skills/map/references/routing.md` now carry the governing conduct; `src/probes/__tests__/exchange-capture-contract-proof.test.ts` pins cancel→scratchpad, reject→dead, accepted-terminal-only, offer-scoped spans, and per-turn/watermark-shaped trigger wording. |
| CC-15 | `acceptReviewSet` result honesty makes final persisted codes clear enough for the reader that observes approval. | built | ● | earned | `formatRequestReview` render change + result-honesty test | Witness: `formatRequestReview` can render accepted review-set persistence as `ref → code`; `session.submitExchangeResponse` appends the accepted `lsn`/created-node codes into the transcript tool-result text; `request-response.test.ts` and the RPC approval test pin `requirement-draft → REQ1`. |
| CC-16 | No forbidden carrier is introduced while closing the sweep. | built | ● | earned | architecture/no-go audit | Witness: `src/probes/__tests__/exchange-capture-contract-proof.test.ts` audits `src/**` for forbidden capture receipt/result, outcome-span, and exchange-linkage/provenance carrier names. |
