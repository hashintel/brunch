...
| CC-02 | Answered choice/choices requests treat selected choice(s) and required comment as response material; non-selected option echo is render context, not capture payload. | spec | ● | earned | conduct guidance + D106-L reference probe | Inputs: `answered.choice(s)`, `answered.options`, D106-L. Oracle: probe or unit fixture proving non-selected option content is not treated as accepted fact. |
| CC-03 | Cancelled ordinary requests demote the unresolved prompt to scratchpad obligation, not graph truth. | spec | ● | earned | ingest/elicit guidance + exchange-proof probe | Inputs: `cancelled` outcome branches. Oracle: cancelled answer/choice/choices transcript yields no offer-payload commit; optional scratchpad note names the unresolved ask. Guard: scratchpad dispositions stay `open | resolved`; "demote" means adding an `open` obligation, never extending the disposition enum. |
| CC-04 | Unavailable ordinary requests contribute no answer payload and only produce re-ask/scratchpad material when the missing response remains relevant. | spec | ● | earned | ingest/elicit guidance + schema-row probe | Inputs: `unavailable` outcome branches. Oracle: unavailable result is not confused with user refusal or accepted content. Schema evidence: `unavailable.message` is required while `cancelled.message` is optional. Wart to avoid accidental cleanup unless scoped: `request_choice` cancelled/unavailable branches retain `tool_meta.next` while the other request kinds omit it. |
