...
| 8 | `executor-family` — 32 schemas | built | ● | earned | registry closure test | Pure TypeBox relinks retain their emitted shape; the aggregate oracle must include both artifact tools omitted by the current 48-tool allowlist projection. |
| 9 | `web-tools-family` — 2 schemas | built | ● | earned | web-tool tests | Git history showed a pure TypeBox relink, not the scoped literal→TypeBox rewrite; the exact two-tool production inventory has adapter provenance/legality, and the row-8 TypeBox identity/serialization oracle proves shape preservation. |
| 10 | `subagents-family` — 2 schemas | built | ● | earned | subagent catalog + registry closure test | Aggregate provenance/legality oracle includes sealed-child `write_worktree_file`, not only foreground `subagent`. |
