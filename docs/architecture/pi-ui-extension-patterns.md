...
| Registered structured-exchange tool seam | Brunch result-builder/schema tests cover self-contained `toolResult.details`; exchange projection now classifies terminal structured-exchange results as response-side entries. | Continue classifying by typed details, not tool name, so unrelated tool results remain prompt-side. |
