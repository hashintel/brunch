...
## 4c. Measured run (2026-06-23 — FE-1049 graph-canvas, ~85 min, read via P0 telemetry)

The first real multi-slice cook run, measured with P0's per-action usage/timing. **The numbers overturn the topology-based ranking above.**

```
PHASE BREAKDOWN (sum across all slices)
test-authoring  ████████████████████████  3860s  64m  75%
implementation  ████████                   1255s  21m
epic verify ×3  ███                          690s  11.5m
                              tests : code  =  3.07 : 1

CRITICAL PATH  (serial DAG floor ≈ 5100s)
types(282 solo) → foundation verify(179) → canvas-join(900) → canvas verify(285)
                → route-join(1260) → route verify(226) → land
  ▔▔▔ join + route slices own ~3460s of the path, and the cost is test-authoring ▔▔▔
```

**Headline:** 75% of wall-clock is *test-authoring*, and the critical-path join/route slices are expensive because agents **rediscover harness facts every slice** (code-splitting, headless React Flow, router seams). The original P0–P5 set optimized execution/caching; the measured cost is rediscovery. So the ranking changes:

| Lever | Mechanism | Est. saving |
| --- | --- | --- |
| **① harness prior-art prime** *(NEW, biggest)* | Inject the harness recipe into join/route slices so agents stop re-deriving it | ~700–900s |
| **② cheap foundation oracle** *(NEW)* | `graph-types` solo-gates the fan-out; swap runtime-TS-compile (227s) for a type-probe | ~190s |
| **③ stagger-prime** *(= P1c)* | Release 1st cold sibling, let its `cacheWrite` prime, fire siblings as cache reads | cold-start ×(N−1)/batch |
| **④ overlap verify/scheduling** *(NEW)* | Overlap the non-dependent semantic step + serial epic-verifies off the CP | ~150–220s |
| **⑤ session retention** *(= P1a)* | Keep the pi prefix warm across actions | blocked on pi API |

Combined ①+②+④ on the critical path ≈ **15–20 min off an 85-min run, no parallelism change**.

## 5. Recommended sequence (post-measurement)

1. **P0 — instrument** ✅ landed; the measured run above is its payoff.
2. **① harness prior-art prime** — biggest measured win, brunch-owned; needs `ln-scope` (prior-art source + injection point is a design choice).
3. **② cheap foundation oracle** — small, all on the CP, unblocks the fan-out sooner.
4. **③ stagger-prime (P1c)** — the parallel-slice caching win; release signal = observed `cacheWrite`.
5. **④ overlap verify/scheduling**, then **P3** (demoted, ~13%), **P2** (pool tuning); **P1a** blocked, **P1b/P4/P5** follow-ups.

The earlier "P3-first" guess is superseded: P3/verify is ~13% of the run, while test-authoring rediscovery (lever ①) is the real floor.
