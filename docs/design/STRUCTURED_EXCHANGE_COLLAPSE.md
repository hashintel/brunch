...
> Status: **built and closed** (single-terminal rename + present-side `present_question` merge + review collapse all landed 2026-06-23). `request_response` is the sole terminal, dispatching `present_question` and `present_review_set` by pending present tool; `request_review` is retired as a tool but preserved as a result-detail discriminant (D108-L). The formerly "remaining" pieces are absorbed: the review-set nested payload is now fully typed at the boundary (`zReviewSetDetailsPayload` + node/edge draft schemas in `src/exchanges/schemas/present.ts`, D105-L), and the discriminant-companion contract landed with the D108-L `src/exchanges/` consolidation (`request_answer`/`request_choice`/`request_choices`/`request_review` survive only as transcript detail discriminants). No open work is tracked against this document.
