...
> Status: **historical / superseded by ask cutover**. This note records the 2026-06-23 `request_response` terminal design; after D116-L, `ask` is the only active interactive structured-exchange terminal. `present_question` / `request_response` names below are legacy persisted transcript vocabulary only; `request_answer`/`request_choice`/`request_choices`/`request_review` survive only as transcript detail discriminants. No open work is tracked against this document.
