...
| Secondary-chat response                      | `handleSecondaryChatMessageRequest()` in `src/server/secondary-chat-route.ts`                                                                                               | side-chat assistant agent                                           | `chat` + `turn` under secondary `chat_id`                                                                                 | Generate refinement discussion around pinned graph item; durable turns via `appendSecondaryChatTurn` | proposal-only tools in edit mode                                       | `strong enough`                                                              | Legacy stateless `POST …/side-chat` removed after FE-716; secondary-chat routes are the sole runtime.                                                                                                                             |
