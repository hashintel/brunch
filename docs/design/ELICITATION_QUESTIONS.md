...
`milestone`, `frontier`.
*Activating concepts:* walking skeleton, tracer-bullet slices, sequencing, risk retirement. The plan plane stops at the reviewed `scope` handoff; buildable slicing is downstream execution state, not a plan node (D103-L/D126-L).
