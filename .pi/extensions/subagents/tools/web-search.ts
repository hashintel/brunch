...
const pushStructuredSnippet = (snippet: string, lines: string[]): boolean => {
  ...
  try {
    ...
    if (typeof tableContainer.caption === 'string') {
      lines.push(`**${tableContainer.caption}**`);
    }
...
const formatBraveContext = (data: BraveContextResponse): string => {
  ...
  for (const result of data.grounding?.generic ?? []) {
    ...
    for (const snippet of result.snippets) {
      ...
      if (!pushStructuredSnippet(clean, lines)) {
  ...
  if (poi) {
    ...
    for (const snippet of poi.snippets) {
      lines.push(stripHtml(snippet.trim()), '');
    }
    sections.push(lines.join('\n'));
  }
...
const searchBraveContext = async (params: WebSearchParams, signal?: AbortSignal): Promise<string> => {
  ...
  if (!apiKey) {
    throw new Error('Missing Brave API key. Set BRAVE_API_KEY in the environment.');
  }
  ...
  if (params.threshold) {
    body.context_threshold_mode = params.threshold;
  }
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    renderCall(args, theme, context) {
      ...
      const meta = [
        params.freshness,
        params.maxTokens ? `${params.maxTokens}tok` : undefined,
        params.threshold,
      ]
        .filter(Boolean)
        .join(', ');
    ...
    renderResult(result, { expanded, isPartial }, theme, context) {
      ...
      const previewLines = expanded ? lines.slice(0, 20) : lines.slice(0, 8);
