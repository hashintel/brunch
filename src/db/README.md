...
- **Migrations** (`../../drizzle/`) — generated by `npm run db:generate` from
  `src/db/schema.ts` and run by `createDb`. Custom data/bootstrap statements
  that are part of schema initialization, such as the singleton `graph_clock`
  seed row, live in migrations.
