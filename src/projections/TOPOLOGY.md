...
| `session/sweep-watermark`             | 1         | ✓           | FE-861 D80-L sweep-window projection. `sweep-watermark.test.ts` locks the transcript-backed marker, conversational tail classification, raw-background and legacy digest-custom exclusion, `present_*`/reserved `capture_*` toolResult exclusion with `request_*` terminal inclusion (FE-1135/FE-1136), monotonic idempotent advance, and graph-LSN watermark separation. |
