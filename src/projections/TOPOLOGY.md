...
| `exchanges/request-choice`            | 6         | ✓           | `request-choice.test.ts` (direct); constructs D106-L `answered.options` echoes from pending-present choices/candidates.                                                                                                                                                                        |
...
| `exchanges/present-review-set`        | 5         | ✓           | Keep-transitive — `.pi/__tests__/structured-exchange-present-request.test.ts` proves proposal payload projection, including D107-L `proposed_code` preservation, while `session/structured-exchange-loop.test.ts` and `probes/project-graph-review-cycle-proof.test.ts` prove review-mode reconstruction and downstream use. |
...
| `exchanges/request-choices`           | 6         | ✓           | Keep-transitive — `.pi/__tests__/structured-exchange-present-request.test.ts` proves multi-select persistence and comment rules, and `session/exchange-projection.test.ts` proves the terminal tuple reconstruction; constructs D106-L `answered.options` echoes from pending-present options. |
