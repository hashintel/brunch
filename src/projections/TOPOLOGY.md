...
| `session/session-presentation`       | 2         | ✓           | `session-presentation.test.ts` — stable semantic ordinary-message, full ask-family, and candidate/digest/review-set offer identities from canonical JSONL; present and terminal details validate once, unresolved validated provider-authored standalone asks project as one semantic open control beside assistant text and converge to their correlated terminal, exact recognized non-offer present-tool failures are omitted while non-exact rivals fail closed without preventing a later open ask, malformed details classify honestly, and approved review terminals preserve the exact graph receipt. `live-session-events.ts` projects cumulative Pi updates and only real `agent_settled` as the convergence boundary. |
