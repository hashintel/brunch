...
| `session/session-presentation`       | 2         | ✓           | `session-presentation.test.ts` — stable semantic message/ask identities from canonical JSONL; malformed Brunch ask details fail as a classified projection result. `live-session-events.ts` projects cumulative Pi updates and only real `agent_settled` as the convergence boundary. |
