...
| `session/live-session-events`        | 2         | ✓           | Projects Pi's typed `assistantMessageEvent.text_delta` directly into transport-neutral semantic deltas and treats only real `agent_settled` as convergence. Non-text and malformed update rivals fail closed. Consumed by the TUI adapter, standalone host, and React/runtime-contract witnesses; focused tests lock run identity, ordinary text, settlement, and malformed-event behavior. |
