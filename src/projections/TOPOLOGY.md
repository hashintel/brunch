...
| `session/sweep-watermark`             | 1         | ✓           | FE-861 D80-L sweep-window projection. `sweep-watermark.test.ts` locks the transcript-backed marker, conversational/digest tail classification, raw-background exclusion, `present_*`/reserved `capture_*` toolResult exclusion (FE-1135), monotonic idempotent advance, and graph-LSN watermark separation.              |
