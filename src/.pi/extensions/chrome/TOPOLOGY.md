...
- **Input editor**: `registerBrunchChrome` installs `BrunchEditorComponent` through `ctx.ui.setEditorComponent` on `session_start` when present. Its labels and border color are sampled fresh on every render: top-right operational mode and mode-keyed border role from `projectBrunchAgentState(ctx.sessionManager.getEntries())`, bottom-right spec title from `BrunchChromeState`, and a below-line sidecar URL when available.
