...
| File                                                             | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`agents.ts`](./agents.ts)                                       | Markdown agent loader: tiny frontmatter parser (no YAML dep), TypeBox-validated schema (`name`, `description`, `tools`, `skills`, `model`, `thinking`), explicit `BACKGROUND_SUBAGENT_IDS` registry, `loadSubagentDefinitions(dir, ids?)` over `src/agents/subagents/<id>.md` → `Map<name, def>`. Projects frontmatter into the shared `AgentManifest` background shape and fails loud on malformed/duplicate/id-drifted agents. |
| [`config.ts`](./config.ts)                                       | TypeBox loader for [`config.json`](./config.json) (`version`, `maxConcurrency`; tolerates `$comment`).                                                                                                                                                                                                                                                                                                                 |
| [`prompt-assembly.ts`](./prompt-assembly.ts)                     | Background prompt assembler: agent body + child-control header + injected world snapshot + exact readable skill grants + background router rules. It never emits the foreground's full skill manifest; no-tools children rely on complete task/body guidance and receive no resource-read instruction.                                                                                                                                                          |
...
skills: analyze           # exact live skill ids; legal only with `read`
...
Skill resolution is capability-honest: frontmatter may name exact code-owned live skill ids only when the same manifest grants `read`; prompt assembly filters the registry to those names. Empty grants emit neither `<brunch-skills>` nor a resource-read instruction, so no-tools bodies/tasks must carry their complete rubric.
