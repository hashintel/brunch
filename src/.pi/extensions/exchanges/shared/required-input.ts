 ...
 * cancel it). Dismissing an available input resolves as cancelled; a missing
 * input capability resolves as unavailable so the caller can leave the turn
 * reactive and let the model reroute.
 */
...
export async function collectRequiredInput(
  ...
  for (;;) {
    ...
    if (value === undefined) return { status: 'cancelled' };
