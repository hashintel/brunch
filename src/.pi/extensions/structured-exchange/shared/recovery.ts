...
function isPresentToolName(value: unknown): value is PresentToolName {
  return typeof value === 'string' && PRESENT_TOOLS.includes(value as PresentToolName);
}

function isRequestToolName(value: unknown): value is RequestToolName {
  return typeof value === 'string' && REQUEST_TOOLS.includes(value as RequestToolName);
}
...
export function isStructuredExchangeRequestDetails(
  ...
  if (value.status !== 'answered' && value.status !== 'cancelled' && value.status !== 'unavailable') {
    return false;
  }
...
export function findIncompleteStructuredExchangePresents(
  ...
  return [...presents.values()].filter((present) => !completed.has(present.details.exchangeId));
}
