...
export const RequestChoiceParams = Type.Object({
  exchangeId: Type.String({
    description:
      "The structured exchange id from the corresponding present_* entry.",
  }),
  respondsToPresentTool: Type.Union([
    Type.Literal("present_options"),
    Type.Literal("present_candidates"),
  ]),
  prompt: Type.String({
    description:
...
function choiceByLabel(
  ...
  return choices.find(
    (choice) => choice.label === selected || choice.id === selected,
  )
}
    ...
    if (!choice) {
      const other =
        typeof ctx.ui.input === "function"
          ? await ctx.ui.input("Other", "Describe your answer")
          : undefined
      ...
      comment =
        (await ctx.ui.input(params.commentPrompt ?? "Optional comment")) ?? ""
    }
