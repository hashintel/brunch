 ...
 * (agent-runtime/execute-run-updates): a state-advancing execute tool must
...
const AGENT_RUNTIME_ROOT = join(process.cwd(), 'src', '.pi', 'extensions', 'agent-runtime');
...
function executeToolDirs(): string[] {
  return readdirSync(AGENT_RUNTIME_ROOT, { withFileTypes: true })
...
describe('execute tool family side-effect contract', () => {
  it('every non-exempt execute tool module exposes sideEffects in its details payload', () => {
    ...
    for (const dir of executeToolDirs()) {
      ...
      const source = readFileSync(join(AGENT_RUNTIME_ROOT, dir, 'index.ts'), 'utf8');
  ...
  it('the observer still special-cases the exempt driver tool', () => {
    const observer = readFileSync(join(AGENT_RUNTIME_ROOT, 'execute-run-updates', 'index.ts'), 'utf8');
