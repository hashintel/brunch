...
> {
  return {
    name: BRUNCH_EXECUTE_COOK_SLICE_EXECUTE_TOOL,
    label: 'execute_cook_slice_execute',
    description:
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (typeof cwd !== 'string' || cwd.trim().length === 0) {
        throw new Error('execute_cook_slice_execute requires an active cwd');
      }
        ...
        content: [
          {
            type: 'text' as const,
            text: [
              `execute_cook_slice_execute: ${result.status}`,
