...
import { exportCookPetri, type CookPetriExportResult } from '../../../../orchestration/cook-petri.js';
...
> {
  return {
    name: BRUNCH_EXECUTE_COOK_PETRI_EXPORT_TOOL,
    label: 'execute_cook_petri_export',
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (typeof cwd !== 'string' || cwd.trim().length === 0)
        throw new Error('execute_cook_petri_export requires an active cwd');
        ...
        content: [
          {
            type: 'text' as const,
            text: [
              `execute_cook_petri_export: ${result.status}`,
