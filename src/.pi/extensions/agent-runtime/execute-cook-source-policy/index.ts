...
> {
  return {
    name: BRUNCH_EXECUTE_COOK_SOURCE_POLICY_TOOL,
    label: 'execute_cook_source_policy',
    description:
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (typeof cwd !== 'string' || cwd.trim().length === 0) {
        throw new Error('execute_cook_source_policy requires an active cwd');
      }
        ...
        content: [
          {
            type: 'text' as const,
            text: [
              `execute_cook_source_policy: ${result.status}`,
