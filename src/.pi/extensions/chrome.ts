...
export function projectBrunchChromeFooterLines(
  ...
  const identity = `${formatChromeIdentity(chrome)}${branch ? ` · branch: ${branch}` : ''}`;
...
export function renderBrunchChrome(ui: BrunchChromeUi, chrome: BrunchChromeState): void {
  ui.setHeader(() => ({
...
function formatContextUsage(usage: BrunchChromeContextUsage | undefined): string {
...
function formatWorker(chrome: BrunchChromeState): string {
  ...
  return [worker.stage, worker.status].filter(Boolean).join('/') || 'not reported';
}
