...
`agent-runtime/system-prompts/` projects the current foreground prompt from the active branch and upserts exactly one stable `<brunch-foreground-prompt>` block through both `before_agent_start` and `before_provider_request` (D134-L/I67-L). `shared/provider-system-prompt.ts` owns the supported provider carrier matrix: prompt strings, string/block `system`, and system/developer message content. Identical recomposition returns the original carrier; changed role, elicitation style, active tools, or resource composition replaces the marked block while preserving unrelated provider content. Heading text is never used as identity.
