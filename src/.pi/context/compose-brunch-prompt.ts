...
function readPromptPack(id: PromptPackId): BrunchPromptPack {
  return {
    ...
    markdown: readFileSync(new URL(`./prompt-packs/${id}.md`, import.meta.url), 'utf8').trim(),
  };
}
...
export function composeBrunchPrompt(state: BrunchPromptCompositionState): BrunchPromptCompositionResult {
  const packSections = PROMPT_PACKS.map((pack) => pack.markdown);
  const dynamicSections = [renderGraphContext(), renderReadinessContext(), renderStructuredExchangeContext()];
