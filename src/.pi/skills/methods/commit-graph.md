...
Invoke `mutate_graph` when the batch can be validated atomically and the user-facing commitment is already settled. For direct agent commits in the current product posture, keep the batch create-only: `create_node` ops plus role-named `create_edge` ops. On `structural_illegal`, use diagnostics to repair and retry within the current strategy's budget; do not expose half-written state or manually patch around CommandExecutor. On ambiguity, stop and ask or route through a proposal/review strategy.
