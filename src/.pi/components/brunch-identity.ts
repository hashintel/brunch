...
export function formatBrunchProductIdentity(options: BrunchProductIdentityOptions): string[] {
  ...
  return [...logo, ...(logo.length > 0 ? [''] : []), ...wordmark, '', versionLine, ...devLine, piLine];
}

function identityStyle(options: BrunchProductIdentityOptions, color: ThemeColor, text: string): string {
...
function cropLogo(lines: string[]): string[] {
  const cropped = [...lines];
  while (cropped.length > 0 && stripAnsi(cropped[0]!).trim().length === 0) cropped.shift();
  while (cropped.length > 0 && stripAnsi(cropped[cropped.length - 1]!).trim().length === 0) cropped.pop();
