...
import type { AskUserQuestionResultDetails } from "./pi-extensions/structured-exchange.js"
...
const scenario = {
  mission:
    "Complete an option-based structured exchange as an agent-as-user evaluator.",
  evaluationFocus:
...
export async function runStructuredExchangeRpcProof(
  ...
  const cwd =
    options.cwd ?? (await mkdtemp(join(tmpdir(), "brunch-exchange-rpc-proof-")))
  ...
  try {
    ...
    const editorRequest = await client.waitFor(
      (event): event is StructuredExchangeRpcProofResult["editorRequest"] =>
...
async function writeProofExtension(cwd: string): Promise<string> {
  ...
  const adapterPath = resolve("src/pi-extensions/structured-exchange.ts")
  const content = `
...
function isEditorRequest(
  value: unknown,
): value is StructuredExchangeRpcProofResult["editorRequest"] {
  return (
    typeof value === "object" &&
    value !== null &&
