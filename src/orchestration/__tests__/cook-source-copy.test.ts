...
import { selectCookSourcePolicy, sourcePolicyPath } from '../cook-source-policy.js';
...
async function createPolicyRun(cwd: string, policy: 'plan_only' | 'host_source_deferred'): Promise<void> {
  ...
  await selectCookSourcePolicy({ cwd, runId: 'run-1', policy });
}

describe('copyCookHostSource', () => {
  it('does not copy when source policy has not been selected', async () => {
    ...
    const result = await copyCookHostSource({ cwd, runId: 'run-1' });
  ...
  it('skips host source copy for plan_only policy', async () => {
    ...
    expect(await pathExists(join(cookWorktreeDir(cwd, 'run-1'), 'src', 'app.ts'))).toBe(false);
  });
  ...
  it('copies host source entries into the worktree while excluding cook state and heavy dirs', async () => {
      ...
      sideEffects: [
        {
          ...
          to: join(cookWorktreeDir(cwd, 'run-1'), 'package.json'),
        },
        { kind: 'copy_entry', from: join(cwd, 'src'), to: join(cookWorktreeDir(cwd, 'run-1'), 'src') },
    ...
    expect(await readFile(join(cookWorktreeDir(cwd, 'run-1'), 'src', 'app.ts'), 'utf8')).toBe(
      'export const app = true;\n',
    );
    expect(await pathExists(join(cookWorktreeDir(cwd, 'run-1'), '.brunch', 'debug', 'skip.txt'))).toBe(false);
    expect(await pathExists(join(cookWorktreeDir(cwd, 'run-1'), '.git', 'HEAD'))).toBe(false);
    expect(await pathExists(join(cookWorktreeDir(cwd, 'run-1'), 'node_modules', 'dep', 'index.js'))).toBe(
      false,
    );
    expect(await pathExists(join(cookWorktreeDir(cwd, 'run-1'), 'dist', 'bundle.js'))).toBe(false);
