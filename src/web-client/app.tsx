...
export function createBrunchWebRouter(options: { queryClient: QueryClient; rpcClient: WebSocketRpcClient }) {
  return createRouter({
    routeTree,
...
function sessionProjectionTargetFromSnapshot(snapshot: WorkspaceSnapshot): SessionProjectionTarget | null {
...
function sessionTranscriptDisplayQueryOptions(
  ...
  return {
    queryKey: ['session.transcriptDisplay', target?.sessionId ?? null, target?.specId ?? null],
    queryFn: () =>
