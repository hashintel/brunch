...
1. On an overlay-capable host, launch an isolated Pi process with `PI_CODING_AGENT_DIR="$(mktemp -d)" pi -e npm:pi-interactive-shell`. Use hands-free sessions so the agent can query bounded output and send text/named keys while a human watches, types to take over, and presses `Ctrl+G` to return control. Health-check import/spawn, resize, status, kill, and empty cleanup after any Pi/package/Node/OS/architecture change.
2. In a sandbox or headless environment where socket-backed execution fails, use the project-owned `npm run tui-driver` Expect/headless-xterm workflow documented in [`../../docs/praxis/manual-testing.md`](../../docs/praxis/manual-testing.md). Always `stop`, `rm`, then confirm `list` is empty.
