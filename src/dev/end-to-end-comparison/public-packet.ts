...
export async function materializeExactExecutionPacket(input: {
  ...
  const template = parsePublicCaseContract(
    parseJson(await readFile(input.publicContractTemplatePath, 'utf8')),
  );
  ...
  try {
    ...
    await writeFile(join(input.packetDir, 'public-contract.json'), `${JSON.stringify(contract, null, 2)}\n`, {
      encoding: 'utf8',
      flag: 'wx',
    });
