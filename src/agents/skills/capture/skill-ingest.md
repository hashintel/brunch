...
To wire this as a quarantined compatibility resource, it would become `src/agents/skills/_suspended/methods/ingest/SKILL.md` enumerated inside `agents/runtime/_suspended/`. A live version should instead land under an activity home such as `src/agents/skills/capture/` or `src/agents/skills/elicitation/` once the elicitor needs advertised prompt resources again. The four current acquisition modes either collapse into the source branch here or shrink to thin trigger-shells that delegate to it; `capture` keeps the banded sweep (this skill cites it rather than duplicating it). That restructuring touches the sealed skills tree and is out of scope for this drafting pass.
