...
Use the live capture references under `src/agents/skills/capture/references/` when this quarantined method needs current graph-writing judgment. For ordinary capture, prefer `intent-capture.md` plus `graph-heuristics.md`; pull design/oracle/plan guidance only when the user actually supplied or requested that material. Exact node/edge vocabulary is schema-owned under `src/graph/schema/**` and `src/graph/policy/**`; readiness terminology lives in `src/agents/contexts/about/readiness-bands.md`; shared graph-authoring judgment — declarative claims, low-confidence routing, contradiction routing, confident endpoints, and role-named mutation grammar — lives in `src/agents/skills/capture/references/graph-heuristics.md`.
...
Do not invent graph payload fields, LSNs, result shapes, or capture-local edge syntax. Follow the role-named mutation grammar in `graph-heuristics.md`.
...
Review captured nodes before adding edges. Use `graph-heuristics.md` for the shared relation-bearing rule: commit missing high-confidence endpoints first, use role-named endpoints, and skip the edge when either endpoint is low-confidence. Spawn or reuse a gap for the missing endpoint/relationship instead.
