...
This quarantined method predates the live activity homes. Current graph kind/edge vocabulary is schema-owned under `src/graph/schema/**` and `src/graph/policy/**`; readiness terminology lives in `src/agents/references/readiness-bands.md`; current graph-mapping and routing judgment lives under `src/agents/skills/map/references/`.
