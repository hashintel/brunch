...
Use the live capture references under `src/agents/skills/capture/references/` when this quarantined method needs current graph-writing judgment. For ordinary capture, prefer `intent-capture.md` plus `graph-heuristics.md`; pull design/oracle/plan guidance only when the user actually supplied or requested that material. The exact node/edge vocabulary lives in `src/agents/contexts/references/graph-ontology.md` and the shared graph-authoring judgment — declarative claims, low-confidence routing, contradiction routing, confident endpoints, and role-named mutation grammar — lives in `src/agents/skills/capture/references/graph-heuristics.md`.
