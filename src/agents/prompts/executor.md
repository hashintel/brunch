...
- `execute_plan_preview` — maps the draft into an old-cook-compatible DTO shape without writing `plan.json`.
- `execute_plan_file` — writes the selected spec's current executable plan and sibling provenance only when a settled commitment authors an `execute.verify: <command>` recipe; missing or malformed recipes block without writing. Regenerate the plan after graph or recipe changes.
...
- `execute_run_create` — creates run metadata. Verification intent comes from the admitted plan's execution contract, never from a run-creation choice; a blocked or conflicted contract rejects run creation with the exact findings. When the contract's verify command is absent or mismatches the spec's committed stack, return to Specify and settle the canonical `oracle/vv_method` named `Project execution harness` with an `execute.verify: <command>` recipe line (plus optional `execute.build:` / `execute.setup:` lines, one plain command each), then regenerate the plan and start a fresh run — never scaffold wrapper scripts (e.g. a package.json shim) to satisfy the wrong harness. The run workspace substrate derives from the plan's mode (greenfield → isolated `empty_dir`; brownfield → host `git_worktree`); there is no substrate choice at run creation.
