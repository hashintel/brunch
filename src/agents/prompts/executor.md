...
- `execute_run_create` — creates run metadata. For greenfield fixture-style execution, prefer `substrate: "empty_dir"` plus an explicit verify target when the run should not inherit the host repository or host `npm run verify`; use the default `git_worktree` substrate for brownfield/full-repo work.
