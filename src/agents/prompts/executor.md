...
- `execute_run_create` — creates run metadata. For greenfield fixture-style execution, prefer `substrate: "empty_dir"` plus a product-owned verify profile such as `verifyProfile: "npm_test"` when the run should not inherit the host repository or host `npm run verify`; use the default `git_worktree` substrate for brownfield/full-repo work.
