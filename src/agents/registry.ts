...
/** Current filesystem home for bundled Brunch agent markdown bodies. */
export function bundledAgentBodyHome(): string {
  return fileURLToPath(new URL('../.pi/agents', import.meta.url));
}
...
export function bundledAgentBodyRepoPath(id: BundledAgentBodyId): string {
  return `src/.pi/agents/${id}/SYSTEM.md`;
}

export function bundledAgentBodyLocation(id: BundledAgentBodyId): string {
  return fileURLToPath(new URL(`../.pi/agents/${id}/SYSTEM.md`, import.meta.url));
}
