...
describe('translateEvent', () => {
  ...
  it('translates thinking content_block_delta to reasoning-delta', () => {
    ...
    const events = translateEvent(sdkMessage);
    expect(events).toEqual([{ type: 'reasoning-delta', id: 'reasoning-0', delta: 'Let me think...' }]);
  });
  ...
  it('translates message_stop to finish-step + finish', () => {
    ...
    const events = translateEvent(sdkMessage);
    expect(events).toEqual([{ type: 'finish-step' }, { type: 'finish', finishReason: 'stop' }]);
  });
  ...
  it('translates tool_use content_block_stop to tool-call (finished)', () => {
    ...
    const events = translateEvent(sdkMessage);
    expect(events).toEqual([
      { type: 'tool-call', id: 'toolu_01', toolName: 'get_weather', args: '{"city":"NYC"}' },
    ]);
  });
});
