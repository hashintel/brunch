...
export async function runObserver(
  ...
  const entities = await getEntitiesForSpecification(db, specificationId);
  ...
  for (const item of parsed.goals) {
    const goal = await createKnowledgeItem(db, specificationId, 'goal', item.content, {
  ...
  for (const item of parsed.terms) {
    const term = await createKnowledgeItem(db, specificationId, 'term', item.content, {
      rationale: item.rationale,
    });
  ...
  for (const item of parsed.contexts) {
    const context = await createKnowledgeItem(db, specificationId, 'context', item.content, {
      rationale: item.rationale,
    });
  ...
  for (const item of parsed.constraints) {
    const constraint = await createKnowledgeItem(db, specificationId, 'constraint', item.content, {
  ...
  for (const d of parsed.decisions) {
    const decision = await createDecision(db, specificationId, d.content, d.rationale);
    ...
    for (const parentId of d.parentDecisionIds) {
      await addDecisionParentDecision(db, decision.id, parentId);
    }
    for (const parentId of d.parentAssumptionIds) {
      await addDecisionParentAssumption(db, decision.id, parentId);
    }
  }

  for (const a of parsed.assumptions) {
    const assumption = await createAssumption(db, specificationId, a.content);
    ...
    for (const parentId of a.parentAssumptionIds) {
      await addAssumptionParentAssumption(db, assumption.id, parentId);
    }
  }
