...
| `exchanges/present-question.ts` | `present_question` | ◐ partial | Present result text | Tool result `renderResult` via exchange markdown adapter | Remaining: present-family golden for heading/body/options plus hidden option-id escaping invariant. |
