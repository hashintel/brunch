 ...
 * via referenceCode (or content as fallback identity).
 */
export function computeReviewSetChangeSummary(
  predecessor: { items: readonly { content: string; referenceCode?: string | null }[] },
  successor: { items: readonly { content: string; referenceCode?: string | null }[] },
): ReviewSetChangeSummary {
  const predecessorKeys = new Map<string, string>();
  for (const item of predecessor.items) {
    const key = item.referenceCode ?? item.content;
