...
/** Shared orchestrator run loop — compile plan, interpret net with the given policy. */
export async function runCompiledOrchestrator(
  ...
  try {
    ...
    await net.run(policy, () => ctx.halted);
  } catch (err) {
    return {
      status: 'halted',
