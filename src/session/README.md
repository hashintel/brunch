...
| `agent_runtime_vocab` | `schema/kinds.ts` | `runtime-state.ts`, `projections/session/runtime-policy.ts`, `projections/session/affordances.ts`, `.pi/extensions/runtime/state.ts` | Pure vocabulary leaf for runtime axes and agent-role ids; imports nothing and mirrors D73-L's graph taxonomy direction on the session side. |
