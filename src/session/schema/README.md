...
`kinds.ts` imports nothing. Consumers import these constants/types from the leaf directly when they only need vocab, or from `session/runtime-state.ts` when they are already using the transcript-state API re-export.
