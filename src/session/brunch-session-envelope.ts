...
import { type FileEntry, type SessionEntry } from '@earendil-works/pi-coding-agent';
...
export async function readBrunchSessionEnvelope(file: string): Promise<BrunchSessionEnvelopeReadResult> {
  ...
  const bindings = entries.filter(isSessionBindingEntry).map((entry) => entry.data);
...
export function assertLinearBrunchSessionEnvelope(envelope: BrunchSessionEnvelope): void {
...
function assertFileBackedTranscriptEntries(entries: readonly unknown[]): asserts entries is FileEntry[] {
...
function assertLinearTranscriptEntries(entries: readonly FileEntry[]): void {
  for (const entry of entries) {
    if (isPiSessionHeader(entry) && typeof entry.parentSession === 'string') {
      throw new NonLinearTranscriptError('Brunch does not support branch-derived Pi sessions');
    }

    if (isSessionEntry(entry) && entry.type === 'branch_summary') {
      throw new NonLinearTranscriptError('Brunch does not support Pi branch-summary transcript entries');
    }
  }
  ...
  for (const entry of entries) {
    ...
    if (siblings.length > 1) {
      throw new NonLinearTranscriptError('Brunch does not support non-linear Pi transcript branches');
    }
  }
}
