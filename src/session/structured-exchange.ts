export const STRUCTURED_EXCHANGE_RESULT_SCHEMA = 'brunch.structured_exchange.result' as const;
...
export function isStructuredExchangeResultDetails(value: unknown): value is StructuredExchangeResultDetails {
  if (!isRecord(value)) return false;
  return (
    value.schema === STRUCTURED_EXCHANGE_RESULT_SCHEMA &&
    value.schemaVersion === 1 &&
    (value.status === 'answered' || value.status === 'cancelled' || value.status === 'unavailable') &&
    typeof value.question === 'string' &&
    (value.mode === 'text' || value.mode === 'single-select' || value.mode === 'multi-select') &&
...
export function isTerminalStructuredExchangeResultDetails(
  ...
  return (
    isStructuredExchangeResultDetails(value) && (value.status === 'answered' || value.status === 'cancelled')
  );
}
