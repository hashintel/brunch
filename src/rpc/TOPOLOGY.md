...
`execute.run` updates may carry lightweight live Petri read hints in addition to `runId`: `petriProjectionSource` (`snapshot` or `replay`), `petriProjectionReplayReason` (`snapshot_missing_or_unreadable` or `snapshot_stale`), and ready/blocked frontier hints. Terminal projections retain their journal timestamp and ordered failed-slice ids; Petrinaut replay definitions require finite `x`/`y`, arcs require positive integer weights, and firings expose exactly `{transitionId,input,output,ts}` with fixed millisecond-UTC ISO timestamps. Complete Petrinaut replay exports do not ride invalidation notifications; clients refetch canonical `execute.run` detail. The executor orchestrate path emits a conservative synchronous `snapshot` hint on per-step completion because the drive observer hook is not awaited, while write-side `execute.replan*` RPC mutations can await `readRunDetail(...)` and publish honest replay/snapshot hints from current read-side truth.
