...
The current `src/agents/` seam owns Pi-independent LLM context ingress. Agent bodies live in `src/agents/prompts/`; prompt-resource skills live in `src/agents/skills/`; live Specify-mode elicitor assembly, including its prompt-frame context renderer, lives in `src/agents/runtime/elicitor/`; reusable model-state context renderers live in `src/agents/contexts/data-model/`; suspended pre-D98 controls are isolated under the sibling `suspended/` homes. The old `src/.pi/context/` prompt-pack subtree remains retired.
