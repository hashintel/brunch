...
import { ingestCookAgentResult } from '../cook-agent-result.js';
import { reportsPath } from '../cook-report.js';
import { cookRunDir, cookRunMetadataPath } from '../cook-run.js';
import { sliceExecutionRequestPath } from '../cook-slice-execute.js';
...
describe('ingestCookAgentResult', () => {
  ...
  it('does not ingest until an agent result file exists', async () => {
    ...
    expect(result).toEqual({
      ...
      runId: 'run-1',
      sliceId: 'task-1',
      resultPath: join(cookRunDir(cwd, 'run-1'), 'agent-output', 'task-1', 'result.json'),
      metadataPath: cookRunMetadataPath(cwd, 'run-1'),
      sideEffects: [],
    });
  });
  ...
  it('ingests a prewritten agent result without running tests or Petri', async () => {
    ...
    await createRequestedSliceRun(cwd);
    const resultPath = join(cookRunDir(cwd, 'run-1'), 'agent-output', 'task-1', 'result.json');
    await writeFile(
      resultPath,
    ...
    expect(await pathExists(join(cookRunDir(cwd, 'run-1'), 'tests-ran.json'))).toBe(false);
  });
});
