...
`isolated-slice-operations.ts` is the shared effect core for serial and parallel slice execution: slice-start/request payload and report vocabulary, one agent/verify attempt with attempt-scoped stream files, success/failure report shapes, integration reports, completion reports, and retry disposition. `slice-stream-events.ts` serializes both stream families through one persisted run-scoped journal and mirrors each event to its attempt file. The run journal is reconnect/order authority and appends first, so a mirror failure cannot leave a successfully persisted event hidden behind an incomplete index; reconnect reads this ordering before applying tail limits. Serial `slice-*`/agent/test modules are thin `run.json` adapters. `parallel-slice-batch/effects.ts` composes the same operations under batch journal/marking authority, buffering shared reports only where journal-before-report ordering requires it. Authority adapters remain distinct; effect algorithms do not.

The standalone `execute_slice_complete` tool composes those same two explicit helpers for manual lifecycle driving: integrate first, then complete only after `slice_integrated`. `drive()` keeps them as separate Petri firings. Standalone `execute_slice_start` enters the same process-local run admission as `drive()` and refuses both a contended owner and persisted parallel-batch authority, so it cannot duplicate a co-firing sibling.
