...
`orchestrate.ts` is the run driver (FE-1125, D112-L). Public `drive()` holds `run-execution-authority.ts` admission for the complete call in `finally`: concurrent callers sharing canonical `{cwd, runId}` await one owner outcome/rejection and execute external effects once; different runs remain independent; rejection removes mutex state. This is process-local D124-L admission, not a split-process lease. The production `execute_orchestrate` tool explicitly composes `petriScheduler` + `frontierFiringPolicy`; a frontier with at least two co-firable slice starts delegates to the bounded D123-L batch above even when progress observers are attached. Batch progress remains per-slice and uses the existing drive callbacks, while single-slice waves, default callers, explicit serial policies, and `maxFirings` remain serial. The retry-current-step tool explicitly retains `linearScheduler` + `serialFiringPolicy` + one firing. `petri-runtime.ts` owns lifecycle-to-marking reconstruction, enabled/blocked transitions, and serial step bindings. Transition and terminal facts append fail-closed; host promotion stays outside the driven chain behind explicit acceptance.
