...
SPEC decisions: D111-L (executor cutover over injected ports), D52-L (layer boundary), D112-L / FE-1125 (run driver), D118-L (scope handoff contract), I58-L (bounded execute-mode side effects).
...
| `slices[].scope_id`, `design_context`, `verification_context` | mapped | The D118-L scope handoff preserves committed scope, design, and verification context into `plan.yaml`; the native worker-request path consumes these fields. |
