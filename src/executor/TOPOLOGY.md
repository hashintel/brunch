...
Slice starts may be co-firable in the immutable topology, but effects still rematerialize and execute serially under one active-slice `run.json` authority. Actual overlapping slice effects remain excluded until the durable parallel-authority model is promoted.
...
`orchestrate.ts` is the run driver (FE-1125, D112-L): `drive()` asks a pure `RunScheduler` for the ready frontier and executes bound Petri transitions through injected `ExecutionPorts`, while `run.json` remains serial loop authority. `petri-runtime.ts` owns lifecycle-to-marking reconstruction, enabled/blocked transitions, and step bindings. `petri-terminal.ts` owns exhausted-vs-halted outcomes. The driver can reuse only a marking snapshot claim-set that still matches live lifecycle facts and enabled transitions. Transition and terminal facts append fail-closed to `petrinaut/events.jsonl`; journal subscriptions are the live hint seam, and the residual `DriveContext.onNetEvent` callback has been removed. Attempt failures remain non-marking facts followed by static retry/exhaustion transitions, with shared attempt-counter helpers feeding agent and verify paths. Marking snapshots remain bounded resume hints, never lifecycle authority. Host promotion stays outside the driven chain behind explicit acceptance.
