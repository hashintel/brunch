...
`petri.ts` owns one topology/SDCPN projection writer used twice: `drive()` calls `preparePetriObservation(...)` before its first lifecycle effect to materialize `.brunch/cook/runs/<runId>/petrinaut/net.json`, `net.sdcpn.json`, and a create-only empty `events.jsonl`; the later `petri_export` transition rewrites the same deterministic definitions and records `status:"petri_exported"`. Preparation never advances `run.json` and never truncates an existing journal. The files are Brunch-owned observer projections over the shared executor topology compiler, not runtime authority. Both paths use `petri-runtime-plan.ts`, so they describe the same plan snapshot that drives the run. Compatibility with Petrinaut's loader remains an external consumer oracle, not a claim established by the local shape test.

`petrinaut/replay-export.ts`, `petrinaut/stream-frames.ts`, and `petrinaut/sse.ts` are read/transport projections only. They validate and reduce `net.sdcpn.json` + `events.jsonl` into replay-equivalent ordered frames and SSE chunks. `petri-events.ts` notifies same-process observers only after durable append; the callback is a wake-up hint, while the journal remains replay truth. Metadata-derived terminal states synthesize the same `run:finish` firing as journal-derived terminals. These surfaces never own lifecycle truth, mutate `run.json`, or grant parallel side-effect authority. Process-local wake-ups are the declared first ceiling; split-process execution requires file watching or a durable broker.
