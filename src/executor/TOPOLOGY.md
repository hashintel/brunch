...
├── petri-events.ts       bounded `petrinaut/events.jsonl` path + append and parse helpers for raw runtime facts
...
├── petri.ts              completed run -> topology-aware raw `petrinaut/net.json` + SDCPN v1 `net.sdcpn.json` projections
...
`petri.ts` writes a topology-aware raw Petrinaut artifact at `.brunch/cook/runs/<runId>/petrinaut/net.json` plus an SDCPN v1 import projection at `petrinaut/net.sdcpn.json` for a completed run and records `status:"petri_exported"`. The files are Brunch-owned projections over the shared executor topology compiler plus current run facts (for example run/slice subnet identity, explicit place/arc wiring, and initial marking), not the runtime authority itself. Export uses the same `petri-runtime-plan.ts` resolution as the driver and observer paths, so the raw net describes the same plan snapshot that drove the run. If the required plan input is missing or unreadable, export refuses loudly and leaves the run at `run_completed` rather than reconstructing topology from ambient metadata. Promotion refs and land branches remain deferred. Compatibility with Petrinaut's loader remains an external consumer oracle, not a claim established by the local shape test.

`petrinaut/replay-export.ts`, `petrinaut/stream-frames.ts`, and `petrinaut/sse.ts` are read/transport projections only. They validate and reduce `net.sdcpn.json` + `events.jsonl` into a finite Petrinaut replay payload, ordered frames, and SSE chunks. They never own lifecycle truth, mutate `run.json`, subscribe to process-local execution, or grant parallel side-effect authority. True live-from-start streaming requires a fresh frontier because SDCPN export still occurs only after `run_completed`.
