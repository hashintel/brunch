...
export async function runStructuredExchangeOrderingProof(
  ...
  const cwd =
    options.cwd ?? (await mkdtemp(join(tmpdir(), "brunch-exchange-ordering-")))
  ...
  try {
    ...
    const selectRequest = await client.waitFor(
      (event): event is ExtensionUiRequest =>
        isExtensionUiRequest(event) && event.method === "select",
    )
    child.stdin.write(
    ...
    const inputRequest = await client.waitFor(
      (event): event is ExtensionUiRequest =>
        isExtensionUiRequest(event) && event.method === "input",
    )
    child.stdin.write(
    ...
    if (!promptResponse.success) {
      throw new Error(
        `Ordering proof prompt failed: ${promptResponse.error ?? "unknown error"}`,
      )
    }
    ...
    const present = toolResults.find(
      (result) => result.toolName === "present_options",
    )
    const request = toolResults.find(
      (result) => result.toolName === "request_choice",
    )
    if (!present || !request) {
    ...
    return {
      ...
      verdict: {
        presentResultBeforeRequestUi:
          presentIndex !== -1 &&
          requestUiIndex !== -1 &&
          presentIndex < requestUiIndex,
        jsonlPresentBeforeRequest:
          jsonlPresentIndex !== -1 &&
          jsonlRequestIndex !== -1 &&
          jsonlPresentIndex < jsonlRequestIndex,
      },
      eventOrder,
...
async function readToolResults(
  ...
  return entries.flatMap((entry) => {
    ...
    if (
      message.toolName !== "present_options" &&
      message.toolName !== "request_choice"
    ) {
      return []
    }
