...
import type { DoltCommit, DoltDiffRow } from './types';
...
export type DiffData = { tables: Record<string, DoltDiffRow[]>; from: string; to: string };

export function useVersions() {
    const [commits, setCommits] = useState<DoltCommit[]>([]);
    const [workingDiff, setWorkingDiff] = useState<DiffData | null>(null);
    ...
    const refreshStatus = useCallback(async () => {
        try {
            const data = await apiFetch<DiffData>(
    ...
    const checkout = useCallback(async (hash: string, sessionId: string): Promise<any | null> => {
        setLoadingCheckoutHash(hash);
        try {
            const data = await apiFetch<any>(`/api/versions/checkout/${hash}?sessionId=${sessionId}`);
