...
Disposition: fixed in code on FE-1348 at the standalone/web live-overlay versus canonical-hydration seam. Route regressions now enforce one actionable representation per exchange, terminal/local-confirmation precedence, `ask_closed` convergence, and subscribe-before-resnapshot monotonicity without deduping durable history. Outer closure remains pending: re-enter the FE-1348 cross-surface row only for an authorized journey proving visible-submit/canonical-settlement agreement. Cost/value: the repair prevents a silently committed unintended answer from being reported as failure; the outer witness remains high value before row closure.
