...
| [Cross-surface graph/session settlement](testing/walkthroughs/2026-08-12/cross-surface-graph-session-settlement-rerun/walkthrough.md) | CS1 is fixed in code by `8c23ada95`, `fd10e839c`, and `abb0b8bd6` but not outer-witnessed. The final authorized rerun stopped on stronger SW2 evidence: a duplicate actionable control falsely reported `ask closed` while canonical settlement succeeded with the wrong financial choice, leaving a stale duplicate and derailing conduct. No proposal, approval, or graph effect occurred; the row is wait-gated on SW2 repair. |
...
#### SW2 · duplicate standalone React actionable controls · product-critical · promoted
...
Disposition: promoted to `shared-session-host-cutover`'s standalone/web live-overlay versus canonical-hydration seam. The FE-1348 cross-surface row is wait-gated on SW2 repair before another journey. Re-enter when that frontier deduplicates actionable controls and guards visible-submit/canonical-settlement agreement. Cost/value: this can silently commit an unintended answer while reporting failure and waste the only authorized provider/user attempt; repair is high value and prerequisite to trustworthy settlement evidence.
